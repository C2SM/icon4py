...
!$OMP PARALLEL PRIVATE (rl_start,rl_end,i_startblk,i_endblk)
    ...
    !$ser data z_rth_pr_1=z_rth_pr(:,:,1,1)

    PRINT *, 'Serializing z_rth_pr_2=z_rth_pr(:,:,1,2)'

    !$ser data z_rth_pr_2=z_rth_pr(:,:,1,2)
...
!$OMP            z_theta_tavg_m1,z_theta_tavg,z_thermal_exp_local) ICON_OMP_DEFAULT_SCHEDULE
    ...
    !$ser data exner=p_nh%prog(nnow)%exner(:,:,1)
    ...
    !$ser data exner_pr=p_nh%diag%exner_pr(:,:,1)

    PRINT *, 'Serializing z_exner_ex_pr=z_exner_ex_pr(:,:,1)'

    !$ser data z_exner_ex_pr=z_exner_ex_pr(:,:,1)
          !$ACC PARALLEL IF(i_am_accel_node) DEFAULT(PRESENT) ASYNC(1)
...
!DIR$ IVDEP
    ...
    !$ser data z_exner_ic=z_exner_ic(:,:)
            !$ACC PARALLEL IF(i_am_accel_node) DEFAULT(PRESENT) ASYNC(1)
            !$ACC LOOP GANG VECTOR
            DO jc = i_startidx, i_endidx
...
!DIR$ IVDEP
    ...
    !$ser data z_dexner_dz_c_1=z_dexner_dz_c(:,:,1,1)
            !$ACC PARALLEL IF(i_am_accel_node) DEFAULT(PRESENT) ASYNC(1)
            !$ACC LOOP GANG VECTOR TILE(32, 4)
            DO jk = nlev, MAX(2,nflatlev(jg)), -1
!DIR$ IVDEP
...
!DIR$ IVDEP
    ...
    !$ser data theta_v=p_nh%prog(nnow)%theta_v(:,:,1)
    ...
    !$ser data rho_ic=p_nh%diag%rho_ic(:,:,1)
...
!DIR$ IVDEP
    ...
    !$ser data z_theta_v_pr_ic=z_theta_v_pr_ic(:,:)

    PRINT *, 'Serializing theta_v_ic=p_nh%diag%theta_v_ic(:,:,1)'

    !$ser data theta_v_ic=p_nh%diag%theta_v_ic(:,:,1)

    PRINT *, 'Serializing z_th_ddz_exner_c=z_th_ddz_exner_c(:,:,1)'

    !$ser data z_th_ddz_exner_c=z_th_ddz_exner_c(:,:,1)
#ifdef _OPENACC
          !$ACC PARALLEL IF(i_am_accel_node) DEFAULT(PRESENT) ASYNC(1)
          !$ACC LOOP GANG VECTOR COLLAPSE(2)
          DO jk = 2, nlev
!DIR$ IVDEP
    ...
    !$ser data w=p_nh%prog(nnew)%w(:,:,1)

    PRINT *, 'Serializing w_concorr_c=p_nh%diag%w_concorr_c(:,:,1)'

    !$ser data w_concorr_c=p_nh%diag%w_concorr_c(:,:,1)
...
!DIR$ IVDEP
    ...
    !$ser data z_dexner_dz_c_2=z_dexner_dz_c(:,:,1,2)
            !$ACC PARALLEL IF(i_am_accel_node) DEFAULT(PRESENT) ASYNC(1)
            !$ACC LOOP GANG VECTOR TILE(32, 4)
...
!$OMP PARALLEL PRIVATE (rl_start,rl_end,i_startblk,i_endblk)
    ...
    !$ser data z_rho_e=z_rho_e(:,:,1)

    PRINT *, 'Serializing z_theta_v_e=z_theta_v_e(:,:,1)'

    !$ser data z_theta_v_e=z_theta_v_e(:,:,1)
            !$ACC PARALLEL IF(i_am_accel_node) DEFAULT(PRESENT) ASYNC(1)
            !$ACC LOOP GANG VECTOR
            DO jk = 1, nlev
              DO jc = i_startidx, i_endidx
...
!$OMP DO PRIVATE(jb,jk,je,i_startidx,i_endidx) ICON_OMP_DEFAULT_SCHEDULE
    ...
    !$ser data z_dwdz_dd=z_dwdz_dd(:,:,1)

    PRINT *, 'Serializing z_graddiv_vn=z_graddiv_vn(:,:,1)'

    !$ser data z_graddiv_vn=z_graddiv_vn(:,:,1)
          !$ACC PARALLEL IF(i_am_accel_node) DEFAULT(PRESENT) ASYNC(1)
          !$ACC LOOP GANG VECTOR COLLAPSE(2)
#ifdef __LOOP_EXCHANGE
          DO je = i_startidx, i_endidx
...
!$OMP DO PRIVATE(jb,i_startidx,i_endidx,jk,je,z_theta1,z_theta2,ikp1,ikp2) ICON_OMP_DEFAULT_SCHEDULE
    ...
    !$ser data z_gradh_exner=z_gradh_exner(:,:,1)
          !$ACC PARALLEL IF(i_am_accel_node) DEFAULT(PRESENT) ASYNC(1)
          !$ACC LOOP GANG VECTOR COLLAPSE(2)
#ifdef __LOOP_EXCHANGE
          DO je = i_startidx, i_endidx
...
!$NEC outerloop_unroll(8)
    ...
    !$ser data z_hydro_corr=z_hydro_corr(:,:,1)
...
!$OMP           ) ICON_OMP_DEFAULT_SCHEDULE
    ...
    !$ser data vn_nnew=p_nh%prog(nnew)%vn(:,:,1)
        !$ACC PARALLEL IF(i_am_accel_node) DEFAULT(PRESENT) ASYNC(1)
...
!$ACC END PARALLEL
    ...
    !$ser data z_graddiv2_vn=z_graddiv2_vn(:,:)
!$ACC PARALLEL IF( i_am_accel_node )  DEFAULT(PRESENT) ASYNC(1)
          !$ACC LOOP GANG(STATIC: 1) VECTOR TILE(32, 4)
#ifdef __LOOP_EXCHANGE
          DO je = i_startidx, i_endidx
...
!$ACC END PARALLEL
    ...
    !$ser data vn=p_nh%prog(nnew)%vn(:,:,1)
!$ACC PARALLEL IF(i_am_accel_node)  DEFAULT(PRESENT) ASYNC(1)
...
!$OMP DO PRIVATE(jb,i_startidx,i_endidx,jk,je) ICON_OMP_DEFAULT_SCHEDULE
    ...
    !$ser data vn_new=p_nh%prog(nnew)%vn(:,:,1)
          !$ACC PARALLEL IF(i_am_accel_node) DEFAULT(PRESENT) ASYNC(1)
          !$ACC LOOP GANG VECTOR COLLAPSE(2)
          DO jk = 1, nlev
!DIR$ IVDEP
...
!$ACC END PARALLEL
    ...
    !$ser data mass_fl_e=p_nh%diag%mass_fl_e(:,:,1)

    PRINT *, 'Serializing z_theta_v_fl_e=z_theta_v_fl_e(:,:,1)'

    !$ser data z_theta_v_fl_e=z_theta_v_fl_e(:,:,1)
!$ACC PARALLEL IF(i_am_accel_node) DEFAULT(PRESENT) ASYNC(1)
          !$ACC LOOP GANG(STATIC: 1) VECTOR TILE(32, 4)
          DO jk = 1,nlev
!DIR$ IVDEP
...
!DIR$ IVDEP
    ...
    !$ser data vn_traj=prep_adv%vn_traj(:,:,1)

    PRINT *, 'Serializing mass_flx_me=prep_adv%mass_flx_me(:,:,1)'

    !$ser data mass_flx_me=prep_adv%mass_flx_me(:,:,1)
!$ACC PARALLEL IF(i_am_accel_node) DEFAULT(PRESENT) ASYNC(1)
              !$ACC LOOP GANG(STATIC: 1) VECTOR TILE(32, 4)
              DO jk = 1, nlev
!$NEC ivdep
...
!$ACC END PARALLEL
    ...
    !$ser data z_w_concorr_me=z_w_concorr_me(:,:,1)
          !$ACC PARALLEL IF(i_am_accel_node) DEFAULT(PRESENT) ASYNC(1)
          !$ACC LOOP GANG VECTOR COLLAPSE(2)
          DO jk = nflatlev(jg), nlev
!DIR$ IVDEP
    ...
    !$ser data vn_ie=p_nh%diag%vn_ie(:,:,1)

    PRINT *, 'Serializing z_vt_ie=z_vt_ie(:,:,1)'

    !$ser data z_vt_ie=z_vt_ie(:,:,1)

    PRINT *, 'Serializing z_kin_hor_e=z_kin_hor_e(:,:,1)'

    !$ser data z_kin_hor_e=z_kin_hor_e(:,:,1)
          !$ACC PARALLEL IF(i_am_accel_node) DEFAULT(PRESENT) ASYNC(1)
          !$ACC LOOP GANG VECTOR COLLAPSE(2)
...
!$OMP   z_a,z_b,z_c,z_g,z_q,z_alpha,z_beta,z_gamma,ic,z_flxdiv_mass,z_flxdiv_theta  ) ICON_OMP_DEFAULT_SCHEDULE
    ...
    !$ser data z_flxdiv_mass=z_flxdiv_mass(:,:)

    PRINT *, 'Serializing z_flxdiv_theta=z_flxdiv_theta(:,:)'

    !$ser data z_flxdiv_theta=z_flxdiv_theta(:,:)
          !$ACC PARALLEL IF(i_am_accel_node) DEFAULT(PRESENT) ASYNC(1)
          !$ACC LOOP GANG VECTOR COLLAPSE(2)
#ifdef __LOOP_EXCHANGE
          DO jc = i_startidx, i_endidx
...
!DIR$ IVDEP
    ...
    !$ser data z_th_ddz_exner_c=z_th_ddz_exner_c(:,:,jb)

    PRINT *, 'Serializing z_contr_w_fl_l=z_contr_w_fl_l(:,:)'

    !$ser data z_contr_w_fl_l=z_contr_w_fl_l(:,:)

    PRINT *, 'Serializing rho_ic=p_nh%diag%rho_ic(:,:,jb)'

    !$ser data rho_ic=p_nh%diag%rho_ic(:,:,jb)

    PRINT *, 'Serializing w_concorr_c=p_nh%diag%w_concorr_c(:,:,jb)'

    !$ser data w_concorr_c=p_nh%diag%w_concorr_c(:,:,jb)
...
!DIR$ IVDEP
    ...
    !$ser data w_nnew=p_nh%prog(nnew)%w(:,:,jb)
...
!DIR$ IVDEP
    ...
    !$ser data z_rho_expl=z_rho_expl(:,:)

    PRINT *, 'Serializing z_exner_expl=z_exner_expl(:,:)'

    !$ser data z_exner_expl=z_exner_expl(:,:)
    ...
    !$ser data exner_pr=p_nh%diag%exner_pr(:,:,jb)
...
!DIR$ IVDEP
    ...
    !$ser data rho_new=p_nh%prog(nnew)%rho(:,:,1)

    PRINT *, 'Serializing exner_new=p_nh%prog(nnew)%exner(:,:,1)'

    !$ser data exner_new=p_nh%prog(nnew)%exner(:,:,1)

    PRINT *, 'Serializing theta_v_new=p_nh%prog(nnew)%theta_v(:,:,1)'

    !$ser data theta_v_new=p_nh%prog(nnew)%theta_v(:,:,1)
        !$ACC PARALLEL IF(i_am_accel_node) DEFAULT(PRESENT) ASYNC(1)
...
!DIR$ IVDEP
    ...
    !$ser savepoint mo_solve_nonhydro_stencil_56_63_8f4253f6-09a1-46f7-a72e-95b3ff4ebb06_start istep=istep mo_solve_nonhydro_ctr=mo_solve_nonhydro_ctr
...
!DIR$ IVDEP
    ...
    !$ser savepoint mo_solve_nonhydro_stencil_56_63_8f4253f6-09a1-46f7-a72e-95b3ff4ebb06_end istep=istep mo_solve_nonhydro_ctr=mo_solve_nonhydro_ctr
    ...
    !$ser data mass_flx_ic=prep_adv%mass_flx_ic(:,:,1)
            !$ACC PARALLEL IF(i_am_accel_node) DEFAULT(PRESENT) ASYNC(1)
            !$ACC LOOP GANG VECTOR COLLAPSE(2)
            DO jk = 1, nlev
!$NEC ivdep
...
!$NEC ivdep
    ...
    !$ser data exner_dyn_incr=p_nh%diag%exner_dyn_incr(:,:,1)
          !$ACC PARALLEL IF(i_am_accel_node) DEFAULT(PRESENT) ASYNC(1)
          !$ACC LOOP GANG VECTOR COLLAPSE(2)
...
!DIR$ IVDEP
    ...
    !$ser data exner=p_nh%prog(nnew)%exner(:,:,1)
...
!DIR$ IVDEP
    ...
    !$ser data w_new=p_nh%prog(nnew)%w(:,:,1)
            !$ACC PARALLEL IF(i_am_accel_node) DEFAULT(PRESENT) ASYNC(1)
            !$ACC LOOP GANG VECTOR COLLAPSE(2)
            DO jk = 1, nlev
#if __INTEL_COMPILER != 1400 || __INTEL_COMPILER_UPDATE != 3
!DIR$ IVDEP
    ...
    !$ser savepoint mo_solve_nonhydro_stencil_56_63_3f33cb44-1850-43af-a392-c804c0530f9f_start istep=istep mo_solve_nonhydro_ctr=mo_solve_nonhydro_ctr
...
!DIR$ IVDEP
    ...
    !$ser savepoint mo_solve_nonhydro_stencil_56_63_3f33cb44-1850-43af-a392-c804c0530f9f_end istep=istep mo_solve_nonhydro_ctr=mo_solve_nonhydro_ctr
...
!$OMP PARALLEL PRIVATE(rl_start,rl_end,i_startblk,i_endblk)
    ...
    !$ser data theta_v=p_nh%prog(nnew)%theta_v(:,:,1)
