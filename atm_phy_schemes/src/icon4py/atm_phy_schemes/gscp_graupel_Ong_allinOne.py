...
def _graupel_scan(
   ...
   rhoqsV = rhoqs * terminal_velocity if (llqs) else 0.0
   ...
   rhoqrV = rhoqr * terminal_velocity if (llqr) else 0.0
   ...
   if ( (Vnew_r == 0.0) & llqr ):
      # because we are at the model top, simply multiply by a factor of (0.5)^(V_intg_exp)
      if (llqr): Vnew_r = terminal_velocity * graupel_const.GrConst_vzxp_ln1o2
   ...
   rhoqgV = rhoqg * terminal_velocity if (llqg) else 0.0
   ...
   rhoqiV = rhoqi * terminal_velocity if (llqi) else 0.0
