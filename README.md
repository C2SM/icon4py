...
Some test depend on serialized data generated by a full model run. Those test are marked with
`pytest.mark.datatest` and can be skipped by specifying
```bash
...
Test for parallel code is collected in a package's `tests/mpi_tests` folder. 
All parallel tests are marked with `@pytest.mark.mpi` and are skipped if 
the `--with-mpi` is not passed option is not passed to `pytest`
In order to run them, you need a MPI installation on your system: 
On Debian-Linux do
```bash
