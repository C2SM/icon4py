...
from icon4pytools.liskov.codegen.integration.interface import IntegrationCodeInterface, StartDeleteData, EndDeleteData, UnusedDirective
...
class TransformFuseStencils(Step):
    ...
    def __call__(self, data: Any=None) -> ts.ParsedDict:
        ...
        """
        if self.fused:
          logger.info(f"Transforming single stencils.")

          removeStartStencil = []
          removeEndStencil = []

          for startFused, endFused in zip(self.parsed.StartFusedStencil, self.parsed.EndFusedStencil, strict=True):
            for startSingle, endSingle in zip(self.parsed.StartStencil, self.parsed.EndStencil, strict=True):
              # print('try: ', startSingle.name)
              if( startFused.startln < startSingle.startln and startSingle.startln < endFused.startln and 
                  startFused.startln < endSingle.startln and endSingle.startln < endFused.startln):
                  try:
          ...
          self.parsed.StartStencil = [x for x in self.parsed.StartStencil if x not in removeStartStencil]
          self.parsed.EndStencil = [x for x in self.parsed.EndStencil if x not in removeEndStencil]

        else:
          logger.info(f"Removing fused stencils.")
