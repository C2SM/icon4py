...
def _unpack_numpy(ffi: cffi.FFI, ptr: cffi.FFI.CData, *sizes: int) -> np.typing.NDArray:
    ...
    return np.frombuffer(ffi.buffer(ptr, length * dtype.itemsize), dtype=dtype).reshape(  # type: ignore[call-overload]  # ffi.buffer return type not in np.frombuffer stubs
        sizes, order="F"
    )
