...
def shrink_to_dimension(
    sizes: dict[gtx.Dimension, int], raw_tables: dict[gtx.FieldOffset, NDArray]
) -> dict[gtx.FieldOffset, NDArray]:
    # TODO log?
    return {k: v[: sizes[k.target[0]]] for k, v in raw_tables.items()}


def add_origin(xp, table: NDArray) -> NDArray:
...
def construct_icon_grid(
    ...
    raw_tables = {
        ...
        dims.C2E2C: c2e2c,
        dims.V2E: v2e,
        dims.E2V: e2v[:, 0:2],  # slicing required for e2v as input data is actually e2c2v
        dims.E2C2V: e2c2v,
        dims.V2C: v2c,
        # dims.C2E2C2E: ,  # TODO
        # dims.C2CEC:  , # TODO
    ...
    raw_tables = {k: adjust_fortran_indices(v) for k, v in raw_tables.items()}

    nproma = get_nproma(raw_tables.values())

    # tables with origin
    raw_tables.update(
        {
            dims.C2E2CO: add_origin(xp, c2e2c),
            dims.E2C2EO: add_origin(xp, e2c2e),
        }
    )
    ...
    raw_tables.update(
        {
            dims.E2ECV: base.construct_1d_sparse_table((nproma, e2c2v.shape[1]), array_ns=xp),
            dims.C2CE: base.construct_1d_sparse_table((nproma, c2e.shape[1]), array_ns=xp),
            dims.E2EC: base.construct_1d_sparse_table((nproma, e2c.shape[1]), array_ns=xp),
        }
    )

    sizes = {dims.EdgeDim: num_edges, dims.VertexDim: num_vertices, dims.CellDim: num_cells}
    tables = shrink_to_dimension(sizes=sizes, raw_tables=raw_tables)
    ...
    start_end_indices = {
        dims.CellDim: (cells_start_index, cells_end_index),
        dims.VertexDim: (vertex_start_index, vertex_end_index),
        dims.EdgeDim: (edge_start_index, edge_end_index),
    }
