...
def lintersect(
    ...
    intersect1 = ccw(line1_p1_lon, line1_p1_lat, line1_p2_lon, line1_p2_lat, line2_p1_lon, line2_p1_lat) * ccw(line1_p1_lon, line1_p1_lat, line1_p2_lon, line1_p2_lat, line2_p2_lon, line2_p2_lat)
    intersect2 = ccw(line2_p1_lon, line2_p1_lat, line2_p2_lon, line2_p2_lat, line1_p1_lon, line1_p1_lat) * ccw(line2_p1_lon, line2_p1_lat, line2_p2_lon, line2_p2_lat, line1_p2_lon, line1_p2_lat)
...
def _divide_flux_area_list_stencil_01(
...
) -> tuple[Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float], Field[[EdgeDim, KDim], float]]:
    ...
    tri_line1_p2_lon = where(lvn_pos, broadcast(ptr_v3_lon(E2EC[0]), (EdgeDim, KDim)), broadcast(ptr_v3_lon(E2EC[1]), (EdgeDim, KDim)))
    tri_line1_p2_lat = where(lvn_pos, broadcast(ptr_v3_lat(E2EC[0]), (EdgeDim, KDim)), broadcast(ptr_v3_lat(E2EC[1]), (EdgeDim, KDim)))

    # get triangle edge 2 (A2V3)
    tri_line2_p1_lon = arrival_pts_2_x
    tri_line2_p1_lat = arrival_pts_2_y
    tri_line2_p2_lon = where(lvn_pos, broadcast(ptr_v3_lon(E2EC[0]), (EdgeDim, KDim)), broadcast(ptr_v3_lon(E2EC[1]), (EdgeDim, KDim)))
    tri_line2_p2_lat = where(lvn_pos, broadcast(ptr_v3_lat(E2EC[0]), (EdgeDim, KDim)), broadcast(ptr_v3_lat(E2EC[1]), (EdgeDim, KDim)))

    # Create first mask does departure-line segment intersects with A1V3
    lintersect_line1 = lintersect(fl_line_p1_lon, fl_line_p1_lat, fl_line_p2_lon, fl_line_p2_lat, tri_line1_p1_lon, tri_line1_p1_lat, tri_line1_p2_lon, tri_line1_p2_lat)
    # Create first mask does departure-line segment intersects with A2V3
    lintersect_line2 = lintersect(fl_line_p1_lon, fl_line_p1_lat, fl_line_p2_lon, fl_line_p2_lat, tri_line2_p1_lon, tri_line2_p1_lat, tri_line2_p2_lon, tri_line2_p2_lat)

    lvn_sys_pos = where((p_vn * broadcast(tangent_orientation_dsl, (EdgeDim, KDim))) >= 0.0, True, False)
    famask_bool = where(famask_int == int32(1), True, False)
    #------------------------------------------------- Case 1
    mask_case1 = lintersect_line1 & lintersect_line2 & famask_bool
    ps1_x, ps1_y = line_intersect(fl_line_p1_lon, fl_line_p1_lat, fl_line_p2_lon, fl_line_p2_lat, tri_line1_p1_lon, tri_line1_p1_lat, tri_line1_p2_lon, tri_line1_p2_lat)
    ps2_x, ps2_y = line_intersect(fl_line_p1_lon, fl_line_p1_lat, fl_line_p2_lon, fl_line_p2_lat, tri_line2_p1_lon, tri_line2_p1_lat, tri_line2_p2_lon, tri_line2_p2_lat)
    ...
    dreg_patch0_2_x = where(mask_case1, where(lvn_sys_pos, arrival_pts_2_x, ps1_x), dreg_patch0_2_x)
    dreg_patch0_2_y = where(mask_case1, where(lvn_sys_pos, arrival_pts_2_y, ps1_y), dreg_patch0_2_y)
    dreg_patch0_3_x = where(mask_case1, ps2_x, dreg_patch0_3_x)
    dreg_patch0_3_y = where(mask_case1, ps2_y, dreg_patch0_3_y)
    dreg_patch0_4_x = where(mask_case1, where(lvn_sys_pos, ps1_x, arrival_pts_2_x), dreg_patch0_4_x)
    dreg_patch0_4_y = where(mask_case1, where(lvn_sys_pos, ps1_y, arrival_pts_2_y), dreg_patch0_4_y)
    # Case 1 - patch 1
    ...
    dreg_patch0_2_x = where(mask_case2a, where(lvn_sys_pos, arrival_pts_2_x, ps1_x), dreg_patch0_2_x)
    dreg_patch0_2_y = where(mask_case2a, where(lvn_sys_pos, arrival_pts_2_y, ps1_y), dreg_patch0_2_y)
    ...
    dreg_patch0_4_x = where(mask_case2a, where(lvn_sys_pos, ps1_x, arrival_pts_2_x), dreg_patch0_4_x)
    dreg_patch0_4_y = where(mask_case2a, where(lvn_sys_pos, ps1_y, arrival_pts_2_y), dreg_patch0_4_y)
    # Case 2a - patch 1
    ...
    dreg_patch1_2_x = where(mask_case2a, where(lvn_sys_pos, ps1_x, depart_pts_1_x), dreg_patch1_2_x)
    dreg_patch1_2_y = where(mask_case2a, where(lvn_sys_pos, ps1_y, depart_pts_1_y), dreg_patch1_2_y)
    dreg_patch1_3_x = where(mask_case2a, where(lvn_sys_pos, depart_pts_1_x, ps1_x), dreg_patch1_3_x)
    dreg_patch1_3_y = where(mask_case2a, where(lvn_sys_pos, depart_pts_1_y, ps1_y), dreg_patch1_3_y)
    # Case 2a - patch 2
    ...
    dreg_patch0_2_x = where(mask_case2b, where(lvn_sys_pos, arrival_pts_2_x, depart_pts_1_x), dreg_patch0_2_x)
    dreg_patch0_2_y = where(mask_case2b, where(lvn_sys_pos, arrival_pts_2_y, depart_pts_1_y), dreg_patch0_2_y)
    dreg_patch0_3_x = where(mask_case2b, ps2_x, dreg_patch0_3_x)
    dreg_patch0_3_y = where(mask_case2b, ps2_y, dreg_patch0_3_y)
    dreg_patch0_4_x = where(mask_case2b, where(lvn_sys_pos, depart_pts_1_x, arrival_pts_2_x), dreg_patch0_4_x)
    dreg_patch0_4_y = where(mask_case2b, where(lvn_sys_pos, depart_pts_1_y, arrival_pts_2_y), dreg_patch0_4_y)
    # Case 2b - patch 1
    ...
    dreg_patch2_2_x = where(mask_case2b, where(lvn_sys_pos, depart_pts_2_x, ps2_x), dreg_patch2_2_x)
    dreg_patch2_2_y = where(mask_case2b, where(lvn_sys_pos, depart_pts_2_y, ps2_y), dreg_patch2_2_y)
    dreg_patch2_3_x = where(mask_case2b, where(lvn_sys_pos, ps2_x, depart_pts_2_x), dreg_patch2_3_x)
    dreg_patch2_3_y = where(mask_case2b, where(lvn_sys_pos, ps2_y, depart_pts_2_y), dreg_patch2_3_y)
    ...
    lintersect_e2_line1 = lintersect(fl_e2_p1_lon, fl_e2_p1_lat, fl_e2_p2_lon, fl_e2_p2_lat, tri_line1_p1_lon, tri_line1_p1_lat, tri_line1_p2_lon, tri_line1_p2_lat)
    mask_case3a = lintersect_e2_line1 & famask_bool
    pi1_x, pi1_y = line_intersect(fl_e2_p1_lon, fl_e2_p1_lat, fl_e2_p2_lon, fl_e2_p2_lat, tri_line1_p1_lon, tri_line1_p1_lat, tri_line1_p2_lon, tri_line1_p2_lat)
    # Case 3a - patch 0
    ...
    dreg_patch0_2_x = where(mask_case3a, where(lvn_sys_pos, arrival_pts_2_x, depart_pts_1_x), dreg_patch0_2_x)
    dreg_patch0_2_y = where(mask_case3a, where(lvn_sys_pos, arrival_pts_2_y, depart_pts_1_y), dreg_patch0_2_y)
    dreg_patch0_3_x = where(mask_case3a, ps2_x, dreg_patch0_3_x)
    dreg_patch0_3_y = where(mask_case3a, ps2_y, dreg_patch0_3_y)
    dreg_patch0_4_x = where(mask_case3a, where(lvn_sys_pos, depart_pts_1_x, arrival_pts_2_x), dreg_patch0_4_x)
    dreg_patch0_4_y = where(mask_case3a, where(lvn_sys_pos, depart_pts_1_y, arrival_pts_2_y), dreg_patch0_4_y)
    # Case 3a - patch 1
    ...
    dreg_patch1_2_x = where(mask_case3a, where(lvn_sys_pos, pi1_x, depart_pts_2_x), dreg_patch1_2_x)
    ...
    dreg_patch1_4_y = where(mask_case3a, where(lvn_sys_pos, depart_pts_1_y, pi1_y), dreg_patch1_4_y)
    # Case 3a - patch 2
    ...
    lintersect_e1_line2 = lintersect(fl_e1_p1_lon, fl_e1_p1_lat, fl_e1_p2_lon, fl_e1_p2_lat, tri_line2_p1_lon, tri_line2_p1_lat, tri_line2_p2_lon, tri_line2_p2_lat)
    mask_case3b = lintersect_e1_line2 & famask_bool
    pi2_x, pi2_y = line_intersect(fl_e1_p1_lon, fl_e1_p1_lat, fl_e1_p2_lon, fl_e1_p2_lat, tri_line2_p1_lon, tri_line2_p1_lat, tri_line2_p2_lon, tri_line2_p2_lat)
    # Case 3b - patch 0
    ...
    dreg_patch0_2_x = where(mask_case3b, where(lvn_sys_pos, arrival_pts_2_x, pi2_x), dreg_patch0_2_x)
    dreg_patch0_2_y = where(mask_case3b, where(lvn_sys_pos, arrival_pts_2_y, pi2_y), dreg_patch0_2_y)
    ...
    dreg_patch2_2_x = where(mask_case3b, where(lvn_sys_pos, depart_pts_2_x, pi2_x), dreg_patch2_2_x)
    dreg_patch2_2_y = where(mask_case3b, where(lvn_sys_pos, depart_pts_2_y, pi2_y), dreg_patch2_2_y)
    ...
    dreg_patch2_4_x = where(mask_case3b, where(lvn_sys_pos, pi2_x, depart_pts_2_x), dreg_patch2_4_x)
    dreg_patch2_4_y = where(mask_case3b, where(lvn_sys_pos, pi2_y, depart_pts_2_y), dreg_patch2_4_y)
