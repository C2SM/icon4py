...
from icon4py.atm_dyn_iconam.mo_solve_nonhydro_stencil_15 import (
    mo_solve_nonhydro_stencil_15_z_rho_e,
    mo_solve_nonhydro_stencil_15_z_theta_v_e,
)
...
def test_mo_solve_nonhydro_stencil_15_z_rho_e():
    mesh = SimpleMesh()

    z_rho_e = zero_field(mesh, EdgeDim, KDim)

    ref = mo_solve_nonhydro_stencil_15_z_rho_e_numpy(np.asarray(z_rho_e))
    mo_solve_nonhydro_stencil_15_z_rho_e(
        z_rho_e,
        offset_provider={},
    )
    assert np.allclose(z_rho_e, ref)
...
def test_mo_solve_nonhydro_stencil_15_z_theta_v_e():
    ...
    ref = mo_solve_nonhydro_stencil_15_z_theta_v_e_numpy(np.asarray(z_theta_v_e))
    mo_solve_nonhydro_stencil_15_z_theta_v_e(
        z_theta_v_e,
        offset_provider={},
    )
