...
from icon4py.common.dimension import CellDim, EdgeDim, C2EDim, KDim
...
class TestCalculateDiagnosticQuantitiesForTurbulence(StencilTest):
    ...
    @staticmethod
    def reference(
        mesh, wgtfac_c: np.array, kh_smag_ec: np.array, vn: np.array, e_bln_c_s: np.array,
        geofac_div: np.array, diff_multfac_smag: np.array, div_ic, hdef_ic
    ...
    def input_data(self, mesh):
        ...
        return dict(
            wgtfac_c=wgtfac_c, vn=vn, geofac_div=geofac_div, kh_smag_ec=kh_smag_ec, e_bln_c_s=e_bln_c_s,
            diff_multfac_smag=diff_multfac_smag, div_ic=div_ic, hdef_ic=hdef_ic
        )
