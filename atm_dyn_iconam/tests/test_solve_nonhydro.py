...
def test_nonhydro_predictor_step(
    ...
    assert dallclose(
        ...
        np.asarray(solve_nonhydro.z_gradh_exner)[5387:31558, :],
        atol=1e-20,
    )
    # stencil 21
    assert dallclose(
    ...
    # stencil 30
    assert dallclose(np.asarray(sp_exit.vt()), np.asarray(diagnostic_state_nh.vt))
    ...
    # stencil 35,36, 37,38
    assert dallclose(np.asarray(sp_exit.z_vt_ie()), np.asarray(solve_nonhydro.z_vt_ie))
    # stencil 35,36
    assert dallclose(
