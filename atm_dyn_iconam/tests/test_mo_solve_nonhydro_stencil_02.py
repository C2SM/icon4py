...
from icon4py.atm_dyn_iconam.mo_solve_nonhydro_stencil_02 import (
    mo_solve_nonhydro_stencil_02_exner_pr,
    mo_solve_nonhydro_stencil_02_z_exner_ex_pr,
)
...
def test_mo_solve_nonhydro_stencil_02_z_exner_ex_pr():
    mesh = SimpleMesh()
    ...
    ref = mo_solve_nonhydro_stencil_02_z_exner_ex_pr_numpy(
        np.asarray(exner),
...
def test_mo_solve_nonhydro_stencil_02_exner_pr():
    mesh = SimpleMesh()
    ...
    ref = mo_solve_nonhydro_stencil_02_exner_pr_numpy(
        np.asarray(exner), np.asarray(exner_ref_mc)
    )
    mo_solve_nonhydro_stencil_02_exner_pr(
        exner, exner_ref_mc, exner_pr, offset_provider={}
    )
    assert np.allclose(exner_pr, ref)
