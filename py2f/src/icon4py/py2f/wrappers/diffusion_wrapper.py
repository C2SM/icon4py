...
from icon4py.common.dimension import (
    ...
    KDim,
    V2EDim,
    VertexDim, CECDim, E2C2VDim,
)
from icon4py.diffusion.diffusion import (
    Diffusion,
    DiffusionConfig,
    DiffusionParams, DiffusionType,
)
...
from icon4py.grid.vertical import VerticalModelParams, VerticalGridSize
...
def diffusion_init(
    ...
    zd_intcoef: Field[[CellDim, C2E2CDim, KDim], float], # special DSL field:   zd_intcoef_dsl in mo_vertical_grid.f90
    zd_geofac: Field[[CellDim, C2E2CODim], float],  # same as geofac_n2s
    zd_diffcoef: Field[[CellDim, KDim], float],  # special DSL field mask instead of list: zd_diffcoef_dsl in mo_vertical_grid.f90
    ...
    zd_indlist: Field[[CellDim, C2E2CDim, KDim], int32], # unused, index list for  steep points, DSL uses mask_hdiff instead (see below)
    zd_blklist: Field[[CellDim, C2E2CDim, KDim], int32],  # unused,
    zd_vertidx: Field[[CellDim, C2E2CDim, KDim], int32],  # DSL uses offsets instead of absolute indices zd_vertoffset_dsl in mo_vertical_grid.f90
    zd_listdim: int,
    ...
    c2e2c0 = np.column_stack(((np.asarray(cells_neighbor_idx)), (np.asarray(range(
        np.asarray(cells_neighbor_idx).shape[0])))))
    ...
    grid = IconGrid().with_config(mesh_config).with_connectivities(connectivities).with_start_end_indices(VertexDim, vertex_starts, vertex_ends).with_start_end_indices(EdgeDim, edge_starts, edge_ends).with_start_end_indices(CellDim, cell_starts, cell_ends)
