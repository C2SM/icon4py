...
# vname='w'
# itime = 10
# for iheight in range(0,35):
#     data_ft = uxds_ft[vname].values[itime,iheight,:]
#     data_py = uxds_py[vname].values[itime,iheight,:]
#     data_dl = data_ft - data_py
#     print('iheight {:} mean delta = {:}  | min {:}, max {:}'.format(iheight, np.mean(data_dl), data_dl.min(), data_dl.max()))
...
for iheight in range(35):
    w_ft = uxds_ft['w'].values[:,iheight,313]
    w_py = uxds_py['w'].values[:,iheight,313]
    itime=0
...
for iheight in range(35):
    w_ft = uxds_ft['rho'].values[:,iheight,313]
    w_py = uxds_py['rho'].values[:,iheight,313]
    itime=1
    print('{:.18e}, {:.18e}, {:.18e}'.format(w_ft[itime], w_py[itime], w_ft[itime]-w_py[itime]))
    plt.plot(w_ft, '-')
    plt.plot(w_py, '--')
