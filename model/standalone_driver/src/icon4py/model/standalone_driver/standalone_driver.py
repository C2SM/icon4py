...
NVTX_MARKERS_ENABLED = os.getenv("ICON4PY_NVTX_MARKERS_ENABLED", "0") == "1"
...
class Icon4pyDriver:
    ...
    def time_integration(
        ...
        log.warning(f"Granule timers enabled: {self.config.driver.granule_timers}\nNVTX_MARKERS_ENABLED: {NVTX_MARKERS_ENABLED}")
        if NVTX_MARKERS_ENABLED:
        ...
        log.warning(
            f"\n"
            f"[FINAL] simulation date : {self.model_time_variables.simulation_current_datetime}, Elapsed wall clock time: {(datetime.datetime.now() - wall_clock_starting_time).total_seconds()}"
            f"\n"
        )
        ...
        if self.granules.diffusion is not None:
            ...
            if self.granules.diffusion.config.apply_to_horizontal_wind:
                ...
                with timer_diffusion:
                    ...
                    if self.config.driver.granule_timers:
                        device_utils.sync(self.backend)
                if NVTX_MARKERS_ENABLED:
                    device_utils.sync(self.backend)
                    nvtx.pop_range()
...
def initialize_driver(
    *,
    ...
    config = dataclasses.replace(
        config, driver=dataclasses.replace(config.driver, output_path=output_path, granule_timers=granule_timers)
    )
