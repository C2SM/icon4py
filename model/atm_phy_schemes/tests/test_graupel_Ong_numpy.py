...
def test_graupel_Ong_serialized_data():
    ...
    debug = False
    data_output = True # do you want to output formatted data for further analysis?
    ...
    for item_no, item in enumerate(ser_const_name):
        ...
        #if (item == "ser_graupel_istart" or item == "ser_graupel_iend"):
        #    print(0, item, serializer.read(item, entrance_savePoint)[0])
        for i in range(Nblocks - 1):
            ...
            #if (item == "ser_graupel_istart" or item == "ser_graupel_iend"):
            #    print(i, item, serializer.read(item, entrance_savePoint)[0])
    ...
    for item in ser_field_name:
        if (item != "ser_graupel_qnc"):
            predict_data[item] = np.zeros((cell_size,k_size), dtype=float64)
        else:
            predict_data[item] = np.zeros(cell_size, dtype=float64)
    for item in ser_tend_name:
        pretend_data[item] = np.zeros((cell_size,k_size), dtype=float64)
    for item in ser_redundant_name:
        if (item != "ser_graupel_klev"):
            redundant_data[item] = np.zeros((cell_size, k_size), dtype=float64)
        else:
            redundant_data[item] = np.zeros((cell_size, k_size), dtype=int32)
    for item in velocity_field_name:
        velocity_data[item] = np.zeros((cell_size, k_size), dtype=float64)

    print("max and min dz: ", ser_data["ser_graupel_dz"].min(), ser_data["ser_graupel_dz"].max())
    print("max and min pres: ", ser_data["ser_graupel_pres"].min(), ser_data["ser_graupel_pres"].max())

    # run graupel

    for i in range(cell_size):
        for k in range(0,const_data["ser_graupel_kend"]+1):

            if (k == 0):
                (
                    predict_data["ser_graupel_temperature"][i, k],
                    predict_data["ser_graupel_qv"][i, k],
                    predict_data["ser_graupel_qc"][i, k],
                    predict_data["ser_graupel_qi"][i, k],
                    predict_data["ser_graupel_qr"][i, k],
                    predict_data["ser_graupel_qs"][i, k],
                    predict_data["ser_graupel_qg"][i, k],
                    velocity_data["rhoqrV_old_kup"][i, k],
                    velocity_data["rhoqsV_old_kup"][i, k],
                    velocity_data["rhoqgV_old_kup"][i, k],
                    velocity_data["rhoqiV_old_kup"][i, k],
                    velocity_data["Vnew_r"][i, k],
                    velocity_data["Vnew_s"][i, k],
                    velocity_data["Vnew_g"][i, k],
                    velocity_data["Vnew_i"][i, k],
                    redundant_data["ser_graupel_clddist"][i, k],
                    redundant_data["ser_graupel_rho_kup"][i, k],
                    redundant_data["ser_graupel_Crho1o2_kup"][i, k],
                    redundant_data["ser_graupel_Crhofac_qi_kup"][i, k],
                    redundant_data["ser_graupel_Cvz0s_kup"][i, k],
                    redundant_data["ser_graupel_qvsw_kup"][i, k]
                ) = _graupel_scan(
                    const_data["ser_graupel_kstart_moist"],
                    const_data["ser_graupel_kend"],
                    const_data["ser_graupel_dt"],
                    ser_data["ser_graupel_dz"][i, k],
                    ser_data["ser_graupel_temperature"][i, k],
                    ser_data["ser_graupel_pres"][i, k],
                    ser_data["ser_graupel_rho"][i, k],
                    ser_data["ser_graupel_qv"][i, k],
                    ser_data["ser_graupel_qc"][i, k],
                    ser_data["ser_graupel_qi"][i, k],
                    ser_data["ser_graupel_qr"][i, k],
                    ser_data["ser_graupel_qs"][i, k],
                    ser_data["ser_graupel_qg"][i, k],
                    ser_data["ser_graupel_qnc"][i],
                ...
                (
                    ...
                    const_data["ser_graupel_l_cv"],
                    const_data["ser_graupel_ithermo_water"],
                    predict_data["ser_graupel_qv"][i,k-1],
                    predict_data["ser_graupel_qc"][i,k-1],
                    predict_data["ser_graupel_qi"][i,k-1],
                    predict_data["ser_graupel_qr"][i,k-1],
                    predict_data["ser_graupel_qs"][i,k-1],
                    predict_data["ser_graupel_qg"][i,k-1],
                    velocity_data["rhoqrV_old_kup"][i,k-1],
                    velocity_data["rhoqsV_old_kup"][i,k-1],
                    velocity_data["rhoqgV_old_kup"][i,k-1],
                    velocity_data["rhoqiV_old_kup"][i,k-1],
                    velocity_data["Vnew_r"][i,k-1],
                    velocity_data["Vnew_s"][i,k-1],
                    velocity_data["Vnew_g"][i,k-1],
                    velocity_data["Vnew_i"][i,k-1],
                    redundant_data["ser_graupel_clddist"][i,k-1],
                    redundant_data["ser_graupel_rho_kup"][i,k-1],
                    redundant_data["ser_graupel_Crho1o2_kup"][i,k-1],
                    redundant_data["ser_graupel_Crhofac_qi_kup"][i,k-1],
                    redundant_data["ser_graupel_Cvz0s_kup"][i,k-1],
                    redundant_data["ser_graupel_qvsw_kup"][i,k-1],
                    k
                )

        if (const_data["ser_graupel_ldiag_ttend"]):
            predict_data["ser_graupel_ddt_tend_t"] = _graupel_t_tendency(
                const_data["ser_graupel_dt"],
                predict_data["ser_graupel_temperature"],
                ser_data["ser_graupel_temperature"]
            )

    if (const_data["ser_graupel_ldiag_qtend"]):
        (
            predict_data["ser_graupel_ddt_tend_qv"],
            predict_data["ser_graupel_ddt_tend_qc"],
            predict_data["ser_graupel_ddt_tend_qi"],
            predict_data["ser_graupel_ddt_tend_qr"],
            predict_data["ser_graupel_ddt_tend_qs"]
        ) = _graupel_q_tendency(
            const_data["ser_graupel_dt"],
            predict_data["ser_graupel_qv"],
            predict_data["ser_graupel_qc"],
            predict_data["ser_graupel_qi"],
            predict_data["ser_graupel_qr"],
            predict_data["ser_graupel_qs"],
            ser_data["ser_graupel_qv"],
            ser_data["ser_graupel_qc"],
            ser_data["ser_graupel_qi"],
            ser_data["ser_graupel_qr"],
            ser_data["ser_graupel_qs"]
        )
    ...
    for i in range(cell_size):
        for k in range(0,const_data["ser_graupel_kend"]+1):
            ...
            (
                predict_data["ser_graupel_prr_gsp"][i,k],
                predict_data["ser_graupel_prs_gsp"][i,k],
                predict_data["ser_graupel_prg_gsp"][i,k],
                predict_data["ser_graupel_pri_gsp"][i,k],
                predict_data["ser_graupel_qrsflux"][i,k]
            ) = _graupel_flux_scan(
                const_data["ser_graupel_kstart_moist"],
                const_data["ser_graupel_kend"],
                ser_data["ser_graupel_rho"][i,k],
    ...
    if (tendency_serialization):
        for item in ser_tend_name:
            diff_data[item] = np.abs(pretend_data[item] - tend_data[item]) / tend_data[item]
            diff_data[item] = np.where(np.abs(tend_data[item]) <= 1.e-30, pretend_data[item], tend_data[item])

    if ( tendency_serialization ):
        print("Max predict-ref tendency difference:")
        for item in ser_tend_name:
            print(item, ": ", np.abs(pretend_data[item] - tend_data[item]).max(), diff_data[item].max())

    print("Max predict-ref difference:")
    for item in mixT_name:
        print(item, ": ", np.abs(predict_data[item] - ref_data[item]).max(), diff_data[item].max())
    ...
    print("qv: ", np.abs(
        ser_data["ser_graupel_qv"] - ref_data["ser_graupel_qv"] +
        ser_data["ser_graupel_qc"] - ref_data["ser_graupel_qc"] +
        ser_data["ser_graupel_qi"] - ref_data["ser_graupel_qi"] +
        ser_data["ser_graupel_qr"] - ref_data["ser_graupel_qr"] +
        ser_data["ser_graupel_qs"] - ref_data["ser_graupel_qs"] +
        ser_data["ser_graupel_qg"] - ref_data["ser_graupel_qg"]
    ).max())

    print("Max predict-init total difference:")
    print("qv: ", np.abs(
        predict_data["ser_graupel_qv"] - ref_data["ser_graupel_qv"] +
        predict_data["ser_graupel_qc"] - ref_data["ser_graupel_qc"] +
        predict_data["ser_graupel_qi"] - ref_data["ser_graupel_qi"] +
        predict_data["ser_graupel_qr"] - ref_data["ser_graupel_qr"] +
        predict_data["ser_graupel_qs"] - ref_data["ser_graupel_qs"] +
        predict_data["ser_graupel_qg"] - ref_data["ser_graupel_qg"]
    ).max())
        ...
        with open(base_dir+'analysis_dz_rank'+str(rank)+'.dat','w') as f:
            for i in range(cell_size):
                for k in range(k_size):
                    ...
                    f.write(" {0:.20e} ".format(ser_data["ser_graupel_dz"][i,k]))
                    f.write("\n")
        ...
        with open(base_dir+'analysis_predict_rank'+str(rank)+'.dat','w') as f:
            for i in range(cell_size):
                for k in range(k_size):
                    f.write( "{0:7d} {1:7d}".format(i,k))
                    for item in mixT_name:
                        f.write(" {0:.20e} ".format(predict_data[item][i,k]))
                    f.write("\n")
        ...
        with open(base_dir+'analysis_tend_rank' + str(rank) + '.dat', 'w') as f:
            for i in range(cell_size):
                for k in range(k_size):
                    f.write("{0:7d} {1:7d}".format(i, k))
                    for item in ser_tend_name:
                        f.write(" {0:.20e} ".format(pretend_data[item][i, k]))
                    f.write("\n")
        ...
        with open(base_dir+'analysis_redundant_rank' + str(rank) + '.dat', 'w') as f:
            for i in range(cell_size):
                for k in range(k_size):
                    f.write("{0:7d} {1:7d}".format(i, k))
                    for item in ser_redundant_name:
                        f.write(" {0:.20e} ".format(redundant_data[item][i, k]))
                    f.write("\n")
        ...
        with open(base_dir+'analysis_velocity_rank'+str(rank)+'.dat','w') as f:
            for i in range(cell_size):
                for k in range(k_size):
                    f.write("{0:7d} {1:7d}".format(i, k))
                    for item in velocity_field_name:
                        f.write(" {0:.20e} ".format(velocity_data[item][i, k]))
                    f.write("\n")
    ...
    for item in mixT_name:
        ...
        assert(np.allclose(predict_data[item],ref_data[item], rtol=1e-10, atol=1e-16))
