...
def compute_zdiff_gradp_dsl(
    ...
    for k in range(nlev):
        for e in range(nedges): # TODO: boundary here is not nedges, rather p_patch(jg)%edges%start_block(2)
            if z_me[e, k] <= z_ifc[e2c[e, 0], k] and \
                z_me[e, k] >= z_ifc[e2c[e, 0], k+1] and \
                z_me[e, k] <= z_ifc[e2c[e, 1], k] and \
                z_me[e, k] >= z_ifc[e2c[e, 1], k + 1]:
                flat_idx[e] = k
    ...
    for je in range(nedges):
        ...
        for jk in range(flat_idx[je] + 1, nlev):
            for jk1 in range(jk_start, nlev):
                if jk1 == nlev or z_me[je, jk] <= z_ifc[e2c[je, 0], jk1] and z_me[je, jk] >= z_ifc[e2c[je, 0], jk1+1]:
    ...
    for je in range(nedges):
        ...
        for jk in range(flat_idx[je] + 1, nlev):
            for jk1 in range(jk_start, nlev):
                if jk1 == nlev or z_me[je, jk] <= z_ifc[e2c[je, 1], jk1] and z_me[je, jk] >= z_ifc[e2c[je, 1], jk1+1]:
    ...
    for je in range(nedges):
        ...
        for jk in range(flat_idx[je] + 1, nlev):
            if z_me[je, jk] < z_aux2[je]:
                for jk1 in range(jk_start, nlev):
                    if jk1 == nlev or z_aux2[je] <= z_ifc[e2c[je, 0], jk1] and z_aux2[je] >= z_ifc[e2c[je, 0], jk1+1]:
                        zdiff_gradp[je, 0, jk1] = z_aux2[je] - z_mc[e2c[je, 0], jk1]
                        jk_start = jk1
                        break
    ...
    for je in range(nedges):
        ...
        for jk in range(flat_idx[je] + 1, nlev):
            if z_me[je, jk] < z_aux2[je]:
                for jk1 in range(jk_start, nlev):
                    if jk1 == nlev or z_aux2[je] <= z_ifc[e2c[je, 1], jk1] and z_aux2[je] >= z_ifc[e2c[je, 1], jk1+1]:
                        zdiff_gradp[je, 1, jk1] = z_aux2[je] - z_mc[e2c[je, 1], jk1]
                        jk_start = jk1
                        break
