...
def construct_metric_state(icon_grid) -> tracer_advection_states.AdvectionMetricState:
...
def construct_diagnostic_init_state(
    icon_grid, savepoint: sb.AdvectionInitSavepoint, allocator: gtx_typing.Allocator, ntracer: int
    ...
    return tracer_advection_states.AdvectionDiagnosticState(
        ...
        vfl_tracer=data_alloc.zero_field(  # TODO (dastrm): should be KHalfDim
            icon_grid, dims.CellDim, dims.KDim, extend={dims.KDim: 1}, allocator=allocator
        ),  # exit field
    )
...
def construct_diagnostic_exit_state(
    ...
    zero_f = data_alloc.zero_field(icon_grid, dims.CellDim, dims.KDim, allocator=allocator)
    return tracer_advection_states.AdvectionDiagnosticState(
...
def construct_idealized_diagnostic_state(
    icon_grid, allocator: gtx_typing.Allocator
) -> tracer_advection_states.AdvectionDiagnosticState:
    return tracer_advection_states.AdvectionDiagnosticState(
        ...
        hfl_tracer=data_alloc.zero_field(icon_grid, dims.EdgeDim, dims.KDim, allocator=allocator),
        vfl_tracer=data_alloc.zero_field(  # TODO (dastrm): should be KHalfDim
            icon_grid, dims.CellDim, dims.KDim, extend={dims.KDim: 1}, allocator=allocator
        ),
    )
...
def construct_idealized_prep_adv(
    ...
    array_ns = data_alloc.array_namespace(primal_normal_x)
    vn = array_ns.repeat(array_ns.expand_dims(vn, axis=-1), icon_grid.num_levels, axis=1)

    return tracer_advection_states.AdvectionPrepAdvState(
        vn_traj=gtx.as_field((dims.EdgeDim, dims.KDim), vn),
        mass_flx_me=gtx.as_field((dims.EdgeDim, dims.KDim), vn),
        mass_flx_ic=data_alloc.zero_field(  # TODO (dastrm): should be KHalfDim
            icon_grid, dims.CellDim, dims.KDim, extend={dims.KDim: 1}, allocator=allocator
        ),
    )
...
def construct_idealized_tracer(
    ...
    x = nodes[0, :, :] - x_center
    y = nodes[1, :, :] - y_center
    tracer = array_ns.sum(weights * get_idealized_ICs(test_config, x, y, x_range, y_range), axis=0)
    log_dbg(tracer, "tracer")

    tracer = array_ns.repeat(array_ns.expand_dims(tracer, axis=-1), icon_grid.num_levels, axis=1)
...
def get_idealized_ICs(test_config, x, y, x_range, y_range):
    ...
    match test_config.initial_conditions:
        ...
        case InitialConditions.CIRCLE_2D:
            ...
            return array_ns.where(x**2 + y**2 <= r**2, 1.0, 0.0)
        case _:
            raise NotImplementedError(
...
def get_idealized_velocity_field(
    ...
    match test_config.velocity_field:
        case VelocityField.CONSTANT:
            u, v = x_range, y_range
        case VelocityField.VORTEX_2D:
            array_ns = data_alloc.array_namespace(edges_center_x)
            v_scal = 1e0  # measure for deformation, originally 1e0
            u = (
                -v_scal
                * x_range
                * (array_ns.sin(array_ns.pi * edges_center_x / x_range) ** 2)
                * array_ns.cos(array_ns.pi * edges_center_y / y_range)
                * array_ns.sin(array_ns.pi * edges_center_y / y_range)
                * array_ns.cos(array_ns.pi * time / time_end)
            )
            v = (
                v_scal
                * y_range
                * array_ns.cos(array_ns.pi * edges_center_x / x_range)
                * array_ns.sin(array_ns.pi * edges_center_x / x_range)
                * (array_ns.sin(array_ns.pi * edges_center_y / y_range) ** 2)
                * array_ns.cos(array_ns.pi * time / time_end)
            )
        case VelocityField.INCREASING_2D:
...
def compute_relative_errors(values, reference):
    ...
    array_ns = data_alloc.array_namespace(values)
    error_l1 = array_ns.sum(array_ns.abs(values - reference)) / array_ns.sum(
        array_ns.abs(reference)
    )
    error_linf = array_ns.max(array_ns.abs(values - reference)) / array_ns.max(
        array_ns.abs(reference)
    )
...
def prepare_torus_quadrature(
    ...
    if use_high_order_quadrature:
        ...
        weights = array_ns.ones_like(cell_center_x).reshape((1, -1))
        nodes = array_ns.stack((cell_center_x, cell_center_y)).reshape((2, 1, -1))
