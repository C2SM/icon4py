...
def gauss3d(
    *,
    ...
    prognostic_state_now.vn.ndarray[:, :] = u_field * primal_normal_x[:, array_ns.newaxis]  # type: ignore[index]  # NDArrayObject Protocol lacks item assignment

    for k_index in range(num_levels - 1, -1, -1):
        z_help = (brunt_vais / phy_const.GRAV) ** 2 * geopot[:, k_index]
        theta_v_ndarray[:, k_index] = t0 * array_ns.exp(z_help)  # type: ignore[index]  # NDArrayObject Protocol lacks item assignment

    if brunt_vais != 0.0:
        z_help = (brunt_vais / phy_const.GRAV) ** 2 * geopot[:, num_levels - 1]
        exner_ndarray[:, num_levels - 1] = (  # type: ignore[index]  # NDArrayObject Protocol lacks item assignment
            phy_const.GRAV / brunt_vais
        ) ** 2 / t0 / phy_const.CPD * (array_ns.exp(-z_help) - 1.0) + 1.0
    else:
        exner_ndarray[:, num_levels - 1] = 1.0 - geopot[:, num_levels - 1] / phy_const.CPD / t0  # type: ignore[index]  # NDArrayObject Protocol lacks item assignment
