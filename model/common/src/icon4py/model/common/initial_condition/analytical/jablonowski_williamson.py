...
def jablonowski_williamson(  # noqa: PLR0915 [too-many-statements]
    *,
    ...
    for k_index in range(num_levels - 1, -1, -1):
        ...
        for _ in range(100):
            eta_v_ndarray[:, k_index] = (eta_old - eta_0) * math.pi * 0.5  # type: ignore[index]  # NDArrayObject Protocol lacks item assignment
        ...
        eta_v_ndarray[:, k_index] = (eta_old - eta_0) * math.pi * 0.5  # type: ignore[index]  # NDArrayObject Protocol lacks item assignment
        exner_ndarray[:, k_index] = (eta_old * p_sfc / phy_const.P0REF) ** phy_const.RD_O_CPD  # type: ignore[index]  # NDArrayObject Protocol lacks item assignment
        theta_v_ndarray[:, k_index] = temperature_jw / exner_ndarray[:, k_index]  # type: ignore[index]  # NDArrayObject Protocol lacks item assignment
        rho_ndarray[:, k_index] = (  # type: ignore[index]  # NDArrayObject Protocol lacks item assignment
    ...
    prognostic_state_now.vn.ndarray[:, :] = testcases_utils.zonalwind_2_normalwind_ndarray(  # type: ignore[index]  # NDArrayObject Protocol lacks item assignment
        grid=grid,
        u0=u0,
