...
def orchestration(fuse_func):
    def wrapper(*args, **kwargs):
        if backend == run_dace_cpu_noopt:
            KDim_sym_=args[2].rho.ndarray.shape[1]
            ...
            kwargs.update({
                ...
                **{f"__gids_{ind.name}_{dim.value}":self._exchange._decomposition_info.global_index(dim, ind) if isinstance(self._exchange, GHexMultiNodeExchange) else np.empty(1, dtype=np.int64) for ind in (di.EntryType.ALL, di.EntryType.OWNED, di.EntryType.HALO) for dim in (CellDim, VertexDim, EdgeDim)},
                #
                **{"CellDim_sym": self.grid.offset_providers['C2E'].table.shape[0], "EdgeDim_sym": self.grid.offset_providers['E2C'].table.shape[0], "KDim_sym": KDim_sym_},
                #
                **{f"DiffusionDiagnosticState_{member}_s{str(stride)}_sym": getattr(k_v[1], member).ndarray.strides[stride] // 8 for k_v in list(zip(list(fuse_func.__annotations__.values()), list(args) + list(kwargs.values()))) for member in ["hdef_ic", "div_ic", "dwdx", "dwdy"] for stride in [0,1] if k_v[0] is DiffusionDiagnosticState_dace_t},
                #
                **{f"PrognosticState_{member}_s{str(stride)}_sym": getattr(k_v[1], member).ndarray.strides[stride] // 8 for k_v in list(zip(list(fuse_func.__annotations__.values()), list(args) + list(kwargs.values()))) for member in ["rho", "w", "vn", "exner", "theta_v"] for stride in [0,1] if k_v[0] is PrognosticState_dace_t},
                })

            diagnostic_state = DiffusionDiagnosticState_dace_t.dtype._typeclass.as_ctypes()(
                hdef_ic=args[1].hdef_ic.data_ptr(),
                div_ic=args[1].div_ic.data_ptr(),
                dwdx=args[1].dwdx.data_ptr(),
                dwdy=args[1].dwdy.data_ptr(),
            )
            diagnostic_state.descriptor = DiffusionDiagnosticState_dace_t

            prognostic_state = PrognosticState_dace_t.dtype._typeclass.as_ctypes()(
                rho=args[2].rho.data_ptr(),
                w=args[2].w.data_ptr(),
                vn=args[2].vn.data_ptr(),
                exner=args[2].exner.data_ptr(),
                theta_v=args[2].theta_v.data_ptr(),
            )
            prognostic_state.descriptor = PrognosticState_dace_t

            new_args = list(args)
            new_args[1] = diagnostic_state
            new_args[2] = prognostic_state
