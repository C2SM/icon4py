        ...
        def wrapper(*args, **kwargs):
            if dace_orchestration():
                if method:
                    ...
                    warnings.warn(
                        "The orchestration decorator is only for methods -at least for now-. Returning the original function.",
                        stacklevel=2,
                    )
                    return fuse_func(
                        *args, **kwargs
                    )  # fallback option is always the non-orchstrated function
                ...
                if not has_exchange or not has_grid:
                    warnings.warn(
                        "No exchnage/grid object found. Returning the original function.",
                        stacklevel=2,
                    )
...
if dace_orchestration():
    ...
    def add_halo_exchanges(
        sdfg: dace.SDFG,
        exchange: Union[SingleNodeExchange, GHexMultiNodeExchange],
        offset_providers: dict[str, Any],
        unique_id: int,
    ):
        ...
        for nested_sdfg in (
        ...
        ):  # loop over all nested sdfgs (aka stencils) and decide which fields need to be exchanged
            ...
            if len(field_dims) > 1:
                raise ValueError(
                    "The output fields to be communicated are not defined on the same dimension."
                )

            # dimension of the fields to be exchanged
            dim = next(iter(field_dims)) if len(field_dims) == 1 else None
            if not dim:
                continue

            if (
                nested_sdfg.parent_nsdfg_node.symbol_mapping.get("horizontal_start", None) is None
                or nested_sdfg.parent_nsdfg_node.symbol_mapping.get("horizontal_end", None) is None
            ):
                continue

            halos_inds = exchange._decomposition_info.local_index(
            ...
            updated_halo_inds = np.intersect1d(
                halos_inds,
                np.arange(
                    nested_sdfg.parent_nsdfg_node.symbol_mapping["horizontal_start"],
                    nested_sdfg.parent_nsdfg_node.symbol_mapping["horizontal_end"],
                ),
            )
            ...
            global_buffers: dict[
                str, dace.data.Data
            ] = {}  # the elements of this dictionary are going to be exchanged
            for (
                buffer_name
            ) in nested_sdfg.gt4py_program_output_fields:  # Check which fields need to be exchanged
                global_buffer_name = (
                    None  # from nested SDFG to the name in the fused SDFG -global sense-
                )
                ...
                if not global_buffer_name:
                    raise ValueError(
                        "Could not link the local buffer_name to the global one -from the nested SDFG to the fused-."
                    )
                ...
                for ones_after_nsdfg in sdfg.all_sdfgs_recursive():
                    ...
                    if (
                        ones_after_nsdfg.parent_nsdfg_node.symbol_mapping.get(
                            "horizontal_start", None
                        )
                        is None
                        or ones_after_nsdfg.parent_nsdfg_node.symbol_mapping.get(
                            "horizontal_end", None
                        )
                        is None
                    ):
                    ...
                    for buffer_name_ in ones_after_nsdfg.gt4py_program_input_fields:
                        ...
                        if not global_buffer_name_:
                            raise ValueError(
                                "Could not link the local buffer_name_ to the global one."
                            )
                        ...
                        for op_ in ones_after_nsdfg.offset_providers_per_input_field[buffer_name_]:
                            ...
                            source_dim = {"C": CellDim, "E": EdgeDim, "V": VertexDim}[
                                offset_literal_value[0]
                            ]
                            if source_dim != next(
                                iter(ones_after_nsdfg.gt4py_program_output_fields.values())
                            ):
                                continue

                            accessed_inds = op[
                                ones_after_nsdfg.parent_nsdfg_node.symbol_mapping[
                                    "horizontal_start"
                                ]
                            ]
                            for ind in range(
                                ...
                                + 1,
                                ones_after_nsdfg.parent_nsdfg_node.symbol_mapping["horizontal_end"],
                            ):
                ...
                if halo_update:
                    global_buffers[global_buffer_name] = sdfg.arrays[
                        global_buffer_name
    ...
    class DaceGHEX:
        python_site_packages: ClassVar = site.getsitepackages()[0]
        ghex_path: ClassVar = (
