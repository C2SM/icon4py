...
def configure_dace_temp_env(default_build_folder: Path, backend) -> core_defs.DeviceType:
    ...
    device_type = backend.executor.step.translation.step.device_type if backend else None
    if device_type == core_defs.DeviceType.CPU:
        device = "cpu"
        ...
        for k_v in flattened_xargs_type_value:
            ...
            for member in orig_cls.__dataclass_fields__:
                for stride in range(getattr(k_v[1], member).ndarray.ndim):
                    concretized_symbols[
                        orchestration_dtypes.stride_symbol_name_from_field(orig_cls, member, stride)
