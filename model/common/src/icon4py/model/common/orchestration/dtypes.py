...
if dace:
    ...
    OffsetProviders_t_members = [
        name
        for name, value in inspect.getmembers(dimension)
        if isinstance(value, FieldOffset) and value.source.kind == DimensionKind.HORIZONTAL
    ]
    ...
    OffsetProviders_int64_t_dict = {
        member: dace.data.Array(
            dtype=dace.int64,
            shape=[
                OffsetProviders_symbols[field_size_symbol_name(connectivity_identifier(member), 0)],
                OffsetProviders_symbols[field_size_symbol_name(connectivity_identifier(member), 1)],
            ],
            strides=[
                OffsetProviders_symbols[
                    field_stride_symbol_name(connectivity_identifier(member), 0)
                ],
                OffsetProviders_symbols[
                    field_stride_symbol_name(connectivity_identifier(member), 1)
                ],
            ],
        )
        for member in OffsetProviders_t_members
    }
    OffsetProviders_int32_t_dict = {
        member: dace.data.Array(
            dtype=dace.int32,
            shape=[
                OffsetProviders_symbols[field_size_symbol_name(connectivity_identifier(member), 0)],
    ...
    OffsetProviders_int64_t = dace.data.Structure(
        OffsetProviders_int64_t_dict, name="OffsetProviders_int64_t"
    )
    OffsetProviders_int32_t = dace.data.Structure(
        OffsetProviders_int32_t_dict, name="OffsetProviders_int32_t"
    )
