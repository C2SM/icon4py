...
class GridFile:
    ...
    class PropertyName(GridFileName):
        ...
        EDGE_LENGTH = "mean_edge_length"  # only for torus
...
class GridManager:
    ...
    def _from_grid_dataset(
        ...
        try:
            # grid geometry is not always present in the grid file, especially it is missing from the icosahedron grid generated by DWD icontools. We set the default to ICOSAHEDRON.
            grid_geometry = dataset.getncattr(GridFile.PropertyName.GEOMETRY)
            if grid_geometry == icon.GridGeometryType.TORUS:
                assert (
                    grid_level == 0 and grid_root == 2
                ), "Torus file should have root=2 and level=0"
        except AttributeError:
            grid_geometry = icon.GridGeometryType.ICOSAHEDRON

        if grid_geometry == icon.GridGeometryType.TORUS:
            edge_length = dataset.getncattr("mean_edge_length")
            global_params = icon.Torus(edge_length, num_cells)
        else:
            global_params = icon.Icosahedron(level=grid_level, root=grid_root)
