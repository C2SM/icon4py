...
def as_numpy(array: NDArrayInterface, copy=False) -> np.ndarray:
    buffer = array.ndarray if isinstance(array, gtx.Field) else array
    if isinstance(buffer, np.ndarray):
        return np.copy(buffer) if copy else buffer
    else:
        import cupy as cp
