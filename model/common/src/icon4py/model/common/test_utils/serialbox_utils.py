...
class IconGridSavepoint(IconSavepoint):
    ...
    def print_connectivity_info(self, name: str, ar: fa.NDArrayInterface):
...
class InterpolationSavepoint(IconSavepoint):
    ...
    def geofac_grg(self):
        ...
        return gtx.as_field(
            (dims.CellDim, dims.C2E2CODim), grg[:num_cells, :, 0]
        ), gtx.as_field((dims.CellDim, dims.C2E2CODim), grg[:num_cells, :, 1])
...
class MetricSavepoint(IconSavepoint):
    ...
    def _linearize_first_2dims(
        self, data: fa.NDArrayInterface, sparse_size: int, target_dims: tuple[gtx.Dimension, ...]
    ):
        old_shape = data.shape
        assert old_shape[1] == sparse_size
        return gtx.as_field(
            target_dims, data.reshape(old_shape[0] * old_shape[1], old_shape[2])
        )
