...
def get_dace_options(
    ...
    if program_name in [
        ...
        if gtx_transformations.GT4PyAutoOptHook.TopLevelDataFlowStep not in optimization_hooks:
            ...
            if "scan_unroll" not in optimization_args:
                optimization_args["scan_unroll"] = True
            if "scan_unroll_factor" not in optimization_args:
                optimization_args["scan_unroll_factor"] = 0
