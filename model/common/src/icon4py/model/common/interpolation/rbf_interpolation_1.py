...
def _compute_z_xn1_z_xn2(
    ...
    for jc in range(lower_bound, upper_bound):
        ...
        z_nx1[jc, 0], z_nx1[jc, 1], z_nx1[jc, 2] = _gvec2cvec(1.0, 0.0, lon[jc], lat[jc])
        z_norm = math.sqrt(np.dot(z_nx1[jc, :], z_nx1[jc, :]))
        z_nx1[jc, :] = 1.0 / z_norm * z_nx1[jc, :]
        z_nx2[jc, 0], z_nx2[jc, 1], z_nx2[jc, 2] = _gvec2cvec(0.0, 1.0, lon[jc], lat[jc])
        z_norm = math.sqrt(np.dot(z_nx2[jc, :], z_nx2[jc, :]))
        z_nx2[jc, :] = 1.0 / z_norm * z_nx2[jc, :]
...
def _compute_rbf_vec_scale_c(mean_characteristic_length):
    ...
    rbf_vec_scale_c = 0.5 / (1.0 + 1.8 * math.log(2.5 / resol) ** 3.75) if resol < 2.5 else 0.5
    rbf_vec_scale_c = (
        rbf_vec_scale_c * (resol / 0.125) ** 0.9 if resol <= 0.125 else rbf_vec_scale_c
    )
...
def _gaussi(p_x, p_scale):
    ...
    p_rbf_val = -1.0 * p_rbf_val * p_rbf_val
...
def _inv_multiq(p_x, p_scale):
    ...
    p_rbf_val = np.sqrt(1.0 + p_rbf_val)
    p_rbf_val = 1.0 / p_rbf_val
    return p_rbf_val
        ...
        for je2 in range(je1 + 1):
            for jc in range(lower_bound, upper_bound):
                ...
                if je1 > istencil[jc] or je2 > istencil[jc]:
                    continue
                cc_e1 = (
                    cartesian_center_e[0][int(ile1)].ndarray,
                ...
                if rbf_vec_kern_c == 1:
                    z_rbfmat[jc, je1, je2] = z_nxprod * _gaussi(
                        z_dist, rbf_vec_scale_c
                    )  # [max(jg, 0)])
                elif rbf_vec_kern_c == 3:
                    z_rbfmat[jc, je1, je2] = z_nxprod * _inv_multiq(
                        z_dist, rbf_vec_scale_c
                    )  # [max(jg, 0)])
