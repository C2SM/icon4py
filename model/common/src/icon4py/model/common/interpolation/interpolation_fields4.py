...
def compute_exner_exfac(
    ...
    llb = horizontal_start
    z_maxslp = np.zeros([c2e.shape[0], ddxn_z_full.shape[1]])
    z_maxhgtd = np.zeros([c2e.shape[0], ddxn_z_full.shape[1]])
    exner_exfac = np.zeros([c2e.shape[0], ddxn_z_full.shape[1]]) + exner_expol
    z_maxslp[llb:, :] = np.max(abs(ddxn_z_full[c2e[llb:, :], :]), axis=1)
    for jk in range(ddxn_z_full.shape[1]):
        z_maxhgtd[llb:, jk] = np.max(abs(ddxn_z_full[c2e[llb:, :], jk] * dual_edge_length[c2e[llb:, :]]), axis=1)
    exner_exfac[llb:, :] = exner_expol * np.where(1.0 - np.square(4.0 * z_maxslp[llb:, :]) < 1.0 - np.square(0.002 * z_maxhgtd[llb:, :]), 1.0 - np.square(4.0 * z_maxslp[llb:, :]), 1.0 - np.square(0.002 * z_maxhgtd[llb:, :]))
    exner_exfac[llb:, :] = np.where(exner_exfac[llb:, :] < 0.0, 0.0, exner_exfac[llb:, :])
    exner_exfac[llb:, :] = np.where(z_maxslp[llb:, :] > 1.5, np.where(-1.0 / 6.0 > 1.0 / 9.0 * (1.5 - z_maxslp[llb:, :]), -1.0 / 6.0, 1.0 / 9.0 * (1.5 - z_maxslp[llb:, :])), exner_exfac[llb:, :])
    return exner_exfac
