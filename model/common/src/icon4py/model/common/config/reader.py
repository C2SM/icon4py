...
class ConfigReader(Generic[T]):
    ...
    def __init__(self, schema: T|type[T]):
        ...
        self._default_config:oc.DictConfig = oc.OmegaConf.structured(schema) # TODO should this use create?
    ...
    def as_type(self)->T:
        mode = oc.SCMode.INSTANTIATE if not isinstance(self._schema, oc.DictConfig) else oc.SCMode.DICT
        return oc.OmegaConf.to_container(self._config, resolve=True, throw_on_missing=True, structured_config_mode=mode)
