...
def test_compute_zdiff_gradp_dsl(icon_grid, metrics_savepoint, interpolation_savepoint, backend, experiment):
    ...
    k_lev = data_alloc.index_field(icon_grid, 
        dims.KDim, dtype=gtx.int32, backend=backend
    )
    flat_idx = data_alloc.zero_field(icon_grid, dims.EdgeDim, dims.KDim, dtype=gtx.int32, backend=backend)
    ...
    assert dallclose(data_alloc.as_numpy(zdiff_gradp_full_field),
                     zdiff_gradp_ref.asnumpy(), atol=1e-10, rtol=1.0e-9)
