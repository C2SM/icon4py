...
class TestComputeZMc(StencilTest):
    ...
    def input_data(self, grid) -> dict:
        z_mc = field_utils.zero_field(grid, dims.CellDim, dims.KDim)
        z_if = field_utils.random_field(grid, dims.CellDim, dims.KDim, extend={dims.KDim: 1})
        horizontal_start = 0
...
def test_compute_ddq_z_half(icon_grid, metrics_savepoint, backend):
    ...
    z_mc = field_utils.zero_field(icon_grid, dims.CellDim, dims.KDim, extend={dims.KDim: 1})
    ...
    ddqz_z_half = field_utils.zero_field(icon_grid, dims.CellDim, dims.KDim, extend={dims.KDim: 1})
...
def test_compute_ddqz_z_full_and_inverse(icon_grid, metrics_savepoint, backend):
    ...
    ddqz_z_full = field_utils.zero_field(icon_grid, dims.CellDim, dims.KDim)
    inv_ddqz_z_full = field_utils.zero_field(icon_grid, dims.CellDim, dims.KDim)
...
def test_compute_scalfac_dd3d(icon_grid, metrics_savepoint, grid_savepoint, backend):
    ...
    scalfac_dd3d_full = field_utils.zero_field(icon_grid, dims.KDim)
...
def test_compute_rayleigh_w(icon_grid, experiment, metrics_savepoint, grid_savepoint, backend):
    ...
    rayleigh_w_full = field_utils.zero_field(icon_grid, dims.KDim, extend={dims.KDim: 1})
    rayleigh_type = 2
...
def test_compute_coeff_dwdz(icon_grid, metrics_savepoint, grid_savepoint, backend):
    ...
    coeff1_dwdz_full = field_utils.zero_field(icon_grid, dims.CellDim, dims.KDim)
    coeff2_dwdz_full = field_utils.zero_field(icon_grid, dims.CellDim, dims.KDim)
...
def test_compute_d2dexdz2_fac_mc(icon_grid, metrics_savepoint, grid_savepoint, backend):
    ...
    if is_roundtrip(backend):
        pytest.skip("skipping: slow backend")
    z_ifc = metrics_savepoint.z_ifc()
    z_mc = field_utils.zero_field(icon_grid, dims.CellDim, dims.KDim)
    compute_z_mc.with_backend(backend)(
        z_ifc=z_ifc,
        z_mc=z_mc,
        horizontal_start=0,
    ...
    d2dexdz2_fac1_mc_full = field_utils.zero_field(icon_grid, dims.CellDim, dims.KDim)
    d2dexdz2_fac2_mc_full = field_utils.zero_field(icon_grid, dims.CellDim, dims.KDim)
    cpd = constants.CPD
    grav = constants.GRAV
...
def test_compute_ddxt_z_full_e(
    ...
    z_ifv = field_utils.zero_field(icon_grid, dims.VertexDim, dims.KDim, extend={dims.KDim: 1})
    ...
    ddxt_z_half_e = field_utils.zero_field(icon_grid, dims.EdgeDim, dims.KDim, extend={dims.KDim: 1})
    ...
    ddxt_z_full = field_utils.zero_field(icon_grid, dims.EdgeDim, dims.KDim)
...
def test_compute_vwind_expl_wgt(icon_grid, metrics_savepoint, backend):
    vwind_expl_wgt_full = field_utils.zero_field(icon_grid, dims.CellDim)
...
def test_compute_ddqz_z_full_e(
    ...
    vertical_start = 0
    vertical_end = icon_grid.num_levels
    ddqz_z_full_e = field_utils.zero_field(icon_grid, dims.EdgeDim, dims.KDim)
...
def test_compute_ddxn_z_full(
    ...
    vertical_start = 0
    vertical_end = icon_grid.num_levels + 1
    ddxn_z_half_e = field_utils.zero_field(icon_grid, dims.EdgeDim, dims.KDim, extend={dims.KDim: 1})
    ...
    ddxn_z_full = field_utils.zero_field(icon_grid, dims.EdgeDim, dims.KDim)
    compute_ddxn_z_full.with_backend(backend)(
...
def test_compute_vwind_impl_wgt(
    ...
    z_ifc = metrics_savepoint.z_ifc()
    inv_dual_edge_length = grid_savepoint.inv_dual_edge_length()
    z_ddxn_z_half_e = field_utils.zero_field(icon_grid, dims.EdgeDim, dims.KDim, extend={dims.KDim: 1})
    tangent_orientation = grid_savepoint.tangent_orientation()
    inv_primal_edge_length = grid_savepoint.inverse_primal_edge_lengths()
    z_ddxt_z_half_e = field_utils.zero_field(icon_grid, dims.EdgeDim, dims.KDim, extend={dims.KDim: 1})
    z_ifv = field_utils.zero_field(icon_grid, dims.VertexDim, dims.KDim, extend={dims.KDim: 1})
    horizontal_start = icon_grid.start_index(edge_domain(horizontal.Zone.LATERAL_BOUNDARY_LEVEL_2))
...
def test_compute_wgtfac_e(metrics_savepoint, interpolation_savepoint, icon_grid, backend):
    if is_roundtrip(backend):
        pytest.skip("skipping: slow backend")
    wgtfac_e = field_utils.zero_field(icon_grid, dims.EdgeDim, dims.KDim, extend={dims.KDim: 1})
...
def test_compute_pg_exdist_dsl(
    ...
    pg_edgeidx = field_utils.zero_field(icon_grid, dims.EdgeDim, dims.KDim, dtype=gtx.int32)
    pg_vertidx = field_utils.zero_field(icon_grid, dims.EdgeDim, dims.KDim, dtype=gtx.int32)
    pg_exdist_dsl = field_utils.zero_field(icon_grid, dims.EdgeDim, dims.KDim)
    z_me = field_utils.zero_field(icon_grid, dims.EdgeDim, dims.KDim)
    z_aux2 = field_utils.zero_field(icon_grid, dims.EdgeDim)
    z_mc = field_utils.zero_field(icon_grid, dims.CellDim, dims.KDim)
    flat_idx = field_utils.zero_field(icon_grid, dims.EdgeDim, dims.KDim)
    z_ifc = metrics_savepoint.z_ifc()
    ...
    pg_edgeidx_dsl = field_utils.zero_field(icon_grid, dims.EdgeDim, dims.KDim, dtype=bool)
...
def test_compute_mask_prog_halo_c(metrics_savepoint, icon_grid, grid_savepoint, backend):
    mask_prog_halo_c_full = field_utils.zero_field(icon_grid, dims.CellDim, dtype=bool)
...
def test_compute_bdy_halo_c(metrics_savepoint, icon_grid, grid_savepoint, backend):
    bdy_halo_c_full = field_utils.zero_field(icon_grid, dims.CellDim, dtype=bool)
    c_refin_ctrl = grid_savepoint.refin_ctrl(dims.CellDim)
...
def test_compute_hmask_dd3d(metrics_savepoint, icon_grid, grid_savepoint, backend):
    hmask_dd3d_full = field_utils.zero_field(icon_grid, dims.EdgeDim)
