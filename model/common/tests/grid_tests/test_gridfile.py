...
def test_grid_file_dimension():
    ...
    try:
        ...
        assert parser.dimension(
            gridfile.DimensionName.CELL_NAME) == utils.R02B04_GLOBAL_NUM_CELLS
        assert parser.dimension(
            gridfile.DimensionName.VERTEX_NAME) == utils.R02B04_GLOBAL_NUM_VERTEX
        assert parser.dimension(
            gridfile.DimensionName.EDGE_NAME) == utils.R02B04_GLOBAL_NUM_EDGES
    except Exception:
        pytest.fail()
    finally:
        parser.close()
...
def test_grid_file_vertex_cell_edge_dimensions(grid_savepoint, grid_file):
    ...
    try:
        parser.open()
        assert parser.dimension(
            gridfile.DimensionName.CELL_NAME) == grid_savepoint.num(dims.CellDim)
        assert parser.dimension(
            gridfile.DimensionName.VERTEX_NAME) == grid_savepoint.num(dims.VertexDim)
        assert parser.dimension(
            gridfile.DimensionName.EDGE_NAME) == grid_savepoint.num(dims.EdgeDim)
