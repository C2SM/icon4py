...
def test_grid_manager_refin_ctrl(grid_savepoint, grid_file, experiment, dim, backend):
    refin_ctrl = utils.run_grid_manager(grid_file, keep_skip_values=True, backend= backend).refinement
...
def test_grid_manager_grid_size(dim, size, backend):
    grid =utils.run_grid_manager(dt_utils.R02B04_GLOBAL, keep_skip_values=True, backend =  backend).grid
...
def test_grid_manager_grid_level_and_root(grid_file, global_num_cells, backend):
    assert global_num_cells == utils.run_grid_manager(grid_file, keep_skip_values=True, backend=backend).grid.global_num_cells
