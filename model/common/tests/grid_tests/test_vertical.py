...
def test_damping_layer_calculation(max_h, damping_height, delta, flat_height):
    vct_a = np.arange(0, max_h, delta)[::-1]
...
def test_damping_layer_calculation_from_icon_input(
    grid_savepoint, experiment, damping_height, flat_height, backend
):
...
@pytest.mark.datatest
def test_grid_size(grid_savepoint, backend):
...
@pytest.mark.datatest
def test_grid_size_raises_for_unknown_vertical_dim(grid_savepoint, backend):
    vertical_grid = configure_vertical_grid(grid_savepoint)
...
def test_vct_a_vct_b_calculation_from_icon_input(
    ...
    assert helpers.dallclose(vct_a, grid_savepoint.vct_a().asnumpy())
    assert helpers.dallclose(vct_b, grid_savepoint.vct_b().asnumpy())
