...
def grid_from_file() -> icon.IconGrid:
    return from_file(cases.SerializedExperiment.MCH_CH_R04B09)
...
@pytest.mark.parametrize("grid_file", (cases.SerializedExperiment.MCH_CH_R04B09, dt_utils.R02B04_GLOBAL))
def test_has_skip_values(grid_file):
    grid = from_file(grid_file)
    assert grid.has_skip_values()


@pytest.mark.parametrize("grid_file", (dt_utils.R02B04_GLOBAL, cases.SerializedExperiment.MCH_CH_R04B09))
