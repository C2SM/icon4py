...
def run_grid_manager(file: str, keep_skip_values: bool, backend: Optional[gtx_backend.Backend]) -> gm.GridManager:
    key ="_".join((file, data_alloc.backend_name(backend), "skip" if keep_skip_values else "no_skip"))
    if not managers.get(key):
        manager = gridtest_utils.get_grid_manager(file, keep_skip_values=keep_skip_values, num_levels=1, backend=backend)
