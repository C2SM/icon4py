...
def test_decomposition_info_masked(
    ...
    owned_indices = decomposition_info.global_index(dim, definitions.DecompositionInfo.EntryType.OWNED)
    assert owned_indices.shape[0] == my_owned

    halo_indices = decomposition_info.global_index(dim, definitions.DecompositionInfo.EntryType.HALO)
...
def test_decomposition_info_local_index(
    ...
    owned_indices = decomposition_info.local_index(dim, definitions.DecompositionInfo.EntryType.OWNED)
    assert owned_indices.shape[0] == my_owned
...
def test_decomposition_info_matches_gridsize(
    ...
    assert (
        decomposition_info.global_index(dims.VertexDim, definitions.DecompositionInfo.EntryType.ALL).shape[0]
        == icon_grid.num_vertices
    )
    assert (
        decomposition_info.global_index(dims.EdgeDim, definitions.DecompositionInfo.EntryType.ALL).shape[0]
        == icon_grid.num_edges
    )
...
def test_exchange_on_dummy_data(
    ...
    halo_points = decomposition_info.local_index(dimension, definitions.DecompositionInfo.EntryType.HALO)
    local_points = decomposition_info.local_index(dimension, definitions.DecompositionInfo.EntryType.OWNED)
