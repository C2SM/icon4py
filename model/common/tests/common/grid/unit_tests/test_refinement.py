...
def test_compute_start_index_for_limited_area_grid(dim, zone, expected):
...
@pytest.mark.parametrize("dim, zone, expected", [
                                            (dims.CellDim, h_grid.Zone.LATERAL_BOUNDARY, 0),
                                            (dims.CellDim, h_grid.Zone.LATERAL_BOUNDARY_LEVEL_2, 0),
                                            (dims.CellDim, h_grid.Zone.LATERAL_BOUNDARY_LEVEL_3, 0),
                                            (dims.CellDim, h_grid.Zone.LATERAL_BOUNDARY_LEVEL_4, 0),
                                            (dims.CellDim, h_grid.Zone.NUDGING, 0),
(dims.CellDim, h_grid.Zone.LOCAL, 0), (dims.CellDim, h_grid.Zone.INTERIOR, 0),
                                            (dims.EdgeDim, h_grid.Zone.LATERAL_BOUNDARY, 0),
                                            (dims.EdgeDim, h_grid.Zone.LATERAL_BOUNDARY_LEVEL_2, 0),
                                            (dims.EdgeDim, h_grid.Zone.LATERAL_BOUNDARY_LEVEL_3, 0),
                                            (dims.EdgeDim, h_grid.Zone.LATERAL_BOUNDARY_LEVEL_4, 0),
                                            (dims.EdgeDim, h_grid.Zone.LATERAL_BOUNDARY_LEVEL_5, 0),
                                            (dims.EdgeDim, h_grid.Zone.LATERAL_BOUNDARY_LEVEL_6, 0),
                                            (dims.EdgeDim, h_grid.Zone.LATERAL_BOUNDARY_LEVEL_7, 0),
                                            (dims.EdgeDim, h_grid.Zone.LATERAL_BOUNDARY_LEVEL_8, 0),
                                            (dims.EdgeDim, h_grid.Zone.NUDGING, 0),
                                            (dims.EdgeDim, h_grid.Zone.NUDGING_LEVEL_2, 0),
(dims.EdgeDim, h_grid.Zone.LOCAL, 0), (dims.EdgeDim, h_grid.Zone.INTERIOR, 0),
                                            (dims.VertexDim, h_grid.Zone.LATERAL_BOUNDARY, 0),
                                            (dims.VertexDim, h_grid.Zone.LATERAL_BOUNDARY_LEVEL_2, 0),
                                            (dims.VertexDim, h_grid.Zone.LATERAL_BOUNDARY_LEVEL_3, 0),
                                            (dims.VertexDim, h_grid.Zone.LATERAL_BOUNDARY_LEVEL_4,0),
(dims.VertexDim, h_grid.Zone.LOCAL, 0), (dims.VertexDim, h_grid.Zone.INTERIOR, 0)
])
def test_compute_start_index_for_global_grid(dim, zone, expected):
...
@pytest.mark.parametrize("dim, zone, expected", [
    ...
    (dims.CellDim, h_grid.Zone.LATERAL_BOUNDARY_LEVEL_4, 0),
    (dims.CellDim, h_grid.Zone.NUDGING, 0),
    (dims.CellDim, h_grid.Zone.LOCAL, "size"),
    (dims.CellDim, h_grid.Zone.INTERIOR, "size"),
    (dims.EdgeDim, h_grid.Zone.LATERAL_BOUNDARY, 0),
    (dims.EdgeDim, h_grid.Zone.LATERAL_BOUNDARY_LEVEL_2, 0),
    (dims.EdgeDim, h_grid.Zone.LATERAL_BOUNDARY_LEVEL_3, 0),
    (dims.EdgeDim, h_grid.Zone.LATERAL_BOUNDARY_LEVEL_4, 0),
    ...
    (dims.EdgeDim, h_grid.Zone.NUDGING, 0),
    (dims.EdgeDim, h_grid.Zone.NUDGING_LEVEL_2, 0),
    (dims.EdgeDim, h_grid.Zone.LOCAL, "size"), (dims.EdgeDim, h_grid.Zone.INTERIOR, "size"),
    (dims.VertexDim, h_grid.Zone.LATERAL_BOUNDARY, 0),
    (dims.VertexDim, h_grid.Zone.LATERAL_BOUNDARY_LEVEL_2, 0),
    (dims.VertexDim, h_grid.Zone.LATERAL_BOUNDARY_LEVEL_3, 0),
    (dims.VertexDim, h_grid.Zone.LATERAL_BOUNDARY_LEVEL_4, 0),
    (dims.VertexDim, h_grid.Zone.LOCAL, "size"), (dims.VertexDim, h_grid.Zone.INTERIOR, "size"),
])
def test_compute_end_index_for_global_grid(dim, zone, expected):
    grid = grid_utils.get_grid_manager_from_identifier(test_defs.Grids.R02B04_GLOBAL.name, 1, True, None).grid
    refinement_field = grid.refinement_control
    # TODO(halungge): for debugging - remove this
    reference_start_indices = grid._start_indices
    reference_end_indices = grid._end_indices
    domain = h_grid.domain(dim)(zone)
    end_index = refin.compute_end_index(domain, refinement_field, array_ns=np)
    assert isinstance(end_index, gtx.int32)
    if expected == "size":
        assert end_index == grid.size[dim]
    else:
        assert end_index == 0
    # grid file start_idx_[c, v, e] is set to grid.size[dim] for global grids, but current grid._start_indices[domain], grid._end_indices[domain] is set to
    # TODO(halungge): for debugging - remove this before merge
    assert end_index == reference_end_indices[domain]
