...
def test_cartesian_centers_edge(
    ...
    match grid.geometry_type:
        case base.GeometryType.ICOSAHEDRON:
            ...
            vector_operations.norm2_on_edges(x, z, y, out=norm, offset_provider={})
...
def test_cartesian_centers_cell(
    ...
    match grid.geometry_type:
        case base.GeometryType.ICOSAHEDRON:
            ...
            vector_operations.norm2_on_cells(x, z, y, out=norm, offset_provider={})
            assert test_utils.dallclose(norm.asnumpy(), 1.0)
        case base.GeometryType.TORUS:
            # on a torus coordinates should be within the domain
            assert all(x.asnumpy() >= 0.0)
...
def test_vertex(
    ...
    match grid.geometry_type:
        case base.GeometryType.ICOSAHEDRON:
            ...
            vector_operations.norm2_on_vertices(x, z, y, out=norm, offset_provider={})
            assert test_utils.dallclose(norm.asnumpy(), 1.0)
        case base.GeometryType.TORUS:
            # on a torus coordinates should be within the domain
            assert all(x.asnumpy() >= 0.0)
