...
class TestComputeCells2VertsInterpolation(stencil_tests.StencilTest):
    ...
    def input_data(self, grid: base.Grid) -> dict:
        cell_in = data_alloc.random_field(grid, dims.CellDim, dims.KHalfDim, dtype=types.wpfloat)
        c_int = data_alloc.random_field(grid, dims.VertexDim, dims.V2CDim, dtype=types.wpfloat)
        vert_out = data_alloc.zero_field(grid, dims.VertexDim, dims.KHalfDim, dtype=types.wpfloat)
