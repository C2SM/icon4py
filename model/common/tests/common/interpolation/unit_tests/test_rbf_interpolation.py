...
def test_rbf_interpolation_coeffs_cell(
    ...
    rbf_vec_coeff_c1, rbf_vec_coeff_c2 = rbf.compute_rbf_interpolation_coeffs_cell(  # type: ignore[misc] # function returns two vars
        cell_center_lat=geometry.get_full_precision(geometry_attrs.CELL_LAT).ndarray,
        cell_center_lon=geometry.get_full_precision(geometry_attrs.CELL_LON).ndarray,
        cell_center_x=geometry.get_full_precision(geometry_attrs.CELL_CENTER_X).ndarray,
        cell_center_y=geometry.get_full_precision(geometry_attrs.CELL_CENTER_Y).ndarray,
        cell_center_z=geometry.get_full_precision(geometry_attrs.CELL_CENTER_Z).ndarray,
        edge_center_x=geometry.get_full_precision(geometry_attrs.EDGE_CENTER_X).ndarray,
        edge_center_y=geometry.get_full_precision(geometry_attrs.EDGE_CENTER_Y).ndarray,
        edge_center_z=geometry.get_full_precision(geometry_attrs.EDGE_CENTER_Z).ndarray,
        edge_normal_x=geometry.get_full_precision(geometry_attrs.EDGE_NORMAL_X).ndarray,
        edge_normal_y=geometry.get_full_precision(geometry_attrs.EDGE_NORMAL_Y).ndarray,
        edge_normal_z=geometry.get_full_precision(geometry_attrs.EDGE_NORMAL_Z).ndarray,
...
def test_rbf_interpolation_coeffs_vertex(
    ...
    rbf_vec_coeff_v1, rbf_vec_coeff_v2 = rbf.compute_rbf_interpolation_coeffs_vertex(
        vertex_lat=geometry.get_full_precision(geometry_attrs.VERTEX_LAT).ndarray,
        vertex_lon=geometry.get_full_precision(geometry_attrs.VERTEX_LON).ndarray,
        vertex_x=geometry.get_full_precision(geometry_attrs.VERTEX_X).ndarray,
        vertex_y=geometry.get_full_precision(geometry_attrs.VERTEX_Y).ndarray,
        vertex_z=geometry.get_full_precision(geometry_attrs.VERTEX_Z).ndarray,
        edge_center_x=geometry.get_full_precision(geometry_attrs.EDGE_CENTER_X).ndarray,
        edge_center_y=geometry.get_full_precision(geometry_attrs.EDGE_CENTER_Y).ndarray,
        edge_center_z=geometry.get_full_precision(geometry_attrs.EDGE_CENTER_Z).ndarray,
        edge_normal_x=geometry.get_full_precision(geometry_attrs.EDGE_NORMAL_X).ndarray,
...
def test_rbf_interpolation_coeffs_edge(
    ...
    rbf_vec_coeff_e = rbf.compute_rbf_interpolation_coeffs_edge(
        edge_lat=geometry.get_full_precision(geometry_attrs.EDGE_LAT).ndarray,
        edge_lon=geometry.get_full_precision(geometry_attrs.EDGE_LON).ndarray,
        edge_center_x=geometry.get_full_precision(geometry_attrs.EDGE_CENTER_X).ndarray,
        edge_center_y=geometry.get_full_precision(geometry_attrs.EDGE_CENTER_Y).ndarray,
        edge_center_z=geometry.get_full_precision(geometry_attrs.EDGE_CENTER_Z).ndarray,
        edge_normal_x=geometry.get_full_precision(geometry_attrs.EDGE_NORMAL_X).ndarray,
        edge_normal_y=geometry.get_full_precision(geometry_attrs.EDGE_NORMAL_Y).ndarray,
        edge_normal_z=geometry.get_full_precision(geometry_attrs.EDGE_NORMAL_Z).ndarray,
        edge_dual_normal_u=geometry.get_full_precision(geometry_attrs.EDGE_DUAL_U).ndarray,
        edge_dual_normal_v=geometry.get_full_precision(geometry_attrs.EDGE_DUAL_V).ndarray,
