...
def test_compute_geofac_n2s(
    ...
    geofac_n2s = compute_geofac_n2s(dual_edge_length.ndarray, geofac_div.ndarray, c2e, e2c, c2e2c, horizontal_start=horizontal_start)
...
def test_compute_geofac_grdiv(
    ...
    horizontal_start = icon_grid.start_index(edge_domain(h_grid.Zone.LATERAL_BOUNDARY_LEVEL_2))
    geofac_grdiv = compute_geofac_grdiv(geofac_div.ndarray, inv_dual_edge_length.ndarray, owner_mask.ndarray, c2e, e2c, e2c2e=e2c2e, horizontal_start=horizontal_start)
...
def test_compute_c_bln_avg(
    ...
    match icon_grid.global_properties.geometry_type:
        case base_grid.GeometryType.ICOSAHEDRON:
            c_bln_avg = compute_mass_conserving_bilinear_cell_average_weight(c2e2c0, lat, lon, cell_areas, cell_owner_mask, exchange=decomposition.single_node_exchange, divergence_averaging_central_cell_weight=divergence_averaging_central_cell_weight, horizontal_start=horizontal_start, horizontal_start_level_3=horizontal_start_p2)
        case base_grid.GeometryType.TORUS:
            c_bln_avg = compute_mass_conserving_bilinear_cell_average_weight_torus(c2e2c0, cell_areas, cell_owner_mask, divergence_averaging_central_cell_weight, horizontal_start, exchange=decomposition.single_node_exchange, horizontal_start_level_3=horizontal_start_p2)
...
def test_compute_e_bln_c_s(
    ...
    match icon_grid.global_properties.geometry_type:
        case base_grid.GeometryType.ICOSAHEDRON:
            e_bln_c_s = compute_e_bln_c_s(c2e, cells_lat, cells_lon, edges_lat, edges_lon, weighting_factor=0.0)
        case base_grid.GeometryType.TORUS:
...
def test_compute_pos_on_tplane_e(
    ...
    match icon_grid.global_properties.geometry_type:
        case base_grid.GeometryType.ICOSAHEDRON:
            pos_on_tplane_e_x, pos_on_tplane_e_y = compute_pos_on_tplane_e_x_y(sphere_radius, primal_normal_v1, primal_normal_v2, dual_normal_v1, dual_normal_v2, cells_lon=cells_lon, cells_lat=cells_lat, edges_lon=edges_lon, edges_lat=edges_lat, owner_mask=owner_mask, e2c=e2c, horizontal_start=horizontal_start)
        case base_grid.GeometryType.TORUS:
