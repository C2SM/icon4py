...
def test_compute_geofac_n2s(
    ...
    geofac_n2s = functools.partial(compute_geofac_n2s, array_ns=xp, exchange=dummy_exchange)(
...
def test_compute_geofac_grdiv(
    ...
    horizontal_start = icon_grid.start_index(edge_domain(h_grid.Zone.LATERAL_BOUNDARY_LEVEL_2))
    geofac_grdiv = functools.partial(compute_geofac_grdiv, array_ns=xp, exchange=dummy_exchange)(
        geofac_div.ndarray,
        inv_dual_edge_length.ndarray,
        owner_mask.ndarray,
        c2e,
...
def test_compute_e_flx_avg(
    ...
    e_flx_avg = functools.partial(compute_e_flx_avg, array_ns=xp, exchange=dummy_exchange)(
        c_bln_avg,
        geofac_div,
        owner_mask,
...
def test_compute_cells_aw_verts(
    ...
    cells_aw_verts = functools.partial(
        compute_cells_aw_verts, array_ns=xp, exchange=dummy_exchange
    )(
        dual_area=dual_area,
