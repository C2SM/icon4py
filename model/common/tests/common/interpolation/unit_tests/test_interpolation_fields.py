...
def test_compute_c_lin_e(
    ...
    func = functools.partial(
        compute_c_lin_e, array_ns=xp, exchange=exchange_utils.noop_exchange
    )
...
def test_compute_geofac_n2s(
    ...
    geofac_n2s = functools.partial(
        compute_geofac_n2s, array_ns=xp, exchange=exchange_utils.noop_exchange
    )(
...
def test_compute_geofac_grg(
    ...
    geofac_grg_0, geofac_grg_1 = functools.partial(
        compute_geofac_grg, array_ns=xp, exchange=exchange_utils.noop_exchange
    )(
...
def test_compute_geofac_grdiv(
    ...
    geofac_grdiv = functools.partial(
        compute_geofac_grdiv, array_ns=xp, exchange=exchange_utils.noop_exchange
    )(
        geofac_div.ndarray,
        inv_dual_edge_length.ndarray,
        owner_mask.ndarray,
...
def test_compute_e_flx_avg(
    ...
    e_flx_avg = functools.partial(
        compute_e_flx_avg, array_ns=xp, exchange=exchange_utils.noop_exchange
    )(
        c_bln_avg,
        geofac_div,
        owner_mask,
...
def test_compute_cells_aw_verts(
    ...
    cells_aw_verts = functools.partial(
        compute_cells_aw_verts, array_ns=xp, exchange=exchange_utils.noop_exchange
    )(
        dual_area=dual_area,
