...
def _experiment_icon_graupel_scan(
    state_kup: tuple[
        vpfloat,
        vpfloat,
        vpfloat,
    ...
    # Update variables and add qi to qrs for water loading
    qi = maximum(vpfloat("0.0"), (rhoqi_intermediate * c1orho + cqit * dt) * cimi) if do_ice_sedimentation else maximum(
        vpfloat("0.0"), qi + cqit * dt)
    qr = maximum(vpfloat("0.0"), (rhoqr_intermediate * c1orho + cqrt * dt) * cimr)
    qs = maximum(vpfloat("0.0"), (rhoqs_intermediate * c1orho + cqst * dt) * cims)
    qg = maximum(vpfloat("0.0"), (rhoqg_intermediate * c1orho + cqgt * dt) * cimg)
...
def experiment_icon_graupel(
    ...
    qg: gtx.Field[[CellDim, KDim], vpfloat],
    qnc: gtx.Field[[CellDim, KDim], vpfloat],  # originally 2D Field, now 3D Field
    temperature_: gtx.Field[[CellDim, KDim], vpfloat],
    qv_: gtx.Field[[CellDim, KDim], vpfloat],
    qc_: gtx.Field[[CellDim, KDim], vpfloat],
    qi_: gtx.Field[[CellDim, KDim], vpfloat],
    qr_: gtx.Field[[CellDim, KDim], vpfloat],
    qs_: gtx.Field[[CellDim, KDim], vpfloat],
    qg_: gtx.Field[[CellDim, KDim], vpfloat],
    rhoqrv_old_kup: gtx.Field[[CellDim, KDim], vpfloat],
    rhoqsv_old_kup: gtx.Field[[CellDim, KDim], vpfloat],
    rhoqgv_old_kup: gtx.Field[[CellDim, KDim], vpfloat],
    rhoqiv_old_kup: gtx.Field[[CellDim, KDim], vpfloat],
    ...
    _experiment_icon_graupel_scan(
        ...
        domain={
            CellDim: (horizontal_start, horizontal_start),
            KDim: (vertical_start, vertical_end),
        }
    )
