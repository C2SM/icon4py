...
class SingleMomentSixClassIconGraupel:
    ...
    def _initialize_local_fields(self):
        ...
        self.qi_tendency = field_alloc.allocate_zero_field(CellDim, KDim, grid=self.grid, dtype=vpfloat)
        self.qr_tendency = field_alloc.allocate_zero_field(CellDim, KDim, grid=self.grid, dtype=vpfloat)
        self.qs_tendency = field_alloc.allocate_zero_field(CellDim, KDim, grid=self.grid, dtype=vpfloat)
        self.qg_tendency = field_alloc.allocate_zero_field(CellDim, KDim, grid=self.grid, dtype=vpfloat)
        self.rhoqrv_old_kup = field_alloc.allocate_zero_field(CellDim, KDim, grid=self.grid, dtype=vpfloat)
        self.rhoqsv_old_kup = field_alloc.allocate_zero_field(CellDim, KDim, grid=self.grid, dtype=vpfloat)
        self.rhoqgv_old_kup = field_alloc.allocate_zero_field(CellDim, KDim, grid=self.grid, dtype=vpfloat)
        self.rhoqiv_old_kup = field_alloc.allocate_zero_field(CellDim, KDim, grid=self.grid, dtype=vpfloat)
        self.vnew_r = field_alloc.allocate_zero_field(CellDim, KDim, grid=self.grid, dtype=vpfloat)
        self.vnew_s = field_alloc.allocate_zero_field(CellDim, KDim, grid=self.grid, dtype=vpfloat)
        self.vnew_g = field_alloc.allocate_zero_field(CellDim, KDim, grid=self.grid, dtype=vpfloat)
        self.vnew_i = field_alloc.allocate_zero_field(CellDim, KDim, grid=self.grid, dtype=vpfloat)
        self.rain_precipitation_flux = field_alloc.allocate_zero_field(CellDim, KDim, grid=self.grid, dtype=vpfloat)
        self.snow_precipitation_flux = field_alloc.allocate_zero_field(CellDim, KDim, grid=self.grid, dtype=vpfloat)
        self.graupel_precipitation_flux = field_alloc.allocate_zero_field(CellDim, KDim, grid=self.grid, dtype=vpfloat)
        self.ice_precipitation_flux = field_alloc.allocate_zero_field(CellDim, KDim, grid=self.grid, dtype=vpfloat)
        self.total_precipitation_flux = field_alloc.allocate_zero_field(CellDim, KDim, grid=self.grid, dtype=vpfloat)
...
def _icon_graupel_flux_ground(
    ...
    ice_flux = 0.5 * ((qi + qi_tendency) * rho * vnew_i + rhoqiv_old_kup) if do_ice_sedimentation else zero
...
def _icon_graupel_flux_above_ground(
    ...
    ice_flux = (qi + qi_tendency) * rho * vnew_i if do_ice_sedimentation else zero
    ...
    total_flux_ = rain_flux + snow_flux + graupel_flux + ice_flux if do_ice_sedimentation else rain_flux + snow_flux + graupel_flux
...
@scan_operator(
    ...
    forward=True,
    init=(
        vpfloat("0.0"),  # temperature tendency
        vpfloat("0.0"),  # qv tendency
        vpfloat("0.0"),  # qc tendency
        vpfloat("0.0"),  # qi tendency
        vpfloat("0.0"),  # qr tendency
        vpfloat("0.0"),  # qs tendency
        vpfloat("0.0"),  # qg tendency
        vpfloat("0.0"),  # qv
        vpfloat("0.0"),  # qc
        vpfloat("0.0"),  # qi
        vpfloat("0.0"),  # qr
...
def _icon_graupel_scan(
    ...
    qv_kup = qv_old_kup + qv_tendency_kup
    qc_kup = qc_old_kup + qc_tendency_kup
    qi_kup = qi_old_kup + qi_tendency_kup
    qr_kup = qr_old_kup + qr_tendency_kup
    qs_kup = qs_old_kup + qs_tendency_kup
    qg_kup = qg_old_kup + qg_tendency_kup
    ...
    if k_lev > 0:
        vnew_s = snow_sed0_kup * exp(icon_graupel_params.ccswxp * log(
            (qs_kup + qs) * 0.5 * rho_kup)) * crho1o2_kup if qs_kup + qs > icon_graupel_params.qmin else vpfloat(
            "0.0")
        vnew_r = rain_v0 * exp(rain_exp_v * log(
            (qr_kup + qr) * 0.5 * rho_kup)) * crho1o2_kup if qr_kup + qr > icon_graupel_params.qmin else vpfloat(
            "0.0")
        vnew_g = icon_graupel_params.graupel_sed0 * exp(icon_graupel_params.graupel_exp_sed * log(
            (qg_kup + qg) * 0.5 * rho_kup)) * crho1o2_kup if qg_kup + qg > icon_graupel_params.qmin else vpfloat(
            "0.0")
        vnew_i = ice_v0 * exp(icon_graupel_params.ice_exp_v * log(
            (qi_kup + qi) * 0.5 * rho_kup)) * crhofac_qi_kup if qi_kup + qi > icon_graupel_params.qmin else vpfloat(
            "0.0")
    ...
    if ((temperature < icon_graupel_params.heterogeneous_freeze_temperature) & (qv > 8.e-6) & (qi <= vpfloat("0.0")) & (
        qv > qvsi)):
    ...
    if (llqc & (temperature > icon_graupel_params.homogeneous_freeze_temperature)):
        ...
        if liquid_autoconversion_option == 1:
            ...
            scaut_c2r = icon_graupel_params.kessler_cloud2rain_autoconversion_coeff_for_cloud * maximum(
                qc - icon_graupel_params.qc0, vpfloat("0.0"))
        ...
        if temperature > icon_graupel_params.homogeneous_freeze_temperature:
            ...
            if llqr & (temperature < icon_graupel_params.threshold_freeze_temperature) & (qr > 0.1 * qc):
                if rain_freezing_option == 1:
                    ...
                    srfrz_r2g = icon_graupel_params.coeff_rain_freeze_mode2 * local_tfrzdiff * sqrt(
                        local_tfrzdiff) * celn27o16qrk
        else:
        ...
        if temperature > icon_graupel_params.homogeneous_freeze_temperature:
            ...
            if temperature < icon_graupel_params.threshold_freeze_temperature:
                if rain_freezing_option == 1:
                    ...
                    srfrz_r2g = icon_graupel_params.coeff_rain_freeze_mode2 * sqrt(
                        (icon_graupel_params.threshold_freeze_temperature - temperature) ** 3.0) * celn27o16qrk
    ...
    qi_tendency = maximum((rhoqi_intermediate * c1orho * cimi - qi) / dt + cqit, -qi / dt) if do_ice_sedimentation else (
        maximum(cqit, -qi / dt)
    )
...
def icon_graupel(
    ...
    vnew_g: gtx.Field[[CellDim, KDim], vpfloat],
    vnew_i: gtx.Field[[CellDim, KDim], vpfloat],
    #dist_cldtop: gtx.Field[[CellDim, KDim], vpfloat],
    #rho_kup: gtx.Field[[CellDim, KDim], vpfloat],
    #crho1o2_kup: gtx.Field[[CellDim, KDim], vpfloat],
    #crhofac_qi_kup: gtx.Field[[CellDim, KDim], vpfloat],
    #snow_sed0_kup: gtx.Field[[CellDim, KDim], vpfloat],
    #qvsw_kup: gtx.Field[[CellDim, KDim], vpfloat],
