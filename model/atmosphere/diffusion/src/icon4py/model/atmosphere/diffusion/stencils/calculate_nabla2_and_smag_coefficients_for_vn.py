...
def _calculate_nabla2_and_smag_coefficients_for_vn(
    diff_multfac_smag: gtx.Field[gtx.Dims[dims.KDim], ta.vpfloat],
    tangent_orientation: fa.EdgeField[ta.wpfloat],
    inv_primal_edge_length: fa.EdgeField[ta.wpfloat],
    inv_vert_vert_length: fa.EdgeField[ta.wpfloat],
    u_vert: fa.VertexKField[ta.vpfloat],
    v_vert: fa.VertexKField[ta.vpfloat],
    primal_normal_vert_x: gtx.Field[gtx.Dims[dims.ECVDim], ta.wpfloat],
    primal_normal_vert_y: gtx.Field[gtx.Dims[dims.ECVDim], ta.wpfloat],
    dual_normal_vert_x: gtx.Field[gtx.Dims[dims.ECVDim], ta.wpfloat],
    dual_normal_vert_y: gtx.Field[gtx.Dims[dims.ECVDim], ta.wpfloat],
    vn: fa.EdgeKField[ta.wpfloat],
    smag_limit: gtx.Field[gtx.Dims[dims.KDim], ta.vpfloat],
    smag_offset: ta.vpfloat,
) -> tuple[
    ...
    diff_multfac_smag_wp, u_vert_wp, v_vert_wp, smag_offset_wp = astype(
        (diff_multfac_smag, u_vert, v_vert, smag_offset), ta.wpfloat
    )
    ...
    dvt_tang_vp = astype(dvt_tang_wp * tangent_orientation, ta.vpfloat)

    kh_smag_1_wp, dvt_norm_wp = astype((kh_smag_1_vp, dvt_norm_vp), ta.wpfloat)
    ...
    kh_smag_2_wp = astype(kh_smag_2_vp, ta.wpfloat)
    kh_smag_2_wp = (kh_smag_2_wp * inv_vert_vert_length) - (
        astype(dvt_tang_vp, ta.wpfloat) * inv_primal_edge_length
    )
    ...
    z_nabla2_e_wp = ta.wpfloat("4.0") * z_nabla2_e_wp

    kh_smag_ec_wp = kh_smag_e_wp
    kh_smag_e_vp = maximum(ta.vpfloat("0.0"), astype(kh_smag_e_wp - smag_offset_wp, ta.vpfloat))
    kh_smag_e_vp = minimum(kh_smag_e_vp, smag_limit)

    return kh_smag_e_vp, astype(kh_smag_ec_wp, ta.vpfloat), z_nabla2_e_wp
...
def calculate_nabla2_and_smag_coefficients_for_vn(
    ...
    vn: fa.EdgeKField[ta.wpfloat],
    smag_limit: gtx.Field[gtx.Dims[dims.KDim], ta.vpfloat],
    kh_smag_e: fa.EdgeKField[ta.vpfloat],
    kh_smag_ec: fa.EdgeKField[ta.vpfloat],
    z_nabla2_e: fa.EdgeKField[ta.wpfloat],
    smag_offset: ta.vpfloat,
