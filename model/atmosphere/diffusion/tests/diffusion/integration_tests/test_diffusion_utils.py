...
def test_scale_k(backend):
    ...
    field = data_alloc.random_field(grid, KDim, allocator=backend)
    scaled_field = data_alloc.zero_field(grid, KDim, allocator=backend)
    factor = 2.0
...
def test_diff_multfac_vn_and_smag_limit_for_initial_step(backend):
    grid = simple_grid.simple_grid(allocator=backend)
    diff_multfac_vn_init = data_alloc.zero_field(grid, KDim, allocator=backend)
    smag_limit_init = data_alloc.zero_field(grid, KDim, allocator=backend)
    k4 = 1.0
    efdt_ratio = 24.0
...
def test_diff_multfac_vn_smag_limit_for_time_step_with_const_value(backend):
    grid = simple_grid.simple_grid(allocator=backend)
    diff_multfac_vn = data_alloc.zero_field(grid, KDim, allocator=backend)
    smag_limit = data_alloc.zero_field(grid, KDim, allocator=backend)
    k4 = 1.0
    substeps = 5.0
    efdt_ratio = 24.0
