...
def _sum_neighbor_contributions_all(
    ...
    prod_p1 = where(mask1 & js_gt1, p_cc(Koff[1]) * p_cellmass_now(Koff[1]), wpfloat(0.0))
    prod_p2 = where(mask1 & js_gt2, p_cc(Koff[2]) * p_cellmass_now(Koff[2]), wpfloat(0.0))
    prod_p3 = where(mask1 & js_gt3, p_cc(Koff[3]) * p_cellmass_now(Koff[3]), wpfloat(0.0))
    prod_p4 = where(mask1 & js_gt4, p_cc(Koff[4]) * p_cellmass_now(Koff[4]), wpfloat(0.0))
    prod_m0 = where(mask2 & js_gt0, p_cc(Koff[-1]) * p_cellmass_now(Koff[-1]), wpfloat(0.0))
    prod_m1 = where(mask2 & js_gt1, p_cc(Koff[-2]) * p_cellmass_now(Koff[-2]), wpfloat(0.0))
    prod_m2 = where(mask2 & js_gt2, p_cc(Koff[-3]) * p_cellmass_now(Koff[-3]), wpfloat(0.0))
    prod_m3 = where(mask2 & js_gt3, p_cc(Koff[-4]) * p_cellmass_now(Koff[-4]), wpfloat(0.0))
    prod_m4 = where(mask2 & js_gt4, p_cc(Koff[-5]) * p_cellmass_now(Koff[-5]), wpfloat(0.0))
