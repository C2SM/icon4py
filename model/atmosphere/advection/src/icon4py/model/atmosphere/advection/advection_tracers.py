...
def compute_lsq_pseudoinv(cell_owner_mask, lsq_pseudoinv, z_lsq_mat_c, lsq_weights_c, min_rlcell_int, lsq_dim_unk: int, lsq_dim_c: int):
    for jja in range(lsq_dim_unk):
        for jjb in range(lsq_dim_c):
            for jjk in range(lsq_dim_unk):
                for jc in range(850, min_rlcell_int):
                    A = z_lsq_mat_c[jc, :, :]
...
def compute_lsq_moments(lsq_moments, xloc, yloc, vertex_lon, vertex_lat, jlv, lsq_dim_unk, nverts, min_rlcell_int):
    for jc in range(min_rlcell_int):
        ...
        lsq_moments[jc, 0] = z_rcarea / 6. * np.dot(fx, dely)
        lsq_moments[jc, 1] = -z_rcarea / 6. * np.dot(fy, delx)
...
def lsq_compute_coeff_cell_sphere(cell_lat, cell_lon, vertex_lat, vertex_lon, c2e2c, c2v, cell_owner_mask, grid_sphere_radius, lsq_dim_unk, lsq_dim_c, lsq_wgt_exp, min_rlcell_int):
    ...
    # zu = np.zeros((lsq_dim_c, lsq_dim_unk, min_rlcell_int,))
    # zs = np.zeros((lsq_dim_unk, min_rlcell_int,))
    # zv_t = np.zeros((lsq_dim_unk, lsq_dim_unk, min_rlcell_int,))
    # icheck = np.zeros((min_rlcell_int,))
    ...
    for jc in range(850, min_rlcell_int): # change 850 to the actual right value
        xytemp_c = np.zeros((lsq_dim_stencil, 2))
        ilc_s = c2e2c[jc, :lsq_dim_stencil]
        for js in range(lsq_dim_stencil):
            xytemp_c[js, 0] = cell_lon[ilc_s[js]].ndarray # geometry_attrs.CELL_LON[ilc_s[js]]
            xytemp_c[js, 1] = cell_lat[ilc_s[js]].ndarray # geometry_attrs.CELL_LAT[ilc_s[js]]

        xytemp_v = np.zeros((nverts, 2))
        jlv = c2v[jc, :nverts]
        for jec in range(nverts):
            xytemp_v[jec, 0] = vertex_lon[jlv[jec]].ndarray # geometry_attrs.VERTEX_LON[jlv[jec]]
            xytemp_v[jec, 1] = vertex_lat[jlv[jec]].ndarray # geometry_attrs.VERTEX_LAT[jlv[jec]]
        ...
        for js in range(lsq_dim_unk):
            for ju in range(lsq_dim_unk):
                z_lsq_mat_c[jc, js, ju] = lsq_weights_c[jc, js] *(lsq_moments_hat[jc, js, ju] - lsq_moments[jc, ju])
            if lsq_dim_stencil < lsq_dim_c: # check ptr_ncells(jc,jb) = lsq_dim_stencil
                z_lsq_mat_c[jc, js, ju] = 0.
    #zs(lsq_dim_unk, nproma)
    #zu(lsq_dim_c, lsq_dim_c, nproma)
    #zv_t(lsq_dim_unk, lsq_dim_unk, nproma)
    # for jc in range(min_rlcell_int):
    #     A = z_lsq_mat_c[jc, :, :]
    #     zu[:, :, jc], zs[:, jc], zv_t[:, : , jc], icheck = scipy.linalg.lapack.dgesdd(A)

    lsq_pseudoinv = compute_lsq_pseudoinv(cell_owner_mask, lsq_pseudoinv, z_lsq_mat_c, lsq_weights_c, min_rlcell_int, lsq_dim_unk, lsq_dim_c)
...
def lsq_compute_coeff_cell_torus(cell_center_x, cell_center_y, vertex_center_x, vertex_center_y, c2e2c, c2v, cell_owner_mask, domain_length, domain_height, lsq_dim_unk, lsq_dim_c, lsq_wgt_exp, min_rlcell_int):
    jc_range = range(1, min_rlcell_int)
    nverts = 3
    lsq_dim_stencil = 3  # change to c2e[1], which should be 3 #ccells % num_edges(jc_range)
    z_dist_g = np.zeros((min_rlcell_int, lsq_dim_stencil, 2))
    lsq_pseudoinv = np.zeros((min_rlcell_int, lsq_dim_unk, lsq_dim_c))
    lsq_weights_c = np.zeros((min_rlcell_int, lsq_dim_stencil))
    jlv = c2v[jc_range, :nverts]
    for jc in range(1, min_rlcell_int):
        ilc_s = c2e2c[jc, :lsq_dim_stencil]

        cc_cell = np.zeros((lsq_dim_stencil,2))
        cc_cv_x = (cell_center_x.ndarray[jc], cell_center_y.ndarray[jc])
        for js in range(lsq_dim_stencil):
            cc_cell[js] = np.asarray([cell_center_x.ndarray[ilc_s[js]], cell_center_y.ndarray[ilc_s[js]]])

        # cc_vert = np.zeros((nverts,2))
        # for jec in range(nverts):
        #     cc_vert[jec] = np.asarray([vertex_center_x.ndarray[jlv[jec]], vertex_center_y.ndarray[jlv[jec]]])

        for js in range(lsq_dim_stencil):
            cc_cell[js] = np.asarray(plane_torus_closest_coordinates(cc_cv_x, cc_cell[js], domain_length, domain_height))
            z_dist_g[jc,js,0] = cc_cell[js][0] - cc_cv_x[0]
            z_dist_g[jc,js,1] = cc_cell[js][1] - cc_cv_x[1]

        lsq_weights_c = compute_lsq_weights_c(z_dist_g, lsq_weights_c, jc, lsq_dim_stencil, lsq_wgt_exp)
        lsq_weights_c = lsq_weights_c / np.max(lsq_weights_c)

        z_lsq_mat_c = np.zeros((lsq_dim_c, lsq_dim_c))
        for js in range(min(lsq_dim_unk, lsq_dim_c)):
            z_lsq_mat_c[js, js] = np.where(cell_owner_mask[jc], 1.0, z_lsq_mat_c[js, js])

        zu, zs, zv_t, icheck = scipy.linalg.lapack.dgesdd(z_lsq_mat_c)  # , compute_uv=1, full_matrices=True)

        lsq_pseudoinv = compute_lsq_pseudoinv(cell_owner_mask, lsq_pseudoinv, zv_t, zs, zu, lsq_weights_c, jc, lsq_dim_unk, lsq_dim_c)
    return lsq_pseudoinv
