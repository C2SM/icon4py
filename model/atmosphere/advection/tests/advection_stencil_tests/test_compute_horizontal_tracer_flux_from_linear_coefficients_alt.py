...
class TestComputeHorizontalTracerFluxFromLinearCoefficientsAlt(helpers.StencilTest):
    ...
    def reference(
        ...
        **kwargs,
        ...
        lvn_pos_inv = xp.where(p_vn > 0.0, False, True)

        p_out_e = (
            xp.where(lvn_pos_inv, z_lsq_coeff_1[e2c][:, 1], z_lsq_coeff_1[e2c][:, 0])
            + distv_bary_1
            * xp.where(lvn_pos_inv, z_lsq_coeff_2[e2c][:, 1], z_lsq_coeff_2[e2c][:, 0])
            + distv_bary_2
            * xp.where(lvn_pos_inv, z_lsq_coeff_3[e2c][:, 1], z_lsq_coeff_3[e2c][:, 0])
        ) * p_mass_flx_e
