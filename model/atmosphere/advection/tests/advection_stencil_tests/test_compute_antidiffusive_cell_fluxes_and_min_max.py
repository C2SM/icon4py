...
class TestComputeAntidiffusiveCellFluxesAndMinMax(helpers.StencilTest):
    ...
    def input_data(self, grid) -> dict:
        geofac_div = data_allocation.random_field(grid, dims.CellDim, dims.C2EDim)
        geofac_div_new = data_allocation.as_1D_sparse_field(geofac_div, dims.CEDim)
        p_rhodz_now = data_allocation.random_field(grid, dims.CellDim, dims.KDim)
        p_rhodz_new = data_allocation.random_field(grid, dims.CellDim, dims.KDim)
        z_mflx_low = data_allocation.random_field(grid, dims.EdgeDim, dims.KDim)
        z_anti = data_allocation.random_field(grid, dims.EdgeDim, dims.KDim)
        p_cc = data_allocation.random_field(grid, dims.CellDim, dims.KDim)
        p_dtime = 5.0

        z_mflx_anti_in = data_allocation.zero_field(grid, dims.CellDim, dims.KDim)
        z_mflx_anti_out = data_allocation.zero_field(grid, dims.CellDim, dims.KDim)
        z_tracer_new_low = data_allocation.zero_field(grid, dims.CellDim, dims.KDim)
        z_tracer_max = data_allocation.zero_field(grid, dims.CellDim, dims.KDim)
        z_tracer_min = data_allocation.zero_field(grid, dims.CellDim, dims.KDim)
