...
class TestComputeFfslBacktrajectory(helpers.StencilTest):
    ...
    def reference(
        ...
        **kwargs,
        ...
        primal_normal_cell_x = xp.expand_dims(xp.asarray(primal_normal_cell_x), axis=-1)
        dual_normal_cell_x = xp.expand_dims(xp.asarray(dual_normal_cell_x), axis=-1)
        primal_normal_cell_y = xp.expand_dims(xp.asarray(primal_normal_cell_y), axis=-1)
        dual_normal_cell_y = xp.expand_dims(xp.asarray(dual_normal_cell_y), axis=-1)
        edge_verts_1_x = xp.expand_dims(xp.asarray(edge_verts_1_x), axis=-1)
        edge_verts_1_y = xp.expand_dims(xp.asarray(edge_verts_1_y), axis=-1)
        edge_verts_2_x = xp.expand_dims(xp.asarray(edge_verts_2_x), axis=-1)
        edge_verts_2_y = xp.expand_dims(xp.asarray(edge_verts_2_y), axis=-1)
        pos_on_tplane_e_1_x = xp.expand_dims(xp.asarray(pos_on_tplane_e_1_x), axis=-1)
        pos_on_tplane_e_1_y = xp.expand_dims(xp.asarray(pos_on_tplane_e_1_y), axis=-1)
        pos_on_tplane_e_2_x = xp.expand_dims(xp.asarray(pos_on_tplane_e_2_x), axis=-1)
        pos_on_tplane_e_2_y = xp.expand_dims(xp.asarray(pos_on_tplane_e_2_y), axis=-1)
