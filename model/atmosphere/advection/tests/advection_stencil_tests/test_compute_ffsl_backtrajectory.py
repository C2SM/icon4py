...
class TestComputeFfslBacktrajectory(helpers.StencilTest):
    ...
    def input_data(self, grid) -> dict:
        p_vn = data_allocation.random_field(grid, dims.EdgeDim, dims.KDim)
        p_vt = data_allocation.random_field(grid, dims.EdgeDim, dims.KDim)
        cell_idx = np.asarray(grid.connectivities[dims.E2CDim], dtype=gtx.int32)
        cell_idx_new = data_allocation.numpy_to_1D_sparse_field(cell_idx, dims.ECDim)
        cell_blk = data_allocation.constant_field(grid, 1, dims.EdgeDim, dims.E2CDim, dtype=gtx.int32)
        cell_blk_new = data_allocation.as_1D_sparse_field(cell_blk, dims.ECDim)

        edge_verts_1_x = data_allocation.random_field(grid, dims.EdgeDim)
        edge_verts_2_x = data_allocation.random_field(grid, dims.EdgeDim)
        edge_verts_1_y = data_allocation.random_field(grid, dims.EdgeDim)
        edge_verts_2_y = data_allocation.random_field(grid, dims.EdgeDim)
        pos_on_tplane_e_1_x = data_allocation.random_field(grid, dims.EdgeDim)
        pos_on_tplane_e_2_x = data_allocation.random_field(grid, dims.EdgeDim)
        pos_on_tplane_e_1_y = data_allocation.random_field(grid, dims.EdgeDim)
        pos_on_tplane_e_2_y = data_allocation.random_field(grid, dims.EdgeDim)
        primal_normal_cell_x = data_allocation.random_field(grid, dims.EdgeDim, dims.E2CDim)
        primal_normal_cell_x_new = data_allocation.as_1D_sparse_field(primal_normal_cell_x, dims.ECDim)
        dual_normal_cell_x = data_allocation.random_field(grid, dims.EdgeDim, dims.E2CDim)
        dual_normal_cell_x_new = data_allocation.as_1D_sparse_field(dual_normal_cell_x, dims.ECDim)
        primal_normal_cell_y = data_allocation.random_field(grid, dims.EdgeDim, dims.E2CDim)
        primal_normal_cell_y_new = data_allocation.as_1D_sparse_field(primal_normal_cell_y, dims.ECDim)
        dual_normal_cell_y = data_allocation.random_field(grid, dims.EdgeDim, dims.E2CDim)
        dual_normal_cell_y_new = data_allocation.as_1D_sparse_field(dual_normal_cell_y, dims.ECDim)
        lvn_sys_pos = data_allocation.constant_field(grid, True, dims.EdgeDim, dims.KDim, dtype=bool)
        p_dt = 2.0
        p_cell_idx = data_allocation.constant_field(grid, 0, dims.EdgeDim, dims.KDim, dtype=gtx.int32)
        p_cell_rel_idx_dsl = data_allocation.constant_field(
            grid, 0, dims.EdgeDim, dims.KDim, dtype=gtx.int32
        )
        p_cell_blk = data_allocation.constant_field(grid, 0, dims.EdgeDim, dims.KDim, dtype=gtx.int32)
        p_coords_dreg_v_1_lon_dsl = data_allocation.random_field(grid, dims.EdgeDim, dims.KDim)
        p_coords_dreg_v_2_lon_dsl = data_allocation.random_field(grid, dims.EdgeDim, dims.KDim)
        p_coords_dreg_v_3_lon_dsl = data_allocation.random_field(grid, dims.EdgeDim, dims.KDim)
        p_coords_dreg_v_4_lon_dsl = data_allocation.random_field(grid, dims.EdgeDim, dims.KDim)
        p_coords_dreg_v_1_lat_dsl = data_allocation.random_field(grid, dims.EdgeDim, dims.KDim)
        p_coords_dreg_v_2_lat_dsl = data_allocation.random_field(grid, dims.EdgeDim, dims.KDim)
        p_coords_dreg_v_3_lat_dsl = data_allocation.random_field(grid, dims.EdgeDim, dims.KDim)
        p_coords_dreg_v_4_lat_dsl = data_allocation.random_field(grid, dims.EdgeDim, dims.KDim)
