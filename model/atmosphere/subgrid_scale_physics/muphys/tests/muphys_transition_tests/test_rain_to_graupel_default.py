...
class TestRainToGraupelDefault(StencilTest):
    ...
    def input_data(self, grid):
        return dict(
            t       = constant_field(grid, 272.731, dims.CellDim, dtype=wpfloat),
            rho     = constant_field(grid, 1.12442, dims.CellDim, dtype=wpfloat),
            qc      = constant_field(grid, 0.0, dims.CellDim, dtype=wpfloat),
            qr      = constant_field(grid, 1.34006e-17, dims.CellDim, dtype=wpfloat),
            qi      = constant_field(grid, 2.02422e-23, dims.CellDim, dtype=wpfloat),
            qs      = constant_field(grid, 1.02627e-19, dims.CellDim, dtype=wpfloat),
            mi      = constant_field(grid, 1.0e-12, dims.CellDim, dtype=wpfloat),
            dvsw    = constant_field(grid, -0.000635669, dims.CellDim, dtype=wpfloat),
            dt      = 30.0,
