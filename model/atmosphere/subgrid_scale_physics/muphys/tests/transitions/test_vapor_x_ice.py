...
class TestVaporXIceDefault(StencilTest):
    ...
    def input_data(self, grid):
        ...
        return dict(
            qi               = constant_field(grid, 9.53048e-07, dims.CellDim, dims.KDim, dtype=wpfloat),
            mi               = constant_field(grid, 1.0e-9, dims.CellDim, dims.KDim, dtype=wpfloat),
            eta              = constant_field(grid, 1.90278e-05, dims.CellDim, dims.KDim, dtype=wpfloat),
            dvsi             = constant_field(grid, 0.000120375, dims.CellDim, dims.KDim, dtype=wpfloat),
