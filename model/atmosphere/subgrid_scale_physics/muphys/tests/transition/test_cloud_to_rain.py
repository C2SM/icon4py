...
from icon4py.model.atmosphere.subgrid_scale_physics.muphys.core.transitions.cloud_to_rain import cloud_to_rain
...
class TestCloudToRain(StencilTest):
    ...
    def input_data(self, grid):
        return dict(
            t                   = constant_field(grid, 267.25, dims.CellDim, dtype=wpfloat),
            qc                  = constant_field(grid, 5.52921e-05, dims.CellDim, dtype=wpfloat),
            qr                  = constant_field(grid, 2.01511e-12, dims.CellDim, dtype=wpfloat),
            nc                  = constant_field(grid, 100.0, dims.CellDim, dtype=wpfloat),
