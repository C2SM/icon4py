...
from gt4py.next import astype, exp, log, maximum, minimum, power, sqrt, where
...
def _cloud_to_rain(
    ...
    tau = maximum(
        TAU_MIN, minimum(wpfloat(1.0) - qc / (qc + qr), TAU_MAX)
    )  # temporary cannot go in where
    phi = power(tau, B_PHI)
...
def _cloud_to_snow(
    ...
    C_RIM = wpfloat(2.61) * ECS * GraupelConsts.v0s
    return where(
