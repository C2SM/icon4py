...
def _sum_neighbor_contributions(
    ...
    p_cc_p1 = where(mask1 & js_eq1, p_cc(dims.KDim + 1), wpfloat(0.0))
    p_cc_p2 = where(mask1 & js_eq2, p_cc(dims.KDim + 2), wpfloat(0.0))
    p_cc_p3 = where(mask1 & js_eq3, p_cc(dims.KDim + 3), wpfloat(0.0))
    p_cc_p4 = where(mask1 & js_eq4, p_cc(dims.KDim + 4), wpfloat(0.0))
    p_cc_m0 = where(mask2 & js_eq0, p_cc(dims.KDim - 1), wpfloat(0.0))
    p_cc_m1 = where(mask2 & js_eq1, p_cc(dims.KDim - 2), wpfloat(0.0))
    p_cc_m2 = where(mask2 & js_eq2, p_cc(dims.KDim - 3), wpfloat(0.0))
    p_cc_m3 = where(mask2 & js_eq3, p_cc(dims.KDim - 4), wpfloat(0.0))
    p_cc_m4 = where(mask2 & js_eq4, p_cc(dims.KDim - 5), wpfloat(0.0))
