...
class TestReconstructCubicCoefficientsSvd(stencil_tests.StencilTest):
    ...
    def input_data(self, grid: base.Grid) -> dict:
        ...
        p_coeff_1_dsl = data_alloc.zero_field(grid, dims.CellDim, KDim)
        p_coeff_2_dsl = data_alloc.zero_field(grid, dims.CellDim, KDim)
        p_coeff_3_dsl = data_alloc.zero_field(grid, dims.CellDim, KDim)
        p_coeff_4_dsl = data_alloc.zero_field(grid, dims.CellDim, KDim)
        p_coeff_5_dsl = data_alloc.zero_field(grid, dims.CellDim, KDim)
        p_coeff_6_dsl = data_alloc.zero_field(grid, dims.CellDim, KDim)
        p_coeff_7_dsl = data_alloc.zero_field(grid, dims.CellDim, KDim)
        p_coeff_8_dsl = data_alloc.zero_field(grid, dims.CellDim, KDim)
        p_coeff_9_dsl = data_alloc.zero_field(grid, dims.CellDim, KDim)
        p_coeff_10_dsl = data_alloc.zero_field(grid, dims.CellDim, KDim)
