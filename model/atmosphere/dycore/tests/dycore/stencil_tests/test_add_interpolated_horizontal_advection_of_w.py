...
class TestAddInterpolatedHorizontalAdvectionOfW(stencil_tests.StencilTest):
    ...
    def input_data(self, grid: base.Grid) -> dict[str, gtx.Field | state_utils.ScalarType]:
        z_v_grad_w = data_alloc.random_field(grid, dims.EdgeDim, dims.KHalfDim, dtype=ta.vpfloat)
        e_bln_c_s = data_alloc.random_field(grid, dims.CellDim, dims.C2EDim, dtype=ta.wpfloat)
        ddt_w_adv = data_alloc.random_field(grid, dims.CellDim, dims.KHalfDim, dtype=ta.vpfloat)
