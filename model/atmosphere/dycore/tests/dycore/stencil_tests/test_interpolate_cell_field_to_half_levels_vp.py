...
class TestInterpolateToHalfLevelsVp(stencil_tests.StencilTest):
    ...
    def input_data(self, grid: base.Grid) -> dict[str, gtx.Field | state_utils.ScalarType]:
        interpolant = random_field(grid, dims.CellDim, KDim, dtype=ta.vpfloat)
        wgtfac_c = random_field(grid, dims.CellDim, KDim, dtype=ta.vpfloat)
        interpolation_to_half_levels_vp = zero_field(grid, dims.CellDim, KDim, dtype=ta.vpfloat)
