...
class TestComputeExplicitPartForRhoAndExner(StencilTest):
    ...
    def input_data(self, grid: base.Grid) -> dict[str, gtx.Field | state_utils.ScalarType]:
        ...
        z_flxdiv_mass = data_alloc.random_field(grid, dims.CellDim, KDim, dtype=ta.vpfloat)
        ...
        z_flxdiv_theta = data_alloc.random_field(grid, dims.CellDim, KDim, dtype=ta.vpfloat)
        theta_v_ic = data_alloc.random_field(
            grid, dims.CellDim, KDim, extend={KDim: 1}, dtype=ta.wpfloat
        )
        ddt_exner_phy = data_alloc.random_field(grid, dims.CellDim, KDim, dtype=ta.vpfloat)

        z_rho_expl = data_alloc.zero_field(grid, dims.CellDim, KDim, dtype=ta.wpfloat)
        z_exner_expl = data_alloc.zero_field(grid, dims.CellDim, KDim, dtype=ta.wpfloat)
