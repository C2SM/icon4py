...
class TestComputeHydrostaticCorrectionTerm(StencilTest):
    ...
    def input_data(self, grid: base.Grid) -> dict[str, gtx.Field | state_utils.ScalarType]:
        ...
        theta_v = data_alloc.random_field(grid, dims.CellDim, KDim, dtype=ta.wpfloat)
        zdiff_gradp = data_alloc.random_field(
            grid, dims.EdgeDim, dims.E2CDim, KDim, dtype=ta.vpfloat
        )
        ...
        z_hydro_corr = data_alloc.zero_field(grid, dims.EdgeDim, KDim, dtype=ta.vpfloat)

        z_hydro_corr = gtx.constructors.zeros(
            domain={
                dims.EdgeDim: (0, grid.num_edges),
                KDim: (grid.num_levels - 1, grid.num_levels),
            },
            dtype=ta.vpfloat,
        )
