...
class TestComputeExplicitVerticalWindFromAdvectionAndVerticalWindDensity(StencilTest):
    ...
    def input_data(self, grid: base.Grid) -> dict[str, gtx.Field | state_utils.ScalarType]:
        w_nnow = data_alloc.random_field(grid, dims.CellDim, dims.KHalfDim, dtype=ta.wpfloat)
        ddt_w_adv_ntl1 = data_alloc.random_field(
            grid, dims.CellDim, dims.KHalfDim, dtype=ta.vpfloat
        )
        ddt_w_adv_ntl2 = data_alloc.random_field(
            grid, dims.CellDim, dims.KHalfDim, dtype=ta.vpfloat
        )
        z_th_ddz_exner_c = data_alloc.random_field(
            grid, dims.CellDim, dims.KHalfDim, dtype=ta.vpfloat
        )
        z_w_expl = data_alloc.zero_field(grid, dims.CellDim, dims.KHalfDim, dtype=ta.wpfloat)
        ...
        z_contr_w_fl_l = data_alloc.zero_field(grid, dims.CellDim, dims.KHalfDim, dtype=ta.wpfloat)
