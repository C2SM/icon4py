...
def test_solve_nonhydro_benchmark(
    ...
    diagnostic_state_nh = dycore_states.DiagnosticStateNonHydro(
        ...
        normal_wind_advective_tendency=common_utils.PredictorCorrectorPair(
            data_alloc.zero_field(mesh, dims.EdgeDim, dims.KDim, backend=backend), data_alloc.zero_field(mesh, dims.EdgeDim, dims.KDim, backend=backend)
        ),
        vertical_wind_advective_tendency=common_utils.PredictorCorrectorPair(
            data_alloc.zero_field(mesh, dims.CellDim, dims.KDim, backend=backend), data_alloc.zero_field(mesh, dims.CellDim, dims.KDim, backend=backend)
        ), # TODO (Yilu)
    ...
    solve_nonhydro = solve_nh.SolveNonhydro(
        ...
        owner_mask=DecompositionInfo.owner_mask(dims.EdgeDim),
        backend=backend,
    )
