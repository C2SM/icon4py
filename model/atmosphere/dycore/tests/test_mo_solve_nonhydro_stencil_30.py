...
class TestMoSolveNonhydroStencil30(StencilTest):
    ...
    def reference(
        ...
        **kwargs,
        ...
        z_vn_avg = np.sum(np.where((mesh.e2c2eO != -1)[:, :, np.newaxis], vn[mesh.e2c2eO] * e_flx_avg, 0), axis=1)
        geofac_grdiv = np.expand_dims(geofac_grdiv, axis=-1)
        z_graddiv_vn = np.sum(np.where((mesh.e2c2eO != -1)[:, :, np.newaxis], vn[mesh.e2c2eO] * geofac_grdiv, 0), axis=1)
