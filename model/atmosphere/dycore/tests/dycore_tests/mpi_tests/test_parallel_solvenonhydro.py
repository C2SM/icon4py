...
@pytest.mark.parametrize(
    "istep_init, jstep_exit, step_date_init,istep_exit, jstep_exit, step_date_exit",
...
def test_run_solve_nonhydro_single_step(
    ...
    print(f"rank={processor_props.rank}/{processor_props.comm_size}: starting verification")
    c_halo_start = icon_grid.get_start_index(CellDim, HorizontalMarkerIndex.halo(CellDim))
    print(
        f"rank={processor_props.rank}/{processor_props.comm_size}: halo (cells) start: {c_halo_start} "
    )
    assert dallclose(
        np.asarray(sp_step_exit.theta_v_new())[decomposition_info.owner_mask(CellDim), :],
        np.asarray(prognostic_state_nnew.theta_v)[decomposition_info.owner_mask(CellDim), :],
    )

    assert dallclose(
        np.asarray(sp_step_exit.exner_new())[decomposition_info.owner_mask(CellDim), :],
