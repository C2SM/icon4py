...
class TestMoMathGradientsGradGreenGaussCellDsl(StencilTest):
    ...
    def reference(
        ...
        **kwargs,
        ...
        p_grad_1_u = np.sum(geofac_grg_x * p_ccpr1[grid.connectivities[C2E2CODim]], axis=1)

        geofac_grg_y = np.expand_dims(geofac_grg_y, axis=-1)
        p_grad_1_v = np.sum(geofac_grg_y * p_ccpr1[grid.connectivities[C2E2CODim]], axis=1)

        p_grad_2_u = np.sum(geofac_grg_x * p_ccpr2[grid.connectivities[C2E2CODim]], axis=1)

        p_grad_2_v = np.sum(geofac_grg_y * p_ccpr2[grid.connectivities[C2E2CODim]], axis=1)
        return dict(
