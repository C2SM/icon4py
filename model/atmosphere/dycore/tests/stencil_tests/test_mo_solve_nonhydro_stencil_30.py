...
class TestMoSolveNonhydroStencil30(StencilTest):
    ...
    def input_data(self, mesh):
        ...
        geofac_grdiv = random_field(mesh, EdgeDim, E2C2EODim, dtype=wpfloat)
        rbf_vec_coeff_e = random_field(mesh, EdgeDim, E2C2EDim, dtype=wpfloat)
        vn = random_field(mesh, EdgeDim, KDim, dtype=wpfloat)
        z_vn_avg = zero_field(mesh, EdgeDim, KDim, dtype=wpfloat)
        z_graddiv_vn = zero_field(mesh, EdgeDim, KDim, dtype=vpfloat)
        vt = zero_field(mesh, EdgeDim, KDim, dtype=vpfloat)
