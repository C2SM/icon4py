    ...
    def reference(
        ...
        **kwargs,
        ...
        w_con_e = np.where(
            ...
            np.sum(np.where((grid.connectivities[E2CDim] != -1)[:, :, np.newaxis], c_lin_e * z_w_con_c_full[grid.connectivities[E2CDim]], 0), axis=1),
            w_con_e,
        )
        difcoef = np.where(
            ((levelmask_offset_0) | (levelmask_offset_1))
        ...
        ddt_vn_apc = np.where(
            ...
            * area_edge
            * (
                np.sum(np.where((grid.connectivities[E2C2EODim] != -1)[:, :, np.newaxis], geofac_grdiv * vn[grid.connectivities[E2C2EODim]], 0), axis=1)
                + tangent_orientation
