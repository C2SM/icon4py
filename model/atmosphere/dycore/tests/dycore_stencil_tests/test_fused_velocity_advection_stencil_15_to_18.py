...
class TestFusedVelocityAdvectionStencil15To18(StencilTest):
    ...
    def _fused_velocity_advection_stencil_16_to_18(
        ...
        ddt_w_adv = np.where(
            condition1,
            compute_advective_vertical_wind_tendency_numpy(z_w_con_c[:, :-1], w, coeff1_dwdz, coeff2_dwdz),
            ddt_w_adv,
        )

        ddt_w_adv = np.where(
            condition1,
            add_interpolated_horizontal_advection_of_w_numpy(grid, e_bln_c_s, z_v_grad_w, ddt_w_adv),
            ddt_w_adv,
        )
