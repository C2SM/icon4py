...
def _compute_rho_virtual_potential_temperatures_and_pressure_gradient(
    ...
    z_rho_tavg_m1_wp = wgt_nnow_rth * rho_now(dims.KDim - 1) + wgt_nnew_rth * rho_var(dims.KDim - 1)
    z_theta_tavg_m1_wp = wgt_nnow_rth * theta_now(dims.KDim - 1) + wgt_nnew_rth * theta_var(
        dims.KDim - 1
    )
    ...
    z_theta_v_pr_mc_m1_wp = z_theta_tavg_m1_wp - theta_ref_mc_wp(dims.KDim - 1)
