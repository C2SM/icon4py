                            ...
                            str(
                                xp.sum(
                                    xp.sum(
                                        expanded_weight
                                        * expanded_dz
                                        * xp.abs(z_fields.z_flxdiv_vn_and_w.ndarray),
                                        axis=1,
                                    ),
                                    axis=0,
                                )
                ...
                if self.config.do_o2_divdamp:
                    if self.config.do_3d_divergence_damping:
                        ...
                        if do_output:
                            ...
                            expanded_weight = xp.repeat(temporary, expanded_dz.shape[1], axis=1)
                            total_weight = xp.sum(
                                xp.sum(expanded_weight * expanded_dz, axis=1), axis=0
                            )
                            ...
                            for k in range(self.grid.num_levels):
                                temporary3[:, k] = (
                                    xp.sum(temporary1[c2e, k], axis=1)
                                    + temporary2[:, k + 1]
                                    + temporary2[:, k]
                                )
                            ...
                            with open(filename, append_write) as f:
                                ...
                                f.write(
                                    ...
                                    + str(
                                        xp.sum(
                                            xp.sum(
                                                expanded_weight * expanded_dz * xp.abs(temporary3),
                                                axis=1,
                                            ),
                                            axis=0,
                                        )
                            ...
                            for _ in range(self.config.number_of_divdamp_step - 1):
                                ...
                                if do_output:
                                    ...
                                    if os.path.exists(filename):
                                        ...
                                        append_write = "w"  # make a new file if not
                                    expanded_dz = (
                                        1.0 / self.metric_state_nonhydro.inv_ddqz_z_full.ndarray
                                    )
                                    temporary = self.cell_params.area.ndarray
                                    temporary = xp.expand_dims(temporary, axis=-1)
                                    expanded_weight = xp.repeat(
                                        temporary, expanded_dz.shape[1], axis=1
                                    )
                                    total_weight = xp.sum(
                                        xp.sum(expanded_weight * expanded_dz, axis=1), axis=0
                                    )
                                            ...
                                            str(
                                                xp.sum(
                                                    xp.sum(
                                                        ...
                                                        * expanded_dz
                                                        * xp.abs(
                                                            z_fields.z_flxdiv_vn_and_w.ndarray
                                                        ),
                                                        axis=1,
                                                    ),
                                                    axis=0,
