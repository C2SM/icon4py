...
def _fused_solve_nonhydro_stencil_15_to_28_predictor(
    ...
    (zero_lower_bound, zero_upper_bound) = (
        (horizontal_lower_01, horizontal_upper_01)
        if idiv_method == 1
    ...
    z_gradh_exner = where(
        ...
        _mo_solve_nonhydro_stencil_18(inv_dual_edge_length, z_exner_ex_pr),
        z_gradh_exner,
    )
    ...
    z_gradh_exner = (
        where(
            ...
            _mo_solve_nonhydro_stencil_19(
                inv_dual_edge_length, z_exner_ex_pr, ddxn_z_full, c_lin_e, z_dexner_dz_c_1
            ),
            z_gradh_exner,
        )
        if igradp_method == 3
    ...
    z_gradh_exner = (
        where(
            ...
            _mo_solve_nonhydro_stencil_22(
                ipeidx_dsl, pg_exdist, z_hydro_corr_horizontal, z_gradh_exner
            ),
            z_gradh_exner,
        )
        if igradp_method == 3
    ...
    vn = (
        where(
            (horizontal_lower < horz_idx < horizontal_upper) & (int32(0) < vert_idx < nlev),
            _mo_solve_nonhydro_stencil_28(vn_incr, vn, iau_wgt_dyn),
            vn,
        )
        if is_iau_active
        else vn
...
def _fused_solve_nonhydro_stencil_15_to_28_corrector(
    ...
    z_graddiv_vn = where(
        ...
        _mo_solve_nonhydro_stencil_17(
            hmask_dd3d, scalfac_dd3d, inv_dual_edge_length, z_dwdz_dd, z_graddiv_vn
        ),
        z_graddiv_vn,
    )
    ...
    z_graddiv2_vn = where(
        (horizontal_lower < horz_idx < horizontal_upper) & (int32(0) < vert_idx < nlev),
        _mo_solve_nonhydro_stencil_25(geofac_grdiv, z_graddiv_vn),
        z_graddiv2_vn,
    )
    ...
    vn = (
        where(
            (horizontal_lower < horz_idx < horizontal_upper) & (int32(0) < vert_idx < nlev),
            _mo_solve_nonhydro_stencil_26(z_graddiv_vn, vn, scal_divdamp_o2),
            vn,
        )
    ...
    vn = (
        where(
            (horizontal_lower < horz_idx < horizontal_upper) & (int32(0) < vert_idx < nlev),
            _mo_solve_nonhydro_stencil_27(
                scal_divdamp, bdy_divdamp, nudgecoeff_e, z_graddiv2_vn, vn
            ),
            vn,
        )
        if limited_area
            ...
            (horizontal_lower < horz_idx < horizontal_upper) & (int32(0) < vert_idx < nlev),
            _mo_solve_nonhydro_4th_order_divdamp(scal_divdamp, z_graddiv2_vn, vn),
            vn,
        )
