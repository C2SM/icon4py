    ...
    def __init__(
        ...
        self._mo_icon_interpolation_scalar_cells2verts_scalar_ri_dsl = (
            mo_icon_interpolation_scalar_cells2verts_scalar_ri_dsl.with_backend(
                self._backend
            ).compute(offset_provider_type=self._grid.offset_providers)
        )
        self._mo_math_gradients_grad_green_gauss_cell_dsl = (
            mo_math_gradients_grad_green_gauss_cell_dsl.with_backend(self._backend).compute(
                offset_provider_type=self._grid.offset_providers
            )
        )
        ...
        self._compute_hydrostatic_correction_term = (
            compute_hydrostatic_correction_term.with_backend(self._backend).compute(
                offset_provider_type=self._grid.offset_providers
            )
        )
        ...
        self._compute_results_for_thermodynamic_variables = (
            compute_results_for_thermodynamic_variables.with_backend(self._backend).compute(
                offset_provider_type=self._grid.offset_providers
            )
        )
        ...
        self._add_vertical_wind_derivative_to_divergence_damping = (
            add_vertical_wind_derivative_to_divergence_damping.with_backend(self._backend).compute(
                offset_provider_type=self._grid.offset_providers
            )
        )
        ...
        self._compute_graddiv2_of_vn = compute_graddiv2_of_vn.with_backend(self._backend).compute(
            offset_provider_type=self._grid.offset_providers
        )
        ...
        self._compute_avg_vn = compute_avg_vn.with_backend(self._backend).compute(
            offset_provider_type=self._grid.offset_providers
        )
        self._accumulate_prep_adv_fields = accumulate_prep_adv_fields.with_backend(
            self._backend
        ).compute(offset_provider_type={})
        self._update_mass_volume_flux = update_mass_volume_flux.with_backend(self._backend).compute(
            offset_provider_type={}
        )
        self._update_dynamical_exner_time_increment = (
            update_dynamical_exner_time_increment.with_backend(self._backend).compute(
                offset_provider_type={}
            )
        )
        ...
        self._predictor_stencils_11_lower_upper = (
            nhsolve_stencils.predictor_stencils_11_lower_upper.with_backend(self._backend).compute(
                offset_provider_type=self._grid.offset_providers
            )
        )
        ...
        self._stencils_39_40 = nhsolve_stencils.stencils_39_40.with_backend(self._backend).compute(
            offset_provider_type=self._grid.offset_providers
        )
        ...
        self._stencils_61_62 = nhsolve_stencils.stencils_61_62.with_backend(self._backend).compute(
            offset_provider_type={}
        )
