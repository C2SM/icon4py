...
from icon4py.common.dimension import C2EDim, C2E2CODim, CellDim, EdgeDim, KDim
...
def _fused_velocity_advection_stencil_16_to_18(
    ...
    ddt_w_adv = where((horz_lower_bound < horz_idx < horz_upper_bound) & (int32(0) < vert_idx),
    ...
    ddt_w_adv = (
        where((horz_lower_bound < horz_idx < horz_upper_bound) & (maximum(3, nrdmax - 2) < vert_idx < nlev - 4),
              _mo_velocity_advection_stencil_18(levelmask, cfl_clipping, owner_mask, z_w_con_c, ddqz_z_half,
                                                area, geofac_n2s, w, ddt_w_adv, scalfac_exdiff, cfl_w_limit, dtime),
              ddt_w_adv,
              )
        if extra_diffu
        else ddt_w_adv
