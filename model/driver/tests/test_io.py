...
def test_io_monitor_write_dataset(test_path):
    ...
    field_configs = [FieldGroupIoConfig(output_interval="HOUR",
...
def test_fieldgroup_monitor_write_dataset_file_roll(test_path):
    ...
    config = FieldGroupIoConfig(output_interval="HOUR",
                                start_time=configured_output_start,
                                filename=filename_stub,
                                variables=["air_density", "exner_function",
                                                   "upward_air_velocity"],
                                timesteps_per_file=1)
    monitor = FieldGroupMonitor(config, vertical=VerticalGridSize(grid.num_levels),
                                horizontal=grid.config.horizontal_config, grid_id=str(grid_id), output_path=test_path)
    ...
    for f in monitor.output_path.iterdir():
        if f.is_file():
            ...
            with load_data_file(f) as ds:
                ...
                assert ds.variables["upward_air_velocity"].shape == (1, grid.num_levels + 1, grid.num_cells)
...
@pytest.mark.parametrize(("config, message"), [(
        FieldGroupIoConfig(
            ...
            output_interval="1 HOUR",
            variables=["exner_function", "air_density"],
        ), "absolute path"), (FieldGroupIoConfig(
                start_time="2023-04-04T11:00:00",
                filename="vars/prognostics.nc",
                output_interval="1 HOUR",
                variables=[],
            ), "No variables provided for output."), (FieldGroupIoConfig(
                start_time="2023-04-04T11:00:00",
                filename="vars/prognostics.nc",
                output_interval="",
                variables=["air_density, exner_function"],
