...
Currently, it does only diffusion and solve_nonhydro (dry atmosphere with no physics). The configuration for the granules and driver is hardcoded in [driver.py](src/icon4py/model/driver/driver.py). Time step, total integration time, number of substeps, and etc. can be configured there.
