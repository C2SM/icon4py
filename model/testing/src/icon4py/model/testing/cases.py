...
from typing import Final, NamedTuple, Sequence, TypedDict
...
TORUS_GRIDS: Final[Sequence[Grid]] = tuple(grid for grid in Grid if grid.kind == GridKind.TORUS)
REGIONAL_GRIDS: Final[Sequence[Grid]] = tuple(grid for grid in Grid if grid.kind == GridKind.REGIONAL)
GLOBAL_GRIDS: Final[Sequence[Grid]] = tuple(grid for grid in Grid if grid.kind == GridKind.GLOBAL)

ALL_GRIDS_WITH_FILES: Final[Sequence[Grid]] = tuple(grid for grid in Grid if grid.file_name)
SMALL_GRIDS_WITH_FILES: Final[Sequence[Grid]] = tuple(grid for grid in Grid if grid.file_name and grid.sizes["cell"] < 50000)
