...
class Grids:
    ...
    #: the planar torus of A. Jocksch's moving-cylinder experiment (Jocksch et al., PPAM 2026):
    #: 20 x 22 cells of 5 km edge length, 880 cells / 1320 edges / 440 vertices, generated by
    #: the icon4py grid_generator; the Fortran reference run reads this same file
    TORUS_20X22_5000M: Final = GridDescription(
        name="torus_20x22_res5000m",
        description="Torus grid with a domain (100_000 m x 95_262.79 m), 20 x 22 cells of 5000 m edge length, generated by the icon4py grid_generator",
        params=icon_grid.GridParams(
            icon_grid.TorusParams(
                domain_length=100000.0,
                domain_height=95262.79441628825,
            ),
        ),
    )
    #: the same file with the Cartesian coordinates shifted by (-50000, -47631.4) so that a
    #: cylinder at the origin is a full disc (the layout of Jocksch's own grid)
    TORUS_20X22_5000M_CENTRED: Final = GridDescription(
        name="torus_20x22_res5000m_centred",
        description="TORUS_20X22_5000M with the Cartesian coordinates shifted so the domain is centred on the origin",
        params=icon_grid.GridParams(
            icon_grid.TorusParams(
                domain_length=100000.0,
                domain_height=95262.79441628825,
...
class Experiments:
    ...
    @staticmethod
    def jocksch_cylinder(
        ihadv_tracer: int, itype_hlimit: int, tag: str = ""
    ) -> ExperimentDescription:
        """The capture of A. Jocksch's moving-cylinder test for one (scheme, limiter) pair.

        'tag' distinguishes run variants of the same pair: '_centred' (the shifted grid file,
        Jocksch's own layout), '_dj1' (the d_j = 1 linear WENO weights) or '_dj1_centred'.

        One ICON run (icon-exclaim branch transport_ajocksch_capture, built with -Kieee
        -Mnofma -gpu=nofma, nproma = 1, 10 levels) of 100 calls of step_advection at one
        model date, tracers 1-4 with 'ihadv_tracer' / 'itype_hlimit' and tracer 5 kept
        as the initial cylinder; savepoints 'advection-init' / 'advection-exit' carry a
        'step' key (1..100) and 'lsq-coefficients' the init-time reconstruction
        coefficients. Not downloadable: the data lives next to the Fortran run and is
        linked into the test data layout by hand, see
        model/atmosphere/tracer_advection/docs/weno_idealized_scope.md.
        """
        centred = tag.endswith("_centred")
        return ExperimentDescription(
            name=f"jocksch_cylinder_ihadv{ihadv_tracer}_hlim{itype_hlimit}{tag}",
            long_name=(
                f"Jocksch moving cylinder, ihadv_tracer={ihadv_tracer}, "
                f"itype_hlimit={itype_hlimit}, {'centred' if centred else 'original'} grid"
                f"{', variant ' + tag.strip('_') if tag else ''}"
            ),
            grid=Grids.TORUS_20X22_5000M_CENTRED if centred else Grids.TORUS_20X22_5000M,
            version=1,
        )
