...
from icon4py.model.common.grid import (
    ...
    icon,
    vertical as v_grid, grid_manager as gm,
)
...
def get_grid_manager(
    ...
    return _download_and_load_gridfile(file_path=grid_file, num_levels=num_levels, keep_skip_values= keep_skip_values, backend=backend)
...
def get_grid_geometry(
    ...
    def _construct_grid_geometry():
        gm = _download_and_load_gridfile(grid_file, keep_skip_values=True, num_levels=num_levels, backend=backend)
        grid = gm.grid
