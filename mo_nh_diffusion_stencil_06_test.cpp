...
#include "mo_nh_diffusion_stencil_06.hpp"
...
class mo_nh_diffusion_stencil_06 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_nh_diffusion_stencil_06 has not been set up! make sure "
    ...
    gridtools::stencil::global_parameter fac_bdydiff_v_gp{fac_bdydiff_v_};
    fn_backend_t cuda_backend{};
    cuda_backend.stream = stream_;
    auto connectivities = gridtools::hymap::keys<>::make_values();
    generated::mo_nh_diffusion_stencil_06(connectivities)(
        cuda_backend, z_nabla2_e_sid, area_edge_sid, vn_sid, fac_bdydiff_v_gp,
  ...
  void copy_pointers(double *z_nabla2_e, double *area_edge, double *vn,
                     double fac_bdydiff_v) {
    z_nabla2_e_ = z_nabla2_e;
    area_edge_ = area_edge;
    vn_ = vn;
    fac_bdydiff_v_ = fac_bdydiff_v;
  }
};
} // namespace cuda_ico
} // namespace dawn_generated
...
void run_mo_nh_diffusion_stencil_06(double *z_nabla2_e, double *area_edge,
  ...
  dawn_generated::cuda_ico::mo_nh_diffusion_stencil_06 s;
  s.copy_pointers(z_nabla2_e, area_edge, vn, fac_bdydiff_v);
...
bool verify_mo_nh_diffusion_stencil_06(const double *vn_dsl, const double *vn,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_nh_diffusion_stencil_06::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_nh_diffusion_stencil_06::getStream();
  ...
  int vn_kSize =
      dawn_generated::cuda_ico::mo_nh_diffusion_stencil_06::get_vn_KSize();
  stencilMetrics =
  ...
  MetricsSerialiser serialiser_vn(stencilMetrics,
                                  ...
                                  "mo_nh_diffusion_stencil_06", "vn");
  serialiser_vn.writeJson(iteration);
#endif
  if (!stencilMetrics.isValid) {
#ifdef __SERIALIZE_ON_ERROR
    serialize_dense_edges(0, (mesh.NumEdges - 1), vn_kSize, (mesh.EdgeStride),
                          vn, "mo_nh_diffusion_stencil_06", "vn", iteration);
    serialize_dense_edges(0, (mesh.NumEdges - 1), vn_kSize, (mesh.EdgeStride),
                          vn_dsl, "mo_nh_diffusion_stencil_06", "vn_dsl",
                          iteration);
    std::cout << "[DSL] serializing vn as error is high.\n" << std::flush;
#endif
  };
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_nh_diffusion_stencil_06", iteration);
#endif
...
void run_and_verify_mo_nh_diffusion_stencil_06(
  ...
  std::cout << "[DSL] Running stencil mo_nh_diffusion_stencil_06 (" << iteration
            << ") ...\n"
            << std::flush;
  run_mo_nh_diffusion_stencil_06(
      z_nabla2_e, area_edge, vn_before, fac_bdydiff_v, const int verticalStart,
      const int verticalEnd, const int horizontalStart,
      const int horizontalEnd);

  std::cout << "[DSL] mo_nh_diffusion_stencil_06 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_nh_diffusion_stencil_06...\n"
            << std::flush;
  verify_mo_nh_diffusion_stencil_06(vn_before, vn, vn_rel_tol, vn_abs_tol,
                                    iteration);
...
void setup_mo_nh_diffusion_stencil_06(dawn::GlobalGpuTriMesh *mesh, int k_size,
                                      cudaStream_t stream,
                                      const int vn_k_size) {
  dawn_generated::cuda_ico::mo_nh_diffusion_stencil_06::setup(
      mesh, k_size, stream, vn_k_size);
}

void free_mo_nh_diffusion_stencil_06() {
  dawn_generated::cuda_ico::mo_nh_diffusion_stencil_06::free();
}
}
