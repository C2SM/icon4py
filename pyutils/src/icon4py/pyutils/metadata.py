...
class StencilImporter:
    """Class which imports a fencil from a fencil import path and provides access to the related fencil data.
    ...
    def __init__(self, fencil_import_path: str) -> None:
        ...
        self.stencil_info = self._get_stencil_info()
    ...
    def _get_stencil_info(self) -> StencilInfo:
        """Generate StencilInfo dataclass from a fencil definition."""
        offsets = self._scan_for_offsets()
        offset_provider = {}
        for offset in offsets:
            offset_provider[offset] = self._provide_offset(offset)
        connectivity_chains = [offset for offset in offsets if offset != Koff.value]
        return StencilInfo(self.fvprog, connectivity_chains, offset_provider)

    def _provide_offset(self, offset: str) -> types.SimpleNamespace | Dimension:
        if offset == Koff.value:
            assert len(Koff.target) == 1
            assert Koff.source == Koff.target[0]
            return Koff.source
        else:
            return self._provide_neighbor_table(offset)
    ...
    def _scan_for_offsets(self) -> list[eve.concepts.SymbolRef]:
        """Scan PAST node for offsets and return a set of all offsets."""
        all_types = (
            self.fvprog.past_node.pre_walk_values()
        ...
        all_offset_labels = (
            self.fvprog.itir.pre_walk_values()
            .if_isinstance(itir.OffsetLiteral)
            .getattr("value")
            .to_list()
        )
        all_dim_labels = [
            dim.value for dim in all_dims if dim.kind == DimensionKind.LOCAL
        ]
