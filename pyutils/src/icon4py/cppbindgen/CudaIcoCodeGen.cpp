...
class Field {
public:
  Field(std::string name, std::string cpp_type, dawn::codegen::FortranAPI::InterfaceType fortran_type,
        std::string bindgen_type, IntendKind intend, std::optional<dawn::ast::LocationType> horizontal_location,
        std::optional<dawn::ast::NeighborChain> sparse_location, bool is_vertical, bool include_center) :
    name_(std::move(name)), cpp_type_(std::move(cpp_type)), fortran_type_(fortran_type),
    bindgen_type_(std::move(bindgen_type)), intend_(intend), horizontal_location_(horizontal_location),
    sparse_location_(sparse_location), is_vertical_(is_vertical), include_center_(include_center) {
    rank_ = 0;
    if(is_vertical_) {
      rank_ += 1;
    }
  ...
  [[nodiscard]] std::string getCppType() const {
    return cpp_type_;
  }

  [[nodiscard]] dawn::codegen::FortranAPI::InterfaceType getFortranType() const {
    return fortran_type_;
  }

  [[nodiscard]] std::string getBindgenType() const {
    return bindgen_type_;
  }
  ...
  [[nodiscard]] IntendKind getIntend() const {
    return intend_;
  }
  ...
  [[nodiscard]] bool isSparse() const {
    return sparse_location_.has_value();
  }
  ...
  [[nodiscard]] bool includesCenter() const {
...
std::vector<Field> getUsedFields(const std::vector<Field>& fieldVector,
                               const std::unordered_set<IntendKind>& intend = {
                                   IntendKind::Output,
                                   IntendKind::InputOutput,
                                   IntendKind::Input}) {
...
std::vector<std::string> getUsedFieldsNames(const std::vector<Field>& fieldVector,
    const std::unordered_set<IntendKind>& intend = { IntendKind::Output, IntendKind::InputOutput,
        IntendKind::Input}) {
...
std::string explodeUsedFields(const std::vector<Field>& fieldVector, const std::unordered_set<IntendKind>& intend =
...
void generateGpuMesh(
    dawn::codegen::Class& wrapperClass, const std::vector<dawn::ast::UnstructuredIterationSpace> &allSpaces) {
  ...
  for (auto space : allSpaces){
    gpuMeshClass.addMember("int*", cudaico::chainToTableString(dawn::ast::UnstructuredIterationSpace(std::move(space.Chain), space.IncludeCenter)));
  }
  ...
  }
  {
    ...
    for (auto space : allSpaces){
      auto tableName = cudaico::chainToTableString(dawn::ast::UnstructuredIterationSpace(std::move(space.Chain), space.IncludeCenter));
      auto curlyInit = cudaico::chainToVectorString(space.Chain);
      gpuMeshFromGlobalCtor.addStatement(tableName + " = mesh->NeighborTables.at(std::tuple<std::vector<dawn::LocationType>, bool>{" +
...
void generateRunFun(dawn::codegen::MemberFunction& runFun, std::string stencilName,
      const std::vector<Field> &fields, const std::vector<dawn::ast::UnstructuredIterationSpace> &allSpaces) {
  ...
  auto dimString = [] (const Field& field) -> std::string {
    if (field.isHorizontal() && field.isVertical() && field.isSparse()) {
      return "mesh_." + dawn::codegen::cudaico::locToStrideString(field.getHorizontalLocation()) + ", " + std::to_string(dawn::ICOChainSize(field.getSparseLocation()) + (field.includesCenter() ? 1 : 0)) + ", kSize_";
    }
    else if (field.isHorizontal() && field.isSparse()) {
      return "mesh_." + dawn::codegen::cudaico::locToStrideString(field.getHorizontalLocation()) + ", " + std::to_string(dawn::ICOChainSize(field.getSparseLocation()) + (field.includesCenter() ? 1 : 0));
    }
    else if (field.isHorizontal() && field.isVertical()) {
      return "mesh_." + dawn::codegen::cudaico::locToStrideString(field.getHorizontalLocation()) + ", kSize_";
    } else if (field.isHorizontal()) {
      return "mesh_." + dawn::codegen::cudaico::locToStrideString(field.getHorizontalLocation()) + "";
  ...
  auto getTaggedStrideMap = [] (const Field& field) -> std::string {
    if (field.isHorizontal() && field.isVertical()) {
      return "gridtools::hymap::keys<unstructured::dim::horizontal, unstructured::dim::vertical>::make_values(1, mesh_." + dawn::codegen::cudaico::locToStrideString(field.getHorizontalLocation()) + ")";
    } else if (field.isHorizontal()) {
      return "gridtools::hymap::keys<unstructured::dim::horizontal>::make_values(1)";
    } else if (field.isVertical()) {
      return "gridtools::hymap::keys<unstructured::dim::vertical>::make_values(1)";
    }
  };

  for (const auto& field : getUsedFields(fields)) {
    if (field.getRank() > 0 && !field.isSparse()) {
      runFun.addStatement("auto " + field.getName() + "_sid = get_sid(" + field.getName()+ "_, " + getTaggedStrideMap(field) + ")");
    }
  }
  ...
  for (const auto& field : getUsedFields(fields)) {
    if (field.getRank() == 0) {
      runFun.addStatement("gridtools::stencil::global_parameter " + field.getName() + "_gp {" + field.getName() + "_}");
    }
  }
  ...
  for (auto space : allSpaces){
    std::string chainShorthand = dawn::codegen::cudaico::chainToShorthand(space);
    std::string chainShorthandGt4py = dawn::codegen::cudaico::chainToShorthand(space, dawn::codegen::cudaico::StringCase::upper, '2');
    runFun.addStatement("nbh_ptr_" + chainShorthand + " " + chainShorthand + "_ptr{.raw_ptr_fortran = mesh_." + chainShorthand + "Table}");
  ...
  for (const auto &field : getUsedFields(fields)) {
    if (field.isSparse()) {
      ...
      for (int i = 0; i < numSparse; i++){
        runFun.addStatement("double *" + field.getName() + "_" + std::to_string(i) + " = &" + field.getName() +  "_[" +
              std::to_string(i) + "*mesh_." + dawn::codegen::cudaico::locToStrideString(field.getHorizontalLocation()) + "]");
      }
      for (int i = 0; i < numSparse; i++){
        runFun.addStatement("auto " + field.getName() + "_sid_" + std::to_string(i) + " = get_sid(" + field.getName() +  "_" + std::to_string(i)  +", gridtools::hymap::keys<unstructured::dim::horizontal>::make_values(1))");
      }
      std::vector<std::string> integralConstantsVec;
      std::vector<std::string> sidsVec;
      for (int i = 0; i < numSparse; i++){
        integralConstantsVec.push_back("integral_constant<int," + std::to_string(i) + ">");
      ...
      runFun.addStatement("auto " + field.getName() + "_sid_comp = sid::composite::keys<" + integralConstants + ">::make_values(" + sids + ")");
    }
  }
...
void generateStencilSetup(dawn::codegen::MemberFunction& setup, const std::vector<Field> &fields) {
  ...
  for(const auto& fieldName :
      getUsedFieldsNames(fields, {IntendKind::Output, IntendKind::InputOutput})) {
    setup.addStatement(fieldName + "_kSize_ = " + fieldName + "_kSize");
  }
}

void generateCopyPtrFun(dawn::codegen::MemberFunction& copyFun, const std::vector<Field> &fields) {
  auto usedAPIFields = getUsedFields(fields);

  for(const auto& field : usedAPIFields) {
    copyFun.addArg(field.getCppType() + (field.getRank() > 0 ? "*" : "") + " " + field.getName());
  }
...
void generateStencilClasses(dawn::codegen::Class& stencilWrapperClass, std::string stencilName,
                              const std::vector<Field> &fields, const std::vector<dawn::ast::UnstructuredIterationSpace> &allSpaces) {
  ...
  for(const auto& field : getUsedFields(fields)) {
    stencilWrapperClass.addMember(field.getCppType() + (field.getRank() > 0 ? "*" : ""), field.getName() + "_");
  }
  stencilWrapperClass.addMember("static int", "kSize_");
  stencilWrapperClass.addMember("static GpuTriMesh", "mesh_");
  stencilWrapperClass.addMember("static bool", "is_setup_");
  stencilWrapperClass.addMember("static cudaStream_t", "stream_");

  for(const auto& field : getUsedFields(fields, {IntendKind::Output, IntendKind::InputOutput})) {
    stencilWrapperClass.addMember("static int", field.getName() + "_kSize_");
  }

  stencilWrapperClass.changeAccessibility("public");
  auto meshGetter = stencilWrapperClass.addMemberFunction("static const GpuTriMesh &", "getMesh");
  meshGetter.finishArgs();
  meshGetter.startBody();
  meshGetter.addStatement("return mesh_");
  meshGetter.commit();

  auto streamGetter = stencilWrapperClass.addMemberFunction("static cudaStream_t", "getStream");
  streamGetter.finishArgs();
  streamGetter.startBody();
  streamGetter.addStatement("return stream_");
  streamGetter.commit();

  auto kSizeGetter = stencilWrapperClass.addMemberFunction("static int", "getKSize");
  ...
  for(const auto& field : getUsedFields(fields, {IntendKind::Output, IntendKind::InputOutput})) {
    auto kSizeFieldGetter = stencilWrapperClass.addMemberFunction(
        "static int",
        "get_" + field.getName() + "_KSize");
    kSizeFieldGetter.finishArgs();
    kSizeFieldGetter.startBody();
    kSizeFieldGetter.addStatement(
        "return " + field.getName() + "_kSize_");
    kSizeFieldGetter.commit();
  }

  // constructor from library
  auto stencilClassFree = stencilWrapperClass.addMemberFunction("static void", "free");
  generateStencilFree(stencilClassFree);
  stencilClassFree.commit();

  auto stencilClassSetup = stencilWrapperClass.addMemberFunction("static void", "setup");
  ...
  // copy to funs
  auto copyPtrFun = stencilWrapperClass.addMemberFunction("void", "copy_pointers");
  generateCopyPtrFun(copyPtrFun, fields);
  copyPtrFun.commit();
}

void generateAllAPIRunFunctions(
    std::stringstream& ssSW, std::string stencilName, const std::vector<Field> &fields, CallerKind callerKind, CodeGenKind codeGenKind) {
  ...
  std::vector<std::stringstream> apiRunFunStreams(callerKind == CallerKind::FromHost ? 2 : 1);
  { // stringstreams need to outlive the correspondind MemberFunctions
    ...
    apiRunFuns.push_back(std::make_unique<dawn::codegen::MemberFunction>(
        "void", "run_" + wrapperName, apiRunFunStreams[0], /*indent level*/ 0, codeGenKind == CodeGenKind::OnlyDecl));

    for(auto& apiRunFun : apiRunFuns) {
      for(const auto& field : fields) {
        apiRunFun->addArg(field.getCppType() + (field.getRank() > 0 ? "*" : "") + " " + field.getName());
      }
    }
    for(auto& apiRunFun : apiRunFuns) {
...
void generateAllAPIVerifyFunctions(std::stringstream& ssSW, std::string stencilName, const std::vector<Field> &fields, CodeGenKind codeGenKind) {
  ...
  { // stringstreams need to outlive the correspondind MemberFunctions
    ...
    dawn::codegen::MemberFunction verifyAPI("bool", "verify_" + wrapperName, verifySS, /*indent level*/ 0,
                             codeGenKind == CodeGenKind::OnlyDecl);
    dawn::codegen::MemberFunction runAndVerifyAPI("void", "run_and_verify_" + wrapperName, runAndVerifySS,
                                   /*indent level*/ 0, codeGenKind == CodeGenKind::OnlyDecl);

    for(const auto& field : getUsedFields(fields, {IntendKind::Output, IntendKind::InputOutput})) {
      verifyAPI.addArg("const " + field.getCppType() + (field.getRank() > 0 ? "*" : "") + " " + field.getName() + "_dsl");
      verifyAPI.addArg("const " + field.getCppType() + (field.getRank() > 0 ? "*" : "") + " " + field.getName());
    }
    for(const auto& field : getUsedFields(fields, {IntendKind::Output, IntendKind::InputOutput})) {
    ...
    for(const auto& field : getUsedFields(fields)) {
      runAndVerifyAPI.addArg(field.getCppType() + (field.getRank() > 0 ? "*" : "") + " " + field.getName());
    }
    for(const auto& field : getUsedFields(fields, {IntendKind::InputOutput, IntendKind::Output})) {
      runAndVerifyAPI.addArg(field.getCppType() + (field.getRank() > 0 ? "*" : "") + " " + field.getName() + "_before");
    }
    ...
    if(codeGenKind == CodeGenKind::WithBody) {
      ...
      for(auto field : getUsedFields(fields, {IntendKind::Output, IntendKind::InputOutput})) {

        verifyAPI.addStatement("int " + field.getName() + "_kSize = " + fullStencilName +
                               "::" + "get_" + field.getName() + "_KSize()");

        std::string num_lev = field.getName() + "_kSize";

        std::string dense_stride =
            "(mesh." + dawn::codegen::cudaico::locToStrideString(field.getHorizontalLocation()) + ")";
        std::string indexOfLastHorElement =
            "(mesh." + dawn::codegen::cudaico::locToDenseSizeStringGpuMesh(field.getHorizontalLocation()) + " -1)";
        std::string num_el = dense_stride + " * " + num_lev;
        verifyAPI.addStatement("stencilMetrics = ::dawn::verify_field(stream, " + num_el + ", " + field.getName() +
                               "_dsl" + "," + field.getName() + ", \"" + field.getName() + "\"" +
                               "," + field.getName() + "_rel_tol" + "," + field.getName() +
                               "_abs_tol, " + "iteration" + ")");
        ...
        verifyAPI.addBlockStatement("if (!stencilMetrics.isValid)", [&]() {
          ...
          if(!field.isSparse()) {
            ...
            verifyAPI.addStatement(serializeCall + "(0" + ", " + indexOfLastHorElement + ", " +
                                   ...
                                   wrapperName + "\", \"" + field.getName() + "\", iteration)");
            // serialize dsl field
            verifyAPI.addStatement(serializeCall + "(0" + ", " + indexOfLastHorElement + ", " +
                                   num_lev + ", " + dense_stride + ", " + field.getName() + "_dsl" +
                                   ", \"" + wrapperName + "\", \"" + field.getName() + "_dsl" +
                                   "\", iteration)");
            verifyAPI.addStatement("std::cout << \"[DSL] serializing " + field.getName() +
                                   " as error is high.\\n\" << std::flush");
          } else {
      ...
      runAndVerifyAPI.addStatement(
          "verify_" + wrapperName + "(" +
          explodeToStr(dawn::concatenateVectors({outputVerifyFields, {"iteration"}})) + ")");
...
void generateMemMgmtFunctions(
    std::stringstream& ssSW, std::string stencilName, const std::vector<Field> &fields, CodeGenKind codeGenKind) {
  const std::string wrapperName = stencilName;
  const std::string fullStencilName =
      "dawn_generated::cuda_ico::" + wrapperName + "::" + stencilName;
  ...
  dawn::codegen::MemberFunction setupFun("void", "setup_" + wrapperName, ssSW, indentLevel, codeGenKind == CodeGenKind::OnlyDecl);
  ...
  dawn::codegen::MemberFunction freeFun("void", "free_" + wrapperName, ssSW, indentLevel, codeGenKind == CodeGenKind::OnlyDecl);
  freeFun.finishArgs();
  if(codeGenKind == CodeGenKind::WithBody) {
    freeFun.startBody();
...
void generateStaticMembersTrailer(
    std::stringstream& ssSW, std::string stencilName, const std::vector<Field> &fields) {
  const std::string wrapperName = stencilName;
  const std::string fullStencilName =
      "dawn_generated::cuda_ico::" + wrapperName + "::" + stencilName;
...
std::string generateStencilInstantiation(const std::vector<Field> &fields, const std::vector<dawn::ast::UnstructuredIterationSpace>& allSpaces, std::string stencilName) {
  using namespace dawn::codegen;
  ...
  ssSW << "extern \"C\" {\n";
  generateAllAPIRunFunctions(ssSW, stencilName, fields, CallerKind::FromHost, CodeGenKind::WithBody);
  generateAllAPIVerifyFunctions(ssSW, stencilName, fields, CodeGenKind::WithBody);
  generateMemMgmtFunctions(ssSW, stencilName, fields, CodeGenKind::WithBody);
  ssSW << "}\n";

  generateStaticMembersTrailer(ssSW, stencilName, fields);

  return ssSW.str();
}

void generateCHeaderSI(std::stringstream& ssSW, std::string stencilName, const std::vector<Field> &fields) {
  ssSW << "extern \"C\" {\n";
  generateAllAPIRunFunctions(ssSW, stencilName, fields, CallerKind::FromDevice, CodeGenKind::OnlyDecl);
  generateAllAPIVerifyFunctions(ssSW, stencilName, fields, CodeGenKind::OnlyDecl);
  generateMemMgmtFunctions(ssSW, stencilName, fields, CodeGenKind::OnlyDecl);
  ssSW << "}\n";
}

std::string generateCHeader(std::string stencilName, const std::vector<Field> &fields) {
  std::stringstream ssSW;
  ssSW << "#pragma once\n";
...
static void
generateF90InterfaceSI(dawn::codegen::FortranInterfaceModuleGen& fimGen, std::string stencilName, const std::vector<Field> &fields) {
  ...
  dawn::codegen::FortranWrapperAPI runWrapper = dawn::codegen::FortranWrapperAPI("wrap_run_" + stencilName);

  // only from host convenience wrapper takes mesh and k_size
  const int fromDeviceAPIIdx = 0;
  const int runAndVerifyIdx = 1;

  auto addArgsToAPI = [&](dawn::codegen::FortranAPI& api, bool includeSavedState, bool optThresholds) {
    ...
    api.addArg("vertical_lower", dawn::codegen::FortranAPI::InterfaceType::INTEGER);
    api.addArg("vertical_upper", dawn::codegen::FortranAPI::InterfaceType::INTEGER);

    api.addArg("horizontal_lower", dawn::codegen::FortranAPI::InterfaceType::INTEGER);
    api.addArg("horizontal_upper", dawn::codegen::FortranAPI::InterfaceType::INTEGER);

    if(includeSavedState) {
      for(const auto& field : getUsedFields(fields, {IntendKind::Output, IntendKind::InputOutput})) {
        if(optThresholds) {
          api.addOptArg(field.getName() +
                            "_rel_tol",
  ...
  auto getFieldArgs = [&](bool includeSavedState, bool includeScalars = true) -> std::vector<std::string> {
  ...
  for(const auto& field : getUsedFields(fields, {IntendKind::Output, IntendKind::InputOutput})) {
    threshold_names.push_back(field.getName() +
                              "_rel_err_tol");
    threshold_names.push_back(field.getName() +
                              "_abs_err_tol");
  }

  auto genCallArgs = [&](dawn::codegen::FortranWrapperAPI& wrapper, std::string first = "",
  ...
  for(const auto& field : getUsedFields(fields, {IntendKind::Output, IntendKind::InputOutput})) {
    runWrapper.addBodyLine("if (present(" +
                           ...
                           "_rel_tol" + ")) then");
    runWrapper.addBodyLine(
        "  " + field.getName() + "_rel_err_tol" +
        " = " + field.getName() + "_rel_tol");
    runWrapper.addBodyLine("else");
    runWrapper.addBodyLine("  " + field.getName() +
  ...
  auto fieldArgs = getFieldArgs(/*includeSavedState*/ true, /*includeScalars*/ false);
  ...
  for(const auto& field : getUsedFields(fields, {IntendKind::Output, IntendKind::InputOutput})) {
    verticalBoundNames.push_back(field.getName() +
                                  "_kvert_max");
  }
  ...
  setup.addArg("stream", dawn::codegen::FortranAPI::InterfaceType::CUDA_STREAM_T);
  for(const auto& field : getUsedFields(fields, {IntendKind::Output, IntendKind::InputOutput})) {
  ...
  setupWrapper.addArg("k_size", dawn::codegen::FortranAPI::InterfaceType::INTEGER);
  setupWrapper.addArg("stream", dawn::codegen::FortranAPI::InterfaceType::CUDA_STREAM_T);

  for(const auto& field : getUsedFields(fields, {IntendKind::Output, IntendKind::InputOutput})) {
    setupWrapper.addOptArg(field.getName() +
                               "_kmax",
                           dawn::codegen::FortranAPI::InterfaceType::INTEGER);
  }
...
std::string generateF90Interface(std::string moduleName, std::string stencilName, const std::vector<Field> &fields) {
  std::stringstream ss;
...
void generateCode(std::filesystem::path workPath, std::string stencilName, int blockSize, int levelsPerThread,
                    const std::vector<Field> &fields, const std::vector<dawn::ast::UnstructuredIterationSpace> &allSpaces) {
    ...
    std::vector<std::string> ppDefines{
            ...
            "#include <chrono>",
            "#define BLOCK_SIZE " + std::to_string(blockSize), // std::to_string(codeGenOptions_.BlockSize),
            "#define LEVELS_PER_THREAD " + std::to_string(levelsPerThread), // + std::to_string(codeGenOptions_.LevelsPerThread),
            "namespace {",
            "    template <int... sizes>",
            "    using block_sizes_t =",
            ...
            "",
            "    using fn_backend_t = gridtools::fn::backend::gpu<block_sizes_t<BLOCK_SIZE, LEVELS_PER_THREAD>>;",
            "} // namespace",
  ...
  for (auto space : allSpaces){
    std::string shortHand = dawn::codegen::cudaico::chainToShorthand(space);
    ppDefines.push_back("struct nbh_ptr_" + shortHand + "{");
    ppDefines.push_back("   const int *raw_ptr_fortran;");
    ppDefines.push_back("};");
    ppDefines.push_back("");
    ppDefines.push_back("__device__");
    int numNbh = dawn::ICOChainSize(space.Chain) + (space.IncludeCenter ? 1 : 0);
    ppDefines.push_back("inline gridtools::array<int, " + std::to_string(numNbh) + "> neighbor_table_neighbors(nbh_ptr_" + shortHand + " const &table, int index) {");

    std::string indexFun = "return {";
    for (int i = 0; i < numNbh; i++) {
      indexFun += "table.raw_ptr_fortran[index+nproma*" + std::to_string(i) + "],";
    }
    indexFun.pop_back();
    indexFun += "};";
    ppDefines.push_back(indexFun);
    ppDefines.push_back("}");
    ppDefines.push_back("");
  }
  ppDefines.push_back("");
...
auto get_sid(Ptr ptr, StrideMap const& strideMap) {
  ...
  if(file) {
    ...
    throw std::runtime_error("Error writing to " + cppPath.string() + ": " + std::strerror(errno));
  }
  ...
  if(headerFile) {
    ...
    throw std::runtime_error("Error writing to " + headerPath.string() + ": " + std::strerror(errno));
  }
  ...
  if(interfaceFile) {
    ...
    throw std::runtime_error("Error writing to " + f90Path.string() + ": " + std::strerror(errno));
  }
...
int main (int argc, char** argv) {
  ...
  if (cmdOptionExists(argv, argv+argc, "--levels-per-thread")){
    levelsPerThread = atoi(getCmdOption(argv, argv+argc, "--levels-per-thread"));
  }
    ...
    for (auto token : sparseTokens){
      switch (token[0]){
        ...
        case 'C':
          ...
          break;	
        default:
          DAWN_ASSERT_MSG(0, "malformed sparse dimension. please use single upper case char divided by 2. examples: C2E, C2E2V");
      }
    }
    return dawn::ast::UnstructuredIterationSpace(std::move(chain), includeCenter);
  };
  ...
  while(std::getline(dataFile, line)) {
    ...
    for (size_t counter = 0; counter < closingBrackets.size(); counter++) {
      layers.push_back(line.substr(openingBrackets[counter] + 1, closingBrackets[counter] - openingBrackets[counter] - 1));
    }

    size_t typePos = layers[0].find("dtype=") + 6;

    std::string basicType = layers[0].substr(typePos, layers[0].size() - typePos);

    std::string cppType;
    std::string bindgenType;
    ...
    if (basicType == "float64") {
      ...
      bindgenType = std::string("bindgen_fk_Double");
    } else if (basicType == "int32") {
      cppType = std::string("int");
      fortranType = dawn::codegen::FortranAPI::InterfaceType::INTEGER;
      bindgenType = std::string("bindgen_fk_Int");
    } else if (basicType == "bool8") {
      cppType = std::string("bool");
      fortranType = dawn::codegen::FortranAPI::InterfaceType::BOOLEAN;
      bindgenType = std::string("bindgen_fk_Bool");
    } else {
      throw std::runtime_error("No types supported but float64, int32 and bool8");
    }
    ...
    auto locationStringToLocation = [](const std::string& locationString){
      if(locationString == "Edge") {
        ...
        throw std::runtime_error("Location needs to be one of Edge, Vertex, Cell");
      }
    };
    ...
    if(intentString == "inout") {
      ...
      throw std::runtime_error("Invalid intendString, which could not be parsed. Needs to be one of the following: in, out, inout");
    }
    ...
    fields.emplace_back(Field(fieldName, cppType, fortranType, bindgenType, intend, optionalHorizontalLocation, optionalSparseLocation, isVertical, includeCenter));
  }


  generateCode(dataPath.remove_filename(), stencilName, blockSize, levelsPerThread, fields, allSpaces);
