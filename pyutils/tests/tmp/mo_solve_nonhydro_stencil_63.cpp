...
#include "mo_solve_nonhydro_stencil_63.hpp"
...
class mo_solve_nonhydro_stencil_63 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_63 has not been set up! make sure "
    ...
    generated::mo_solve_nonhydro_stencil_63(connectivities)(
...
void run_mo_solve_nonhydro_stencil_63(double *inv_ddqz_z_full, double *w,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_63 s;
...
bool verify_mo_solve_nonhydro_stencil_63(const double *z_dwdz_dd_dsl,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_63::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_63::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_63::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int z_dwdz_dd_kSize = dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_63::
      get_z_dwdz_dd_KSize();
  ...
  MetricsSerialiser serialiser_z_dwdz_dd(
      ...
      "mo_solve_nonhydro_stencil_63", "z_dwdz_dd");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_cells(
        ...
        "mo_solve_nonhydro_stencil_63", "z_dwdz_dd", iteration);
    serialize_dense_cells(0, (mesh.NumCells - 1), z_dwdz_dd_kSize,
                          (mesh.CellStride), z_dwdz_dd_dsl,
                          "mo_solve_nonhydro_stencil_63", "z_dwdz_dd_dsl",
                          iteration);
...
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_63", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_63(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_63 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_63(inv_ddqz_z_full, w, w_concorr_c,
                                   z_dwdz_dd_before, verticalStart, verticalEnd,
                                   horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_63 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_63...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_63(z_dwdz_dd_before, z_dwdz_dd,
...
void setup_mo_solve_nonhydro_stencil_63(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_63::setup(
      mesh, k_size, stream, z_dwdz_dd_k_size);
}

void free_mo_solve_nonhydro_stencil_63() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_63::free();
}
}
