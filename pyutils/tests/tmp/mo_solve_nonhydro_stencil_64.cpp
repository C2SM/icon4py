...
#include "mo_solve_nonhydro_stencil_64.hpp"
...
class mo_solve_nonhydro_stencil_64 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_64 has not been set up! make sure "
    ...
    generated::mo_solve_nonhydro_stencil_64(connectivities)(
...
void run_mo_solve_nonhydro_stencil_64(double *mass_flx_ic,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_64 s;
...
bool verify_mo_solve_nonhydro_stencil_64(const double *mass_flx_ic_dsl,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_64::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_64::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_64::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int mass_flx_ic_kSize = dawn_generated::cuda_ico::
      mo_solve_nonhydro_stencil_64::get_mass_flx_ic_KSize();
  stencilMetrics =
  ...
  MetricsSerialiser serialiser_mass_flx_ic(
      ...
      "mo_solve_nonhydro_stencil_64", "mass_flx_ic");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_cells(
        ...
        mass_flx_ic, "mo_solve_nonhydro_stencil_64", "mass_flx_ic", iteration);
    serialize_dense_cells(0, (mesh.NumCells - 1), mass_flx_ic_kSize,
                          (mesh.CellStride), mass_flx_ic_dsl,
                          "mo_solve_nonhydro_stencil_64", "mass_flx_ic_dsl",
                          iteration);
...
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_64", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_64(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_64 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_64(mass_flx_ic_before, verticalStart,
                                   verticalEnd, horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_64 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_64...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_64(mass_flx_ic_before, mass_flx_ic,
...
void setup_mo_solve_nonhydro_stencil_64(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_64::setup(
      mesh, k_size, stream, mass_flx_ic_k_size);
}

void free_mo_solve_nonhydro_stencil_64() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_64::free();
}
}
