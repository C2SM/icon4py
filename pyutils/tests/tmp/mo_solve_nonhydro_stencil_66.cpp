...
#include "mo_solve_nonhydro_stencil_66.hpp"
...
class mo_solve_nonhydro_stencil_66 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_66 has not been set up! make sure "
    ...
    generated::mo_solve_nonhydro_stencil_66(connectivities)(
        cuda_backend, bdy_halo_c_sid, rho_sid, theta_v_sid, exner_sid,
        rd_o_cvd_gp, rd_o_p0ref_gp, horizontalStart, horizontalEnd,
  ...
  void copy_pointers(int *bdy_halo_c, double *rho, double *theta_v,
                     double *exner, double rd_o_cvd, double rd_o_p0ref) {
    bdy_halo_c_ = bdy_halo_c;
    rho_ = rho;
    theta_v_ = theta_v;
    exner_ = exner;
    rd_o_cvd_ = rd_o_cvd;
...
void run_mo_solve_nonhydro_stencil_66(
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_66 s;
  s.copy_pointers(bdy_halo_c, rho, theta_v, exner, rd_o_cvd, rd_o_p0ref);
...
bool verify_mo_solve_nonhydro_stencil_66(
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_66::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_66::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_66::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int theta_v_kSize = dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_66::
      get_theta_v_KSize();
  ...
  MetricsSerialiser serialiser_theta_v(
      ...
      "mo_solve_nonhydro_stencil_66", "theta_v");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_cells(0, (mesh.NumCells - 1), theta_v_kSize,
                          ...
                          "mo_solve_nonhydro_stencil_66", "theta_v", iteration);
    serialize_dense_cells(
        0, (mesh.NumCells - 1), theta_v_kSize, (mesh.CellStride), theta_v_dsl,
        "mo_solve_nonhydro_stencil_66", "theta_v_dsl", iteration);
    std::cout << "[DSL] serializing theta_v as error is high.\n" << std::flush;
#endif
  }
  int exner_kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_66::get_exner_KSize();
  stencilMetrics = ::dawn::verify_field(
  ...
  MetricsSerialiser serialiser_exner(stencilMetrics,
                                     ...
                                     "mo_solve_nonhydro_stencil_66", "exner");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_cells(0, (mesh.NumCells - 1), exner_kSize,
                          ...
                          "mo_solve_nonhydro_stencil_66", "exner", iteration);
    serialize_dense_cells(
        0, (mesh.NumCells - 1), exner_kSize, (mesh.CellStride), exner_dsl,
        "mo_solve_nonhydro_stencil_66", "exner_dsl", iteration);
    std::cout << "[DSL] serializing exner as error is high.\n" << std::flush;
#endif
  };
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_66", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_66(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_66 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_66(
      bdy_halo_c, rho, theta_v_before, exner_before, rd_o_cvd, rd_o_p0ref,
      verticalStart, verticalEnd, horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_66 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_66...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_66(theta_v_before, theta_v, exner_before,
...
void setup_mo_solve_nonhydro_stencil_66(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_66::setup(
      mesh, k_size, stream, theta_v_k_size, exner_k_size);
}

void free_mo_solve_nonhydro_stencil_66() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_66::free();
}
}
