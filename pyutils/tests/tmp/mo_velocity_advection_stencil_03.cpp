...
#include "mo_velocity_advection_stencil_03.hpp"
...
class mo_velocity_advection_stencil_03 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  static void setup(const dawn::GlobalGpuTriMesh *mesh, int kSize,
                    cudaStream_t stream, const int z_vt_ie_kSize) {
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_velocity_advection_stencil_03 has not been set up! make sure "
    ...
    generated::mo_velocity_advection_stencil_03(connectivities)(
        cuda_backend, wgtfac_e_sid, vt_sid, z_vt_ie_sid, horizontalStart,
  ...
  void copy_pointers(double *wgtfac_e, double *vt, double *z_vt_ie) {
    wgtfac_e_ = wgtfac_e;
    vt_ = vt;
    z_vt_ie_ = z_vt_ie;
  }
...
void run_mo_velocity_advection_stencil_03(
  ...
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_03 s;
  s.copy_pointers(wgtfac_e, vt, z_vt_ie);
...
bool verify_mo_velocity_advection_stencil_03(const double *z_vt_ie_dsl,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_03::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_03::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_03::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int z_vt_ie_kSize = dawn_generated::cuda_ico::
      mo_velocity_advection_stencil_03::get_z_vt_ie_KSize();
  ...
  MetricsSerialiser serialiser_z_vt_ie(
      ...
      "mo_velocity_advection_stencil_03", "z_vt_ie");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_edges(
        ...
        "mo_velocity_advection_stencil_03", "z_vt_ie", iteration);
    serialize_dense_edges(
        0, (mesh.NumEdges - 1), z_vt_ie_kSize, (mesh.EdgeStride), z_vt_ie_dsl,
        "mo_velocity_advection_stencil_03", "z_vt_ie_dsl", iteration);
    std::cout << "[DSL] serializing z_vt_ie as error is high.\n" << std::flush;
#endif
  };
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_velocity_advection_stencil_03", iteration);
#endif
...
void run_and_verify_mo_velocity_advection_stencil_03(
  ...
  std::cout << "[DSL] Running stencil mo_velocity_advection_stencil_03 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_velocity_advection_stencil_03(wgtfac_e, vt, z_vt_ie_before,
                                       verticalStart, verticalEnd,
                                       horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_velocity_advection_stencil_03 run time: " << time
            << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_velocity_advection_stencil_03...\n"
            << std::flush;
  verify_mo_velocity_advection_stencil_03(
      z_vt_ie_before, z_vt_ie, z_vt_ie_rel_tol, z_vt_ie_abs_tol, iteration);
...
void setup_mo_velocity_advection_stencil_03(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_03::setup(
      mesh, k_size, stream, z_vt_ie_k_size);
}

void free_mo_velocity_advection_stencil_03() {
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_03::free();
}
}
