...
#include "mo_solve_nonhydro_stencil_67.hpp"
...
class mo_solve_nonhydro_stencil_67 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_67 has not been set up! make sure "
    ...
    gridtools::stencil::global_parameter rd_o_p0ref_gp{rd_o_p0ref_};
    fn_backend_t cuda_backend{};
    cuda_backend.stream = stream_;
    auto connectivities = gridtools::hymap::keys<>::make_values();
    generated::mo_solve_nonhydro_stencil_67(connectivities)(
        cuda_backend, rho_sid, theta_v_sid, exner_sid, rd_o_cvd_gp,
        rd_o_p0ref_gp, horizontalStart, horizontalEnd, verticalStart,
        verticalEnd);
#ifndef NDEBUG
  ...
  void copy_pointers(double *rho, double *theta_v, double *exner,
                     double rd_o_cvd, double rd_o_p0ref) {
    rho_ = rho;
    theta_v_ = theta_v;
    exner_ = exner;
    rd_o_cvd_ = rd_o_cvd;
    rd_o_p0ref_ = rd_o_p0ref;
  }
};
} // namespace cuda_ico
} // namespace dawn_generated
...
extern "C" {
void run_mo_solve_nonhydro_stencil_67(
    double *rho, double *theta_v, double *exner, double rd_o_cvd,
    double rd_o_p0ref, const int verticalStart, const int verticalEnd,
    const int horizontalStart, const int horizontalEnd) {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_67 s;
  s.copy_pointers(rho, theta_v, exner, rd_o_cvd, rd_o_p0ref);
  s.run(verticalStart, verticalEnd, horizontalStart, horizontalEnd);
  return;
}

bool verify_mo_solve_nonhydro_stencil_67(
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_67::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_67::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_67::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int theta_v_kSize = dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_67::
      get_theta_v_KSize();
  ...
  MetricsSerialiser serialiser_theta_v(
      stencilMetrics, metricsNameFromEnvVar("SLURM_JOB_ID"),
      "mo_solve_nonhydro_stencil_67", "theta_v");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_cells(0, (mesh.NumCells - 1), theta_v_kSize,
                          ...
                          "mo_solve_nonhydro_stencil_67", "theta_v", iteration);
    serialize_dense_cells(
        0, (mesh.NumCells - 1), theta_v_kSize, (mesh.CellStride), theta_v_dsl,
        "mo_solve_nonhydro_stencil_67", "theta_v_dsl", iteration);
    std::cout << "[DSL] serializing theta_v as error is high.\n" << std::flush;
#endif
  }
  int exner_kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_67::get_exner_KSize();
  stencilMetrics = ::dawn::verify_field(
  ...
  MetricsSerialiser serialiser_exner(stencilMetrics,
                                     ...
                                     "mo_solve_nonhydro_stencil_67", "exner");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_cells(0, (mesh.NumCells - 1), exner_kSize,
                          ...
                          "mo_solve_nonhydro_stencil_67", "exner", iteration);
    serialize_dense_cells(
        0, (mesh.NumCells - 1), exner_kSize, (mesh.CellStride), exner_dsl,
        "mo_solve_nonhydro_stencil_67", "exner_dsl", iteration);
    std::cout << "[DSL] serializing exner as error is high.\n" << std::flush;
#endif
  };
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_67", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_67(
    double *rho, double *theta_v, double *exner, double rd_o_cvd,
    double rd_o_p0ref, double *theta_v_before, double *exner_before,
    const int verticalStart, const int verticalEnd, const int horizontalStart,
    const int horizontalEnd, const double theta_v_rel_tol,
    const double theta_v_abs_tol, const double exner_rel_tol,
    const double exner_abs_tol) {
  static int iteration = 0;
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_67 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_67(rho, theta_v_before, exner_before, rd_o_cvd,
                                   rd_o_p0ref, verticalStart, verticalEnd,
                                   horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_67 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_67...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_67(theta_v_before, theta_v, exner_before,
...
void setup_mo_solve_nonhydro_stencil_67(dawn::GlobalGpuTriMesh *mesh,
                                        int k_size, cudaStream_t stream,
                                        const int theta_v_k_size,
                                        const int exner_k_size) {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_67::setup(
      mesh, k_size, stream, theta_v_k_size, exner_k_size);
}

void free_mo_solve_nonhydro_stencil_67() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_67::free();
}
}
