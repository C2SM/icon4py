...
#include "mo_solve_nonhydro_stencil_40.hpp"
...
class mo_solve_nonhydro_stencil_40 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_40 has not been set up! make sure "
    ...
    auto e_bln_c_s_sid_comp = sid::composite::keys<
        ...
        integral_constant<int, 2>>::make_values(e_bln_c_s_sid_0,
                                                e_bln_c_s_sid_1,
                                                e_bln_c_s_sid_2);
    generated::mo_solve_nonhydro_stencil_40(connectivities)(
        cuda_backend, e_bln_c_s_sid_comp, z_w_concorr_me_sid, wgtfacq_c_sid,
  ...
  void copy_pointers(double *e_bln_c_s, double *z_w_concorr_me,
                     double *wgtfacq_c, double *w_concorr_c) {
...
extern "C" {
void run_mo_solve_nonhydro_stencil_40(double *e_bln_c_s, double *z_w_concorr_me,
                                      double *wgtfacq_c, double *w_concorr_c,
                                      const int verticalStart,
                                      const int verticalEnd,
                                      const int horizontalStart,
                                      const int horizontalEnd) {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_40 s;
  s.copy_pointers(e_bln_c_s, z_w_concorr_me, wgtfacq_c, w_concorr_c);
  s.run(verticalStart, verticalEnd, horizontalStart, horizontalEnd);
  return;
}

bool verify_mo_solve_nonhydro_stencil_40(const double *w_concorr_c_dsl,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_40::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_40::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_40::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int w_concorr_c_kSize = dawn_generated::cuda_ico::
      mo_solve_nonhydro_stencil_40::get_w_concorr_c_KSize();
  stencilMetrics =
  ...
  MetricsSerialiser serialiser_w_concorr_c(
      ...
      "mo_solve_nonhydro_stencil_40", "w_concorr_c");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_cells(
        ...
        w_concorr_c, "mo_solve_nonhydro_stencil_40", "w_concorr_c", iteration);
    serialize_dense_cells(0, (mesh.NumCells - 1), w_concorr_c_kSize,
                          (mesh.CellStride), w_concorr_c_dsl,
                          "mo_solve_nonhydro_stencil_40", "w_concorr_c_dsl",
                          iteration);
...
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_40", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_40(
    double *e_bln_c_s, double *z_w_concorr_me, double *wgtfacq_c,
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_40 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_40(e_bln_c_s, z_w_concorr_me, wgtfacq_c,
                                   w_concorr_c_before, verticalStart,
                                   verticalEnd, horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_40 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_40...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_40(w_concorr_c_before, w_concorr_c,
...
void setup_mo_solve_nonhydro_stencil_40(dawn::GlobalGpuTriMesh *mesh,
                                        int k_size, cudaStream_t stream,
                                        const int w_concorr_c_k_size) {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_40::setup(
      mesh, k_size, stream, w_concorr_c_k_size);
}

void free_mo_solve_nonhydro_stencil_40() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_40::free();
}
}
