...
#include "mo_nh_diffusion_stencil_13.hpp"
...
class mo_nh_diffusion_stencil_13 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  static void setup(const dawn::GlobalGpuTriMesh *mesh, int kSize,
                    cudaStream_t stream, const int z_nabla2_e_kSize) {
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_nh_diffusion_stencil_13 has not been set up! make sure "
    ...
    generated::mo_nh_diffusion_stencil_13(connectivities)(
        cuda_backend, kh_smag_e_sid, inv_dual_edge_length_sid, theta_v_sid,
        z_nabla2_e_sid, horizontalStart, horizontalEnd, verticalStart,
        verticalEnd);
#ifndef NDEBUG
  ...
  void copy_pointers(double *kh_smag_e, double *inv_dual_edge_length,
                     double *theta_v, double *z_nabla2_e) {
...
void run_mo_nh_diffusion_stencil_13(
    ...
    const int horizontalStart, const int horizontalEnd) {
  dawn_generated::cuda_ico::mo_nh_diffusion_stencil_13 s;
  s.copy_pointers(kh_smag_e, inv_dual_edge_length, theta_v, z_nabla2_e);
...
bool verify_mo_nh_diffusion_stencil_13(const double *z_nabla2_e_dsl,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_nh_diffusion_stencil_13::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_nh_diffusion_stencil_13::getStream();
  int kSize = dawn_generated::cuda_ico::mo_nh_diffusion_stencil_13::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int z_nabla2_e_kSize = dawn_generated::cuda_ico::mo_nh_diffusion_stencil_13::
  ...
  MetricsSerialiser serialiser_z_nabla2_e(
      ...
      "mo_nh_diffusion_stencil_13", "z_nabla2_e");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_edges(
        ...
        "mo_nh_diffusion_stencil_13", "z_nabla2_e", iteration);
    serialize_dense_edges(0, (mesh.NumEdges - 1), z_nabla2_e_kSize,
                          (mesh.EdgeStride), z_nabla2_e_dsl,
                          "mo_nh_diffusion_stencil_13", "z_nabla2_e_dsl",
                          iteration);
...
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_nh_diffusion_stencil_13", iteration);
#endif
...
void run_and_verify_mo_nh_diffusion_stencil_13(
  ...
  std::cout << "[DSL] Running stencil mo_nh_diffusion_stencil_13 (" << iteration
            << ") ...\n"
            << std::flush;
  run_mo_nh_diffusion_stencil_13(kh_smag_e, inv_dual_edge_length, theta_v,
                                 z_nabla2_e_before, verticalStart, verticalEnd,
                                 horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_nh_diffusion_stencil_13 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_nh_diffusion_stencil_13...\n"
            << std::flush;
  verify_mo_nh_diffusion_stencil_13(z_nabla2_e_before, z_nabla2_e,
                                    z_nabla2_e_rel_tol, z_nabla2_e_abs_tol,
                                    iteration);
...
void setup_mo_nh_diffusion_stencil_13(dawn::GlobalGpuTriMesh *mesh, int k_size,
  ...
  dawn_generated::cuda_ico::mo_nh_diffusion_stencil_13::setup(
      mesh, k_size, stream, z_nabla2_e_k_size);
}

void free_mo_nh_diffusion_stencil_13() {
  dawn_generated::cuda_ico::mo_nh_diffusion_stencil_13::free();
}
}
