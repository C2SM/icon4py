...
#include "mo_nh_diffusion_stencil_14.hpp"
...
class mo_nh_diffusion_stencil_14 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
private:
  ...
  inline static int z_temp_kSize_;
...
public:
  ...
  static int get_z_temp_KSize() { return z_temp_kSize_; }

  static void free() {}

  static void setup(const dawn::GlobalGpuTriMesh *mesh, int kSize,
                    cudaStream_t stream, const int z_temp_kSize) {
    mesh_ = GpuTriMesh(mesh);
    kSize_ = kSize;
    is_setup_ = true;
    stream_ = stream;
    z_temp_kSize_ = z_temp_kSize;
  }
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_nh_diffusion_stencil_14 has not been set up! make sure "
    ...
    neighbor_table_4new_sparse<3> c2ce_ptr{};
    auto connectivities =
        gridtools::hymap::keys<generated::C2E_t,
                               generated::C2CE_t>::make_values(ce_ptr,
                                                               c2ce_ptr);
    generated::mo_nh_diffusion_stencil_14(connectivities)(
        cuda_backend, z_nabla2_e_sid, geofac_div_sid, z_temp_sid,
  ...
  void copy_pointers(double *z_nabla2_e, double *geofac_div, double *z_temp) {
...
void run_mo_nh_diffusion_stencil_14(double *z_nabla2_e, double *geofac_div,
  ...
  dawn_generated::cuda_ico::mo_nh_diffusion_stencil_14 s;
  s.copy_pointers(z_nabla2_e, geofac_div, z_temp);
...
bool verify_mo_nh_diffusion_stencil_14(const double *z_temp_dsl,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_nh_diffusion_stencil_14::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_nh_diffusion_stencil_14::getStream();
  int kSize = dawn_generated::cuda_ico::mo_nh_diffusion_stencil_14::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int z_temp_kSize =
      dawn_generated::cuda_ico::mo_nh_diffusion_stencil_14::get_z_temp_KSize();
  stencilMetrics = ::dawn::verify_field(
      stream, (mesh.CellStride) * z_temp_kSize, z_temp_dsl, z_temp, "z_temp",
      z_temp_rel_tol, z_temp_abs_tol, iteration);
#ifdef __SERIALIZE_METRICS
  MetricsSerialiser serialiser_z_temp(stencilMetrics,
                                      metricsNameFromEnvVar("SLURM_JOB_ID"),
                                      "mo_nh_diffusion_stencil_14", "z_temp");
  serialiser_z_temp.writeJson(iteration);
#endif
  if (!stencilMetrics.isValid) {
#ifdef __SERIALIZE_ON_ERROR
    serialize_dense_cells(0, (mesh.NumCells - 1), z_temp_kSize,
                          (mesh.CellStride), z_temp,
                          "mo_nh_diffusion_stencil_14", "z_temp", iteration);
    serialize_dense_cells(
        0, (mesh.NumCells - 1), z_temp_kSize, (mesh.CellStride), z_temp_dsl,
        "mo_nh_diffusion_stencil_14", "z_temp_dsl", iteration);
    std::cout << "[DSL] serializing z_temp as error is high.\n" << std::flush;
#endif
  };
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_nh_diffusion_stencil_14", iteration);
#endif
...
void run_and_verify_mo_nh_diffusion_stencil_14(
  ...
  std::cout << "[DSL] Running stencil mo_nh_diffusion_stencil_14 (" << iteration
            << ") ...\n"
            << std::flush;
  run_mo_nh_diffusion_stencil_14(z_nabla2_e, geofac_div, z_temp_before,
                                 verticalStart, verticalEnd, horizontalStart,
                                 horizontalEnd);

  std::cout << "[DSL] mo_nh_diffusion_stencil_14 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_nh_diffusion_stencil_14...\n"
            << std::flush;
  verify_mo_nh_diffusion_stencil_14(z_temp_before, z_temp, z_temp_rel_tol,
                                    z_temp_abs_tol, iteration);
...
void setup_mo_nh_diffusion_stencil_14(dawn::GlobalGpuTriMesh *mesh, int k_size,
  ...
  dawn_generated::cuda_ico::mo_nh_diffusion_stencil_14::setup(
      mesh, k_size, stream, z_temp_k_size);
}

void free_mo_nh_diffusion_stencil_14() {
  dawn_generated::cuda_ico::mo_nh_diffusion_stencil_14::free();
}
}
