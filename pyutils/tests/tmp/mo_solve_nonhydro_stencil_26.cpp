...
#include "mo_solve_nonhydro_stencil_26.hpp"
...
class mo_solve_nonhydro_stencil_26 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_26 has not been set up! make sure "
    ...
    gridtools::stencil::global_parameter scal_divdamp_o2_gp{scal_divdamp_o2_};
    fn_backend_t cuda_backend{};
    cuda_backend.stream = stream_;
    auto connectivities = gridtools::hymap::keys<>::make_values();
    generated::mo_solve_nonhydro_stencil_26(connectivities)(
        cuda_backend, z_graddiv_vn_sid, vn_sid, scal_divdamp_o2_gp,
  ...
  void copy_pointers(double *z_graddiv_vn, double *vn, double scal_divdamp_o2) {
    z_graddiv_vn_ = z_graddiv_vn;
    vn_ = vn;
    scal_divdamp_o2_ = scal_divdamp_o2;
  }
};
} // namespace cuda_ico
} // namespace dawn_generated
...
void run_mo_solve_nonhydro_stencil_26(double *z_graddiv_vn, double *vn,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_26 s;
  s.copy_pointers(z_graddiv_vn, vn, scal_divdamp_o2);
...
bool verify_mo_solve_nonhydro_stencil_26(const double *vn_dsl, const double *vn,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_26::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_26::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_26::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int vn_kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_26::get_vn_KSize();
  stencilMetrics =
  ...
  MetricsSerialiser serialiser_vn(stencilMetrics,
                                  ...
                                  "mo_solve_nonhydro_stencil_26", "vn");
  serialiser_vn.writeJson(iteration);
#endif
  if (!stencilMetrics.isValid) {
#ifdef __SERIALIZE_ON_ERROR
    serialize_dense_edges(0, (mesh.NumEdges - 1), vn_kSize, (mesh.EdgeStride),
                          vn, "mo_solve_nonhydro_stencil_26", "vn", iteration);
    serialize_dense_edges(0, (mesh.NumEdges - 1), vn_kSize, (mesh.EdgeStride),
                          vn_dsl, "mo_solve_nonhydro_stencil_26", "vn_dsl",
                          iteration);
    std::cout << "[DSL] serializing vn as error is high.\n" << std::flush;
#endif
  };
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_26", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_26(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_26 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_26(z_graddiv_vn, vn_before, scal_divdamp_o2,
                                   verticalStart, verticalEnd, horizontalStart,
                                   horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_26 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_26...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_26(vn_before, vn, vn_rel_tol, vn_abs_tol,
                                      iteration);
...
void setup_mo_solve_nonhydro_stencil_26(dawn::GlobalGpuTriMesh *mesh,
                                        ...
                                        const int vn_k_size) {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_26::setup(
      mesh, k_size, stream, vn_k_size);
}

void free_mo_solve_nonhydro_stencil_26() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_26::free();
}
}
