...
#include "mo_solve_nonhydro_stencil_46.hpp"
...
class mo_solve_nonhydro_stencil_46 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  static void setup(const dawn::GlobalGpuTriMesh *mesh, int kSize,
                    cudaStream_t stream, const int w_nnew_kSize,
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_46 has not been set up! make sure "
    ...
    generated::mo_solve_nonhydro_stencil_46(connectivities)(
        cuda_backend, w_nnew_sid, z_contr_w_fl_l_sid, horizontalStart,
  ...
  void copy_pointers(double *w_nnew, double *z_contr_w_fl_l) {
    w_nnew_ = w_nnew;
...
extern "C" {
void run_mo_solve_nonhydro_stencil_46(double *w_nnew, double *z_contr_w_fl_l,
                                      const int verticalStart,
                                      const int verticalEnd,
                                      const int horizontalStart,
                                      const int horizontalEnd) {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_46 s;
  s.copy_pointers(w_nnew, z_contr_w_fl_l);
  s.run(verticalStart, verticalEnd, horizontalStart, horizontalEnd);
  return;
}

bool verify_mo_solve_nonhydro_stencil_46(
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_46::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_46::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_46::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int w_nnew_kSize = dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_46::
      get_w_nnew_KSize();
  ...
  MetricsSerialiser serialiser_w_nnew(stencilMetrics,
                                      ...
                                      "mo_solve_nonhydro_stencil_46", "w_nnew");
  ...
  MetricsSerialiser serialiser_z_contr_w_fl_l(
      ...
      "mo_solve_nonhydro_stencil_46", "z_contr_w_fl_l");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_cells(0, (mesh.NumCells - 1), z_contr_w_fl_l_kSize,
                          ...
                          "mo_solve_nonhydro_stencil_46", "z_contr_w_fl_l_dsl",
                          iteration);
...
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_46", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_46(
    double *w_nnew, double *z_contr_w_fl_l, double *w_nnew_before,
    double *z_contr_w_fl_l_before, const int verticalStart,
    const int verticalEnd, const int horizontalStart, const int horizontalEnd,
    const double w_nnew_rel_tol, const double w_nnew_abs_tol,
    const double z_contr_w_fl_l_rel_tol, const double z_contr_w_fl_l_abs_tol) {
  static int iteration = 0;
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_46 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_46(w_nnew_before, z_contr_w_fl_l_before,
                                   verticalStart, verticalEnd, horizontalStart,
                                   horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_46 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_46...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_46(
      w_nnew_before, w_nnew, z_contr_w_fl_l_before, z_contr_w_fl_l,
      w_nnew_rel_tol, w_nnew_abs_tol, z_contr_w_fl_l_rel_tol,
      z_contr_w_fl_l_abs_tol, iteration);

  iteration++;
}

void setup_mo_solve_nonhydro_stencil_46(dawn::GlobalGpuTriMesh *mesh,
                                        int k_size, cudaStream_t stream,
                                        const int w_nnew_k_size,
                                        const int z_contr_w_fl_l_k_size) {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_46::setup(
      mesh, k_size, stream, w_nnew_k_size, z_contr_w_fl_l_k_size);
}

void free_mo_solve_nonhydro_stencil_46() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_46::free();
}
}
