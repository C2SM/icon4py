...
#include "mo_velocity_advection_stencil_15.hpp"
...
class mo_velocity_advection_stencil_15 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
private:
  ...
  inline static int z_w_con_c_full_kSize_;
...
public:
  ...
  static int get_z_w_con_c_full_KSize() { return z_w_con_c_full_kSize_; }

  static void free() {}

  static void setup(const dawn::GlobalGpuTriMesh *mesh, int kSize,
                    cudaStream_t stream, const int z_w_con_c_full_kSize) {
    mesh_ = GpuTriMesh(mesh);
    kSize_ = kSize;
    is_setup_ = true;
    stream_ = stream;
    z_w_con_c_full_kSize_ = z_w_con_c_full_kSize;
  }
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_velocity_advection_stencil_15 has not been set up! make sure "
    ...
    generated::mo_velocity_advection_stencil_15(connectivities)(
        cuda_backend, z_w_con_c_sid, z_w_con_c_full_sid, horizontalStart,
  ...
  void copy_pointers(double *z_w_con_c, double *z_w_con_c_full) {
...
void run_mo_velocity_advection_stencil_15(
  ...
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_15 s;
  s.copy_pointers(z_w_con_c, z_w_con_c_full);
...
bool verify_mo_velocity_advection_stencil_15(
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_15::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_15::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_15::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int z_w_con_c_full_kSize = dawn_generated::cuda_ico::
      mo_velocity_advection_stencil_15::get_z_w_con_c_full_KSize();
  stencilMetrics = ::dawn::verify_field(
      stream, (mesh.CellStride) * z_w_con_c_full_kSize, z_w_con_c_full_dsl,
      z_w_con_c_full, "z_w_con_c_full", z_w_con_c_full_rel_tol,
      z_w_con_c_full_abs_tol, iteration);
#ifdef __SERIALIZE_METRICS
  MetricsSerialiser serialiser_z_w_con_c_full(
      stencilMetrics, metricsNameFromEnvVar("SLURM_JOB_ID"),
      "mo_velocity_advection_stencil_15", "z_w_con_c_full");
  serialiser_z_w_con_c_full.writeJson(iteration);
#endif
  if (!stencilMetrics.isValid) {
#ifdef __SERIALIZE_ON_ERROR
    serialize_dense_cells(0, (mesh.NumCells - 1), z_w_con_c_full_kSize,
                          (mesh.CellStride), z_w_con_c_full,
                          "mo_velocity_advection_stencil_15", "z_w_con_c_full",
                          iteration);
    serialize_dense_cells(0, (mesh.NumCells - 1), z_w_con_c_full_kSize,
                          (mesh.CellStride), z_w_con_c_full_dsl,
                          "mo_velocity_advection_stencil_15",
                          "z_w_con_c_full_dsl", iteration);
    std::cout << "[DSL] serializing z_w_con_c_full as error is high.\n"
              << std::flush;
#endif
  };
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_velocity_advection_stencil_15", iteration);
#endif
...
void run_and_verify_mo_velocity_advection_stencil_15(
  ...
  std::cout << "[DSL] Running stencil mo_velocity_advection_stencil_15 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_velocity_advection_stencil_15(z_w_con_c, z_w_con_c_full_before,
                                       verticalStart, verticalEnd,
                                       horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_velocity_advection_stencil_15 run time: " << time
            << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_velocity_advection_stencil_15...\n"
            << std::flush;
  verify_mo_velocity_advection_stencil_15(z_w_con_c_full_before, z_w_con_c_full,
                                          z_w_con_c_full_rel_tol,
                                          z_w_con_c_full_abs_tol, iteration);
...
void setup_mo_velocity_advection_stencil_15(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_15::setup(
      mesh, k_size, stream, z_w_con_c_full_k_size);
}

void free_mo_velocity_advection_stencil_15() {
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_15::free();
}
}
