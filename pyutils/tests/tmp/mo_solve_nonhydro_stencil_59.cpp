...
#include "mo_solve_nonhydro_stencil_59.hpp"
...
class mo_solve_nonhydro_stencil_59 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_59 has not been set up! make sure "
    ...
    generated::mo_solve_nonhydro_stencil_59(connectivities)(
        cuda_backend, exner_sid, exner_dyn_incr_sid, horizontalStart,
  ...
  void copy_pointers(double *exner, double *exner_dyn_incr) {
    exner_ = exner;
...
extern "C" {
void run_mo_solve_nonhydro_stencil_59(double *exner, double *exner_dyn_incr,
                                      const int verticalStart,
                                      const int verticalEnd,
                                      const int horizontalStart,
                                      const int horizontalEnd) {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_59 s;
  s.copy_pointers(exner, exner_dyn_incr);
  s.run(verticalStart, verticalEnd, horizontalStart, horizontalEnd);
  return;
}

bool verify_mo_solve_nonhydro_stencil_59(const double *exner_dyn_incr_dsl,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_59::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_59::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_59::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int exner_dyn_incr_kSize = dawn_generated::cuda_ico::
      mo_solve_nonhydro_stencil_59::get_exner_dyn_incr_KSize();
  ...
  MetricsSerialiser serialiser_exner_dyn_incr(
      ...
      "mo_solve_nonhydro_stencil_59", "exner_dyn_incr");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_cells(0, (mesh.NumCells - 1), exner_dyn_incr_kSize,
                          ...
                          "mo_solve_nonhydro_stencil_59", "exner_dyn_incr_dsl",
                          iteration);
...
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_59", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_59(
    double *exner, double *exner_dyn_incr, double *exner_dyn_incr_before,
    const int verticalStart, const int verticalEnd, const int horizontalStart,
    const int horizontalEnd, const double exner_dyn_incr_rel_tol,
    const double exner_dyn_incr_abs_tol) {
  static int iteration = 0;
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_59 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_59(exner, exner_dyn_incr_before, verticalStart,
                                   verticalEnd, horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_59 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_59...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_59(exner_dyn_incr_before, exner_dyn_incr,
...
void setup_mo_solve_nonhydro_stencil_59(dawn::GlobalGpuTriMesh *mesh,
                                        int k_size, cudaStream_t stream,
                                        const int exner_dyn_incr_k_size) {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_59::setup(
      mesh, k_size, stream, exner_dyn_incr_k_size);
}

void free_mo_solve_nonhydro_stencil_59() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_59::free();
}
}
