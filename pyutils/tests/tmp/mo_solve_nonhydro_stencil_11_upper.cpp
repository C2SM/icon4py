...
#include "mo_solve_nonhydro_stencil_11_upper.hpp"
...
class mo_solve_nonhydro_stencil_11_upper {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_11_upper has not been set up! make "
    ...
    auto z_rth_pr_sid = get_sid(
        z_rth_pr_,
        gridtools::hymap::keys<
            unstructured::dim::horizontal,
            unstructured::dim::vertical>::make_values(1, mesh_.CellStride));

    auto theta_ref_ic_sid = get_sid(
        theta_ref_ic_,
        gridtools::hymap::keys<
            unstructured::dim::horizontal,
            unstructured::dim::vertical>::make_values(1, mesh_.CellStride));
    ...
    generated::mo_solve_nonhydro_stencil_11_upper(connectivities)(
        cuda_backend, wgtfacq_c_sid, z_rth_pr_sid, theta_ref_ic_sid,
        z_theta_v_pr_ic_sid, theta_v_ic_sid, horizontalStart, horizontalEnd,
  ...
  void copy_pointers(double *wgtfacq_c, double *z_rth_pr, double *theta_ref_ic,
                     double *z_theta_v_pr_ic, double *theta_v_ic) {
    wgtfacq_c_ = wgtfacq_c;
    z_rth_pr_ = z_rth_pr;
    theta_ref_ic_ = theta_ref_ic;
...
void run_mo_solve_nonhydro_stencil_11_upper(
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_11_upper s;
  s.copy_pointers(wgtfacq_c, z_rth_pr, theta_ref_ic, z_theta_v_pr_ic,
                  theta_v_ic);
...
bool verify_mo_solve_nonhydro_stencil_11_upper(
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_11_upper::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_11_upper::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_11_upper::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int z_theta_v_pr_ic_kSize = dawn_generated::cuda_ico::
      mo_solve_nonhydro_stencil_11_upper::get_z_theta_v_pr_ic_KSize();
  ...
  MetricsSerialiser serialiser_z_theta_v_pr_ic(
      ...
      "mo_solve_nonhydro_stencil_11_upper", "z_theta_v_pr_ic");
  ...
  MetricsSerialiser serialiser_theta_v_ic(
      stencilMetrics, metricsNameFromEnvVar("SLURM_JOB_ID"),
      "mo_solve_nonhydro_stencil_11_upper", "theta_v_ic");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_cells(
        ...
        "mo_solve_nonhydro_stencil_11_upper", "theta_v_ic", iteration);
    serialize_dense_cells(0, (mesh.NumCells - 1), theta_v_ic_kSize,
                          (mesh.CellStride), theta_v_ic_dsl,
                          "mo_solve_nonhydro_stencil_11_upper",
                          "theta_v_ic_dsl", iteration);
    std::cout << "[DSL] serializing theta_v_ic as error is high.\n"
              << std::flush;
#endif
  };
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_11_upper", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_11_upper(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_11_upper ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_11_upper(
      wgtfacq_c, z_rth_pr, theta_ref_ic, z_theta_v_pr_ic_before,
      theta_v_ic_before, verticalStart, verticalEnd, horizontalStart,
      horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_11_upper run time: " << time
            << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_11_upper...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_11_upper(
      z_theta_v_pr_ic_before, z_theta_v_pr_ic, theta_v_ic_before, theta_v_ic,
      z_theta_v_pr_ic_rel_tol, z_theta_v_pr_ic_abs_tol, theta_v_ic_rel_tol,
      theta_v_ic_abs_tol, iteration);
...
void setup_mo_solve_nonhydro_stencil_11_upper(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_11_upper::setup(
      mesh, k_size, stream, z_theta_v_pr_ic_k_size, theta_v_ic_k_size);
}

void free_mo_solve_nonhydro_stencil_11_upper() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_11_upper::free();
}
}
