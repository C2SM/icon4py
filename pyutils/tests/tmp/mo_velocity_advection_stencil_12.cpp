...
#include "mo_velocity_advection_stencil_12.hpp"
...
class mo_velocity_advection_stencil_12 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_velocity_advection_stencil_12 has not been set up! make sure "
    ...
    generated::mo_velocity_advection_stencil_12(connectivities)(
        cuda_backend, z_w_con_c_sid, horizontalStart, horizontalEnd,
  ...
  void copy_pointers(double *z_w_con_c) { z_w_con_c_ = z_w_con_c; }
};
} // namespace cuda_ico
} // namespace dawn_generated

extern "C" {
void run_mo_velocity_advection_stencil_12(double *z_w_con_c,
                                          const int verticalStart,
                                          const int verticalEnd,
                                          const int horizontalStart,
                                          const int horizontalEnd) {
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_12 s;
  s.copy_pointers(z_w_con_c);
  s.run(verticalStart, verticalEnd, horizontalStart, horizontalEnd);
  return;
}

bool verify_mo_velocity_advection_stencil_12(const double *z_w_con_c_dsl,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_12::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_12::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_12::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int z_w_con_c_kSize = dawn_generated::cuda_ico::
      mo_velocity_advection_stencil_12::get_z_w_con_c_KSize();
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_cells(
        ...
        "mo_velocity_advection_stencil_12", "z_w_con_c", iteration);
    serialize_dense_cells(0, (mesh.NumCells - 1), z_w_con_c_kSize,
                          (mesh.CellStride), z_w_con_c_dsl,
                          "mo_velocity_advection_stencil_12", "z_w_con_c_dsl",
                          iteration);
...
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_velocity_advection_stencil_12", iteration);
#endif
...
void run_and_verify_mo_velocity_advection_stencil_12(
    double *z_w_con_c, double *z_w_con_c_before, const int verticalStart,
  ...
  std::cout << "[DSL] Running stencil mo_velocity_advection_stencil_12 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_velocity_advection_stencil_12(z_w_con_c_before, verticalStart,
                                       verticalEnd, horizontalStart,
                                       horizontalEnd);

  std::cout << "[DSL] mo_velocity_advection_stencil_12 run time: " << time
            << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_velocity_advection_stencil_12...\n"
            << std::flush;
  verify_mo_velocity_advection_stencil_12(z_w_con_c_before, z_w_con_c,
...
void setup_mo_velocity_advection_stencil_12(dawn::GlobalGpuTriMesh *mesh,
                                            int k_size, cudaStream_t stream,
                                            const int z_w_con_c_k_size) {
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_12::setup(
      mesh, k_size, stream, z_w_con_c_k_size);
}

void free_mo_velocity_advection_stencil_12() {
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_12::free();
}
}
