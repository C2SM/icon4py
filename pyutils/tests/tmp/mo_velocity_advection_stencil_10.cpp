...
#include "mo_velocity_advection_stencil_10.hpp"
...
class mo_velocity_advection_stencil_10 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_velocity_advection_stencil_10 has not been set up! make sure "
    ...
    generated::mo_velocity_advection_stencil_10(connectivities)(
        cuda_backend, z_w_concorr_mc_sid, wgtfac_c_sid, w_concorr_c_sid,
  ...
  void copy_pointers(double *z_w_concorr_mc, double *wgtfac_c,
                     double *w_concorr_c) {
...
void run_mo_velocity_advection_stencil_10(double *z_w_concorr_mc,
  ...
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_10 s;
  s.copy_pointers(z_w_concorr_mc, wgtfac_c, w_concorr_c);
...
bool verify_mo_velocity_advection_stencil_10(const double *w_concorr_c_dsl,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_10::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_10::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_10::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int w_concorr_c_kSize = dawn_generated::cuda_ico::
      mo_velocity_advection_stencil_10::get_w_concorr_c_KSize();
  stencilMetrics =
...
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_velocity_advection_stencil_10", iteration);
#endif
...
void run_and_verify_mo_velocity_advection_stencil_10(
  ...
  std::cout << "[DSL] Running stencil mo_velocity_advection_stencil_10 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_velocity_advection_stencil_10(
      z_w_concorr_mc, wgtfac_c, w_concorr_c_before, verticalStart, verticalEnd,
      horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_velocity_advection_stencil_10 run time: " << time
            << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_velocity_advection_stencil_10...\n"
            << std::flush;
  verify_mo_velocity_advection_stencil_10(w_concorr_c_before, w_concorr_c,
                                          w_concorr_c_rel_tol,
                                          w_concorr_c_abs_tol, iteration);
...
void setup_mo_velocity_advection_stencil_10(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_10::setup(
      mesh, k_size, stream, w_concorr_c_k_size);
}

void free_mo_velocity_advection_stencil_10() {
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_10::free();
}
}
