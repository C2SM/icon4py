...
#include "mo_solve_nonhydro_stencil_37.hpp"
...
class mo_solve_nonhydro_stencil_37 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_37 has not been set up! make sure "
    ...
    generated::mo_solve_nonhydro_stencil_37(connectivities)(
...
void run_mo_solve_nonhydro_stencil_37(double *vn, double *vt, double *vn_ie,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_37 s;
...
bool verify_mo_solve_nonhydro_stencil_37(
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_37::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_37::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_37::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int vn_ie_kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_37::get_vn_ie_KSize();
  ...
  MetricsSerialiser serialiser_vn_ie(stencilMetrics,
                                     ...
                                     "mo_solve_nonhydro_stencil_37", "vn_ie");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_edges(0, (mesh.NumEdges - 1), vn_ie_kSize,
                          ...
                          "mo_solve_nonhydro_stencil_37", "vn_ie", iteration);
    serialize_dense_edges(
        0, (mesh.NumEdges - 1), vn_ie_kSize, (mesh.EdgeStride), vn_ie_dsl,
        "mo_solve_nonhydro_stencil_37", "vn_ie_dsl", iteration);
    std::cout << "[DSL] serializing vn_ie as error is high.\n" << std::flush;
#endif
  }
  int z_vt_ie_kSize = dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_37::
      get_z_vt_ie_KSize();
  stencilMetrics = ::dawn::verify_field(
  ...
  MetricsSerialiser serialiser_z_vt_ie(
      ...
      "mo_solve_nonhydro_stencil_37", "z_vt_ie");
  ...
  MetricsSerialiser serialiser_z_kin_hor_e(
      stencilMetrics, metricsNameFromEnvVar("SLURM_JOB_ID"),
      "mo_solve_nonhydro_stencil_37", "z_kin_hor_e");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_edges(
        ...
        z_kin_hor_e, "mo_solve_nonhydro_stencil_37", "z_kin_hor_e", iteration);
    serialize_dense_edges(0, (mesh.NumEdges - 1), z_kin_hor_e_kSize,
                          (mesh.EdgeStride), z_kin_hor_e_dsl,
                          "mo_solve_nonhydro_stencil_37", "z_kin_hor_e_dsl",
                          iteration);
...
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_37", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_37(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_37 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_37(vn, vt, vn_ie_before, z_vt_ie_before,
                                   z_kin_hor_e_before, verticalStart,
                                   verticalEnd, horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_37 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_37...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_37(
...
void setup_mo_solve_nonhydro_stencil_37(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_37::setup(
      mesh, k_size, stream, vn_ie_k_size, z_vt_ie_k_size, z_kin_hor_e_k_size);
}

void free_mo_solve_nonhydro_stencil_37() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_37::free();
}
}
