...
#include "mo_velocity_advection_stencil_17.hpp"
...
class mo_velocity_advection_stencil_17 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_velocity_advection_stencil_17 has not been set up! make sure "
    ...
    auto e_bln_c_s_sid_comp = sid::composite::keys<
        ...
        integral_constant<int, 2>>::make_values(e_bln_c_s_sid_0,
                                                e_bln_c_s_sid_1,
                                                e_bln_c_s_sid_2);
    generated::mo_velocity_advection_stencil_17(connectivities)(
        cuda_backend, e_bln_c_s_sid_comp, z_v_grad_w_sid, ddt_w_adv_sid,
  ...
  void copy_pointers(double *e_bln_c_s, double *z_v_grad_w, double *ddt_w_adv) {
...
void run_mo_velocity_advection_stencil_17(double *e_bln_c_s, double *z_v_grad_w,
  ...
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_17 s;
  s.copy_pointers(e_bln_c_s, z_v_grad_w, ddt_w_adv);
...
bool verify_mo_velocity_advection_stencil_17(const double *ddt_w_adv_dsl,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_17::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_17::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_17::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int ddt_w_adv_kSize = dawn_generated::cuda_ico::
      mo_velocity_advection_stencil_17::get_ddt_w_adv_KSize();
  ...
  MetricsSerialiser serialiser_ddt_w_adv(
      ...
      "mo_velocity_advection_stencil_17", "ddt_w_adv");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_cells(
        ...
        "mo_velocity_advection_stencil_17", "ddt_w_adv", iteration);
    serialize_dense_cells(0, (mesh.NumCells - 1), ddt_w_adv_kSize,
                          (mesh.CellStride), ddt_w_adv_dsl,
                          "mo_velocity_advection_stencil_17", "ddt_w_adv_dsl",
                          iteration);
...
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_velocity_advection_stencil_17", iteration);
#endif
...
void run_and_verify_mo_velocity_advection_stencil_17(
  ...
  std::cout << "[DSL] Running stencil mo_velocity_advection_stencil_17 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_velocity_advection_stencil_17(e_bln_c_s, z_v_grad_w, ddt_w_adv_before,
                                       verticalStart, verticalEnd,
                                       horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_velocity_advection_stencil_17 run time: " << time
            << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_velocity_advection_stencil_17...\n"
            << std::flush;
  verify_mo_velocity_advection_stencil_17(ddt_w_adv_before, ddt_w_adv,
...
void setup_mo_velocity_advection_stencil_17(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_17::setup(
      mesh, k_size, stream, ddt_w_adv_k_size);
}

void free_mo_velocity_advection_stencil_17() {
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_17::free();
}
}
