...
#include "mo_solve_nonhydro_stencil_28.hpp"
...
class mo_solve_nonhydro_stencil_28 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_28 has not been set up! make sure "
    ...
    generated::mo_solve_nonhydro_stencil_28(connectivities)(
        cuda_backend, vn_incr_sid, vn_sid, iau_wgt_dyn_gp, horizontalStart,
  ...
  void copy_pointers(double *vn_incr, double *vn, double iau_wgt_dyn) {
    vn_incr_ = vn_incr;
    vn_ = vn;
...
void run_mo_solve_nonhydro_stencil_28(
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_28 s;
  s.copy_pointers(vn_incr, vn, iau_wgt_dyn);
...
bool verify_mo_solve_nonhydro_stencil_28(const double *vn_dsl, const double *vn,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_28::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_28::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_28::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int vn_kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_28::get_vn_KSize();
  stencilMetrics =
  ...
  MetricsSerialiser serialiser_vn(stencilMetrics,
                                  ...
                                  "mo_solve_nonhydro_stencil_28", "vn");
  serialiser_vn.writeJson(iteration);
#endif
  if (!stencilMetrics.isValid) {
#ifdef __SERIALIZE_ON_ERROR
    serialize_dense_edges(0, (mesh.NumEdges - 1), vn_kSize, (mesh.EdgeStride),
                          vn, "mo_solve_nonhydro_stencil_28", "vn", iteration);
    serialize_dense_edges(0, (mesh.NumEdges - 1), vn_kSize, (mesh.EdgeStride),
                          vn_dsl, "mo_solve_nonhydro_stencil_28", "vn_dsl",
                          iteration);
    std::cout << "[DSL] serializing vn as error is high.\n" << std::flush;
#endif
  };
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_28", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_28(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_28 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_28(vn_incr, vn_before, iau_wgt_dyn,
                                   verticalStart, verticalEnd, horizontalStart,
                                   horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_28 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_28...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_28(vn_before, vn, vn_rel_tol, vn_abs_tol,
                                      iteration);
...
void setup_mo_solve_nonhydro_stencil_28(dawn::GlobalGpuTriMesh *mesh,
                                        ...
                                        const int vn_k_size) {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_28::setup(
      mesh, k_size, stream, vn_k_size);
}

void free_mo_solve_nonhydro_stencil_28() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_28::free();
}
}
