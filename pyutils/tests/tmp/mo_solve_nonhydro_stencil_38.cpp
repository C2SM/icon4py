...
#include "mo_solve_nonhydro_stencil_38.hpp"
...
class mo_solve_nonhydro_stencil_38 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_38 has not been set up! make sure "
    ...
    generated::mo_solve_nonhydro_stencil_38(connectivities)(
        cuda_backend, vn_sid, wgtfacq_e_sid, vn_ie_sid, horizontalStart,
  ...
  void copy_pointers(double *vn, double *wgtfacq_e, double *vn_ie) {
    vn_ = vn;
    wgtfacq_e_ = wgtfacq_e;
    vn_ie_ = vn_ie;
  }
};
} // namespace cuda_ico
...
void run_mo_solve_nonhydro_stencil_38(double *vn, double *wgtfacq_e,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_38 s;
  s.copy_pointers(vn, wgtfacq_e, vn_ie);
...
bool verify_mo_solve_nonhydro_stencil_38(const double *vn_ie_dsl,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_38::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_38::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_38::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int vn_ie_kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_38::get_vn_ie_KSize();
  ...
  MetricsSerialiser serialiser_vn_ie(stencilMetrics,
                                     ...
                                     "mo_solve_nonhydro_stencil_38", "vn_ie");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_edges(0, (mesh.NumEdges - 1), vn_ie_kSize,
                          ...
                          "mo_solve_nonhydro_stencil_38", "vn_ie", iteration);
    serialize_dense_edges(
        0, (mesh.NumEdges - 1), vn_ie_kSize, (mesh.EdgeStride), vn_ie_dsl,
        "mo_solve_nonhydro_stencil_38", "vn_ie_dsl", iteration);
    std::cout << "[DSL] serializing vn_ie as error is high.\n" << std::flush;
#endif
  };
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_38", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_38(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_38 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_38(vn, wgtfacq_e, vn_ie_before, verticalStart,
                                   verticalEnd, horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_38 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_38...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_38(vn_ie_before, vn_ie, vn_ie_rel_tol,
                                      vn_ie_abs_tol, iteration);
...
void setup_mo_solve_nonhydro_stencil_38(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_38::setup(
      mesh, k_size, stream, vn_ie_k_size);
}

void free_mo_solve_nonhydro_stencil_38() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_38::free();
}
}
