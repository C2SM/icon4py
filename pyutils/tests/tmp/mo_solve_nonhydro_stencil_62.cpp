...
#include "mo_solve_nonhydro_stencil_62.hpp"
...
class mo_solve_nonhydro_stencil_62 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  static void setup(const dawn::GlobalGpuTriMesh *mesh, int kSize,
                    cudaStream_t stream, const int w_new_kSize) {
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_62 has not been set up! make sure "
    ...
    generated::mo_solve_nonhydro_stencil_62(connectivities)(
        cuda_backend, w_now_sid, grf_tend_w_sid, w_new_sid, dtime_gp,
  ...
  void copy_pointers(double *w_now, double *grf_tend_w, double *w_new,
                     double dtime) {
    w_now_ = w_now;
...
void run_mo_solve_nonhydro_stencil_62(double *w_now, double *grf_tend_w,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_62 s;
  s.copy_pointers(w_now, grf_tend_w, w_new, dtime);
...
bool verify_mo_solve_nonhydro_stencil_62(const double *w_new_dsl,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_62::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_62::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_62::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int w_new_kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_62::get_w_new_KSize();
  ...
  MetricsSerialiser serialiser_w_new(stencilMetrics,
                                     ...
                                     "mo_solve_nonhydro_stencil_62", "w_new");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_cells(0, (mesh.NumCells - 1), w_new_kSize,
                          ...
                          "mo_solve_nonhydro_stencil_62", "w_new", iteration);
    serialize_dense_cells(
        0, (mesh.NumCells - 1), w_new_kSize, (mesh.CellStride), w_new_dsl,
        "mo_solve_nonhydro_stencil_62", "w_new_dsl", iteration);
    std::cout << "[DSL] serializing w_new as error is high.\n" << std::flush;
#endif
  };
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_62", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_62(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_62 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_62(w_now, grf_tend_w, w_new_before, dtime,
                                   verticalStart, verticalEnd, horizontalStart,
                                   horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_62 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_62...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_62(w_new_before, w_new, w_new_rel_tol,
                                      w_new_abs_tol, iteration);
...
void setup_mo_solve_nonhydro_stencil_62(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_62::setup(
      mesh, k_size, stream, w_new_k_size);
}

void free_mo_solve_nonhydro_stencil_62() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_62::free();
}
}
