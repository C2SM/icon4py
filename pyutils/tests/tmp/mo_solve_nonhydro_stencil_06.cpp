...
#include "mo_solve_nonhydro_stencil_06.hpp"
...
class mo_solve_nonhydro_stencil_06 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
private:
  ...
  inline static int z_dexner_dz_c_1_kSize_;
...
public:
  ...
  static int get_z_dexner_dz_c_1_KSize() { return z_dexner_dz_c_1_kSize_; }

  static void free() {}

  static void setup(const dawn::GlobalGpuTriMesh *mesh, int kSize,
                    cudaStream_t stream, const int z_dexner_dz_c_1_kSize) {
    mesh_ = GpuTriMesh(mesh);
    kSize_ = kSize;
    is_setup_ = true;
    stream_ = stream;
    z_dexner_dz_c_1_kSize_ = z_dexner_dz_c_1_kSize;
  }
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_06 has not been set up! make sure "
    ...
    generated::mo_solve_nonhydro_stencil_06(connectivities)(
        cuda_backend, z_exner_ic_sid, inv_ddqz_z_full_sid, z_dexner_dz_c_1_sid,
  ...
  void copy_pointers(double *z_exner_ic, double *inv_ddqz_z_full,
                     double *z_dexner_dz_c_1) {
...
void run_mo_solve_nonhydro_stencil_06(
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_06 s;
  s.copy_pointers(z_exner_ic, inv_ddqz_z_full, z_dexner_dz_c_1);
...
bool verify_mo_solve_nonhydro_stencil_06(const double *z_dexner_dz_c_1_dsl,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_06::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_06::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_06::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int z_dexner_dz_c_1_kSize = dawn_generated::cuda_ico::
      mo_solve_nonhydro_stencil_06::get_z_dexner_dz_c_1_KSize();
  stencilMetrics = ::dawn::verify_field(
      stream, (mesh.CellStride) * z_dexner_dz_c_1_kSize, z_dexner_dz_c_1_dsl,
      z_dexner_dz_c_1, "z_dexner_dz_c_1", z_dexner_dz_c_1_rel_tol,
      z_dexner_dz_c_1_abs_tol, iteration);
#ifdef __SERIALIZE_METRICS
  MetricsSerialiser serialiser_z_dexner_dz_c_1(
      stencilMetrics, metricsNameFromEnvVar("SLURM_JOB_ID"),
      "mo_solve_nonhydro_stencil_06", "z_dexner_dz_c_1");
  serialiser_z_dexner_dz_c_1.writeJson(iteration);
#endif
  if (!stencilMetrics.isValid) {
#ifdef __SERIALIZE_ON_ERROR
    serialize_dense_cells(0, (mesh.NumCells - 1), z_dexner_dz_c_1_kSize,
                          (mesh.CellStride), z_dexner_dz_c_1,
                          "mo_solve_nonhydro_stencil_06", "z_dexner_dz_c_1",
                          iteration);
    serialize_dense_cells(0, (mesh.NumCells - 1), z_dexner_dz_c_1_kSize,
                          (mesh.CellStride), z_dexner_dz_c_1_dsl,
                          "mo_solve_nonhydro_stencil_06", "z_dexner_dz_c_1_dsl",
                          iteration);
    std::cout << "[DSL] serializing z_dexner_dz_c_1 as error is high.\n"
              << std::flush;
#endif
  };
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_06", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_06(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_06 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_06(z_exner_ic, inv_ddqz_z_full,
                                   z_dexner_dz_c_1_before, verticalStart,
                                   verticalEnd, horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_06 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_06...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_06(z_dexner_dz_c_1_before, z_dexner_dz_c_1,
...
void setup_mo_solve_nonhydro_stencil_06(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_06::setup(
      mesh, k_size, stream, z_dexner_dz_c_1_k_size);
}

void free_mo_solve_nonhydro_stencil_06() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_06::free();
}
}
