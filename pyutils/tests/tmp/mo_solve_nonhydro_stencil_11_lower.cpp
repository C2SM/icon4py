...
#include "mo_solve_nonhydro_stencil_11_lower.hpp"
...
class mo_solve_nonhydro_stencil_11_lower {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_11_lower has not been set up! make "
    ...
    generated::mo_solve_nonhydro_stencil_11_lower(connectivities)(
        cuda_backend, z_theta_v_pr_ic_sid, horizontalStart, horizontalEnd,
  ...
  void copy_pointers(double *z_theta_v_pr_ic) {
...
void run_mo_solve_nonhydro_stencil_11_lower(double *z_theta_v_pr_ic,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_11_lower s;
  s.copy_pointers(z_theta_v_pr_ic);
...
bool verify_mo_solve_nonhydro_stencil_11_lower(
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_11_lower::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_11_lower::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_11_lower::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int z_theta_v_pr_ic_kSize = dawn_generated::cuda_ico::
      mo_solve_nonhydro_stencil_11_lower::get_z_theta_v_pr_ic_KSize();
  ...
  MetricsSerialiser serialiser_z_theta_v_pr_ic(
      ...
      "mo_solve_nonhydro_stencil_11_lower", "z_theta_v_pr_ic");
...
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_11_lower", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_11_lower(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_11_lower ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_11_lower(z_theta_v_pr_ic_before, verticalStart,
                                         verticalEnd, horizontalStart,
                                         horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_11_lower run time: " << time
            << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_11_lower...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_11_lower(
      z_theta_v_pr_ic_before, z_theta_v_pr_ic, z_theta_v_pr_ic_rel_tol,
      z_theta_v_pr_ic_abs_tol, iteration);

  iteration++;
}

void setup_mo_solve_nonhydro_stencil_11_lower(
    dawn::GlobalGpuTriMesh *mesh, int k_size, cudaStream_t stream,
    const int z_theta_v_pr_ic_k_size) {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_11_lower::setup(
      mesh, k_size, stream, z_theta_v_pr_ic_k_size);
}

void free_mo_solve_nonhydro_stencil_11_lower() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_11_lower::free();
}
}
