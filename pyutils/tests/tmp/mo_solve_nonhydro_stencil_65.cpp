...
#include "mo_solve_nonhydro_stencil_65.hpp"
...
class mo_solve_nonhydro_stencil_65 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_65 has not been set up! make sure "
    ...
    generated::mo_solve_nonhydro_stencil_65(connectivities)(
        cuda_backend, rho_ic_sid, vwind_expl_wgt_sid, vwind_impl_wgt_sid,
        w_now_sid, w_new_sid, w_concorr_c_sid, mass_flx_ic_sid, r_nsubsteps_gp,
  ...
  void copy_pointers(double *rho_ic, double *vwind_expl_wgt,
                     double *vwind_impl_wgt, double *w_now, double *w_new,
                     double *w_concorr_c, double *mass_flx_ic,
                     double r_nsubsteps) {
    rho_ic_ = rho_ic;
...
void run_mo_solve_nonhydro_stencil_65(
    ...
    const int horizontalStart, const int horizontalEnd) {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_65 s;
  s.copy_pointers(rho_ic, vwind_expl_wgt, vwind_impl_wgt, w_now, w_new,
                  w_concorr_c, mass_flx_ic, r_nsubsteps);
...
bool verify_mo_solve_nonhydro_stencil_65(const double *mass_flx_ic_dsl,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_65::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_65::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_65::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int mass_flx_ic_kSize = dawn_generated::cuda_ico::
      mo_solve_nonhydro_stencil_65::get_mass_flx_ic_KSize();
  stencilMetrics =
  ...
  MetricsSerialiser serialiser_mass_flx_ic(
      ...
      "mo_solve_nonhydro_stencil_65", "mass_flx_ic");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_cells(
        ...
        mass_flx_ic, "mo_solve_nonhydro_stencil_65", "mass_flx_ic", iteration);
    serialize_dense_cells(0, (mesh.NumCells - 1), mass_flx_ic_kSize,
                          (mesh.CellStride), mass_flx_ic_dsl,
                          "mo_solve_nonhydro_stencil_65", "mass_flx_ic_dsl",
                          iteration);
...
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_65", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_65(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_65 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_65(
      rho_ic, vwind_expl_wgt, vwind_impl_wgt, w_now, w_new, w_concorr_c,
      mass_flx_ic_before, r_nsubsteps, verticalStart, verticalEnd,
      horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_65 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_65...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_65(mass_flx_ic_before, mass_flx_ic,
...
void setup_mo_solve_nonhydro_stencil_65(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_65::setup(
      mesh, k_size, stream, mass_flx_ic_k_size);
}

void free_mo_solve_nonhydro_stencil_65() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_65::free();
}
}
