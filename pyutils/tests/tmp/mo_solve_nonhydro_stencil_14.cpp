...
#include "mo_solve_nonhydro_stencil_14.hpp"
...
class mo_solve_nonhydro_stencil_14 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_14 has not been set up! make sure "
    ...
    generated::mo_solve_nonhydro_stencil_14(connectivities)(
...
extern "C" {
void run_mo_solve_nonhydro_stencil_14(double *z_rho_e, double *z_theta_v_e,
                                      const int verticalStart,
                                      const int verticalEnd,
                                      const int horizontalStart,
                                      const int horizontalEnd) {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_14 s;
  s.copy_pointers(z_rho_e, z_theta_v_e);
  s.run(verticalStart, verticalEnd, horizontalStart, horizontalEnd);
  return;
}

bool verify_mo_solve_nonhydro_stencil_14(
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_14::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_14::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_14::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int z_rho_e_kSize = dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_14::
      get_z_rho_e_KSize();
  ...
  MetricsSerialiser serialiser_z_rho_e(
      ...
      "mo_solve_nonhydro_stencil_14", "z_rho_e");
  ...
  MetricsSerialiser serialiser_z_theta_v_e(
      stencilMetrics, metricsNameFromEnvVar("SLURM_JOB_ID"),
      "mo_solve_nonhydro_stencil_14", "z_theta_v_e");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_edges(
        ...
        z_theta_v_e, "mo_solve_nonhydro_stencil_14", "z_theta_v_e", iteration);
    serialize_dense_edges(0, (mesh.NumEdges - 1), z_theta_v_e_kSize,
                          (mesh.EdgeStride), z_theta_v_e_dsl,
                          "mo_solve_nonhydro_stencil_14", "z_theta_v_e_dsl",
                          iteration);
...
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_14", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_14(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_14 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_14(z_rho_e_before, z_theta_v_e_before,
                                   verticalStart, verticalEnd, horizontalStart,
                                   horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_14 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_14...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_14(
...
void setup_mo_solve_nonhydro_stencil_14(dawn::GlobalGpuTriMesh *mesh,
                                        int k_size, cudaStream_t stream,
                                        const int z_rho_e_k_size,
                                        const int z_theta_v_e_k_size) {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_14::setup(
      mesh, k_size, stream, z_rho_e_k_size, z_theta_v_e_k_size);
}

void free_mo_solve_nonhydro_stencil_14() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_14::free();
}
}
