...
#include "mo_velocity_advection_stencil_06.hpp"
...
class mo_velocity_advection_stencil_06 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_velocity_advection_stencil_06 has not been set up! make sure "
    ...
    generated::mo_velocity_advection_stencil_06(connectivities)(
        cuda_backend, wgtfacq_e_sid, vn_sid, vn_ie_sid, horizontalStart,
  ...
  void copy_pointers(double *wgtfacq_e, double *vn, double *vn_ie) {
...
void run_mo_velocity_advection_stencil_06(
  ...
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_06 s;
  s.copy_pointers(wgtfacq_e, vn, vn_ie);
...
bool verify_mo_velocity_advection_stencil_06(const double *vn_ie_dsl,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_06::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_06::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_06::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int vn_ie_kSize = dawn_generated::cuda_ico::mo_velocity_advection_stencil_06::
      get_vn_ie_KSize();
  ...
  MetricsSerialiser serialiser_vn_ie(
      ...
      "mo_velocity_advection_stencil_06", "vn_ie");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_edges(
        ...
        "mo_velocity_advection_stencil_06", "vn_ie", iteration);
    serialize_dense_edges(
        0, (mesh.NumEdges - 1), vn_ie_kSize, (mesh.EdgeStride), vn_ie_dsl,
        "mo_velocity_advection_stencil_06", "vn_ie_dsl", iteration);
    std::cout << "[DSL] serializing vn_ie as error is high.\n" << std::flush;
#endif
  };
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_velocity_advection_stencil_06", iteration);
#endif
...
void run_and_verify_mo_velocity_advection_stencil_06(
  ...
  std::cout << "[DSL] Running stencil mo_velocity_advection_stencil_06 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_velocity_advection_stencil_06(wgtfacq_e, vn, vn_ie_before,
                                       verticalStart, verticalEnd,
                                       horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_velocity_advection_stencil_06 run time: " << time
            << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_velocity_advection_stencil_06...\n"
            << std::flush;
  verify_mo_velocity_advection_stencil_06(vn_ie_before, vn_ie, vn_ie_rel_tol,
...
void setup_mo_velocity_advection_stencil_06(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_06::setup(
      mesh, k_size, stream, vn_ie_k_size);
}

void free_mo_velocity_advection_stencil_06() {
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_06::free();
}
}
