...
#include "mo_solve_nonhydro_stencil_47.hpp"
...
class mo_solve_nonhydro_stencil_47 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_47 has not been set up! make sure "
    ...
    generated::mo_solve_nonhydro_stencil_47(connectivities)(
        cuda_backend, w_nnew_sid, z_contr_w_fl_l_sid, w_concorr_c_sid,
...
void run_mo_solve_nonhydro_stencil_47(double *w_nnew, double *z_contr_w_fl_l,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_47 s;
  s.copy_pointers(w_nnew, z_contr_w_fl_l, w_concorr_c);
...
bool verify_mo_solve_nonhydro_stencil_47(
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_47::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_47::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_47::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int w_nnew_kSize = dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_47::
      get_w_nnew_KSize();
  ...
  MetricsSerialiser serialiser_w_nnew(stencilMetrics,
                                      ...
                                      "mo_solve_nonhydro_stencil_47", "w_nnew");
  ...
  MetricsSerialiser serialiser_z_contr_w_fl_l(
      ...
      "mo_solve_nonhydro_stencil_47", "z_contr_w_fl_l");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_cells(0, (mesh.NumCells - 1), z_contr_w_fl_l_kSize,
                          ...
                          "mo_solve_nonhydro_stencil_47", "z_contr_w_fl_l_dsl",
                          iteration);
...
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_47", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_47(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_47 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_47(w_nnew_before, z_contr_w_fl_l_before,
                                   w_concorr_c, verticalStart, verticalEnd,
                                   horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_47 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_47...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_47(
...
void setup_mo_solve_nonhydro_stencil_47(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_47::setup(
      mesh, k_size, stream, w_nnew_k_size, z_contr_w_fl_l_k_size);
}

void free_mo_solve_nonhydro_stencil_47() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_47::free();
}
}
