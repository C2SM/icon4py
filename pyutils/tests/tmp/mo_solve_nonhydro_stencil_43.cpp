...
#include "mo_solve_nonhydro_stencil_43.hpp"
...
class mo_solve_nonhydro_stencil_43 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_43 has not been set up! make sure "
    ...
    generated::mo_solve_nonhydro_stencil_43(connectivities)(
        cuda_backend, z_w_expl_sid, w_nnow_sid, ddt_w_adv_ntl1_sid,
        z_th_ddz_exner_c_sid, z_contr_w_fl_l_sid, rho_ic_sid, w_concorr_c_sid,
        vwind_expl_wgt_sid, dtime_gp, cpd_gp, horizontalStart, horizontalEnd,
  ...
  void copy_pointers(double *z_w_expl, double *w_nnow, double *ddt_w_adv_ntl1,
                     double *z_th_ddz_exner_c, double *z_contr_w_fl_l,
                     double *rho_ic, double *w_concorr_c,
                     double *vwind_expl_wgt, double dtime, double cpd) {
    z_w_expl_ = z_w_expl;
    w_nnow_ = w_nnow;
...
void run_mo_solve_nonhydro_stencil_43(
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_43 s;
  s.copy_pointers(z_w_expl, w_nnow, ddt_w_adv_ntl1, z_th_ddz_exner_c,
                  z_contr_w_fl_l, rho_ic, w_concorr_c, vwind_expl_wgt, dtime,
                  cpd);
...
bool verify_mo_solve_nonhydro_stencil_43(
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_43::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_43::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_43::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int z_w_expl_kSize = dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_43::
      get_z_w_expl_KSize();
  ...
  MetricsSerialiser serialiser_z_w_expl(
      ...
      "mo_solve_nonhydro_stencil_43", "z_w_expl");
  ...
  MetricsSerialiser serialiser_z_contr_w_fl_l(
      stencilMetrics, metricsNameFromEnvVar("SLURM_JOB_ID"),
      "mo_solve_nonhydro_stencil_43", "z_contr_w_fl_l");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_cells(0, (mesh.NumCells - 1), z_contr_w_fl_l_kSize,
                          ...
                          "mo_solve_nonhydro_stencil_43", "z_contr_w_fl_l_dsl",
                          iteration);
...
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_43", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_43(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_43 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_43(
      z_w_expl_before, w_nnow, ddt_w_adv_ntl1, z_th_ddz_exner_c,
      z_contr_w_fl_l_before, rho_ic, w_concorr_c, vwind_expl_wgt, dtime, cpd,
      verticalStart, verticalEnd, horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_43 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_43...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_43(
...
void setup_mo_solve_nonhydro_stencil_43(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_43::setup(
      mesh, k_size, stream, z_w_expl_k_size, z_contr_w_fl_l_k_size);
}

void free_mo_solve_nonhydro_stencil_43() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_43::free();
}
}
