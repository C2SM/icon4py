...
#include "mo_solve_nonhydro_stencil_05.hpp"
...
class mo_solve_nonhydro_stencil_05 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_05 has not been set up! make sure "
    ...
    generated::mo_solve_nonhydro_stencil_05(connectivities)(
        cuda_backend, wgtfac_c_sid, z_exner_ex_pr_sid, z_exner_ic_sid,
  ...
  void copy_pointers(double *wgtfac_c, double *z_exner_ex_pr,
                     double *z_exner_ic) {
    wgtfac_c_ = wgtfac_c;
...
void run_mo_solve_nonhydro_stencil_05(double *wgtfac_c, double *z_exner_ex_pr,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_05 s;
  s.copy_pointers(wgtfac_c, z_exner_ex_pr, z_exner_ic);
...
bool verify_mo_solve_nonhydro_stencil_05(const double *z_exner_ic_dsl,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_05::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_05::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_05::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int z_exner_ic_kSize = dawn_generated::cuda_ico::
      mo_solve_nonhydro_stencil_05::get_z_exner_ic_KSize();
  ...
  MetricsSerialiser serialiser_z_exner_ic(
      ...
      "mo_solve_nonhydro_stencil_05", "z_exner_ic");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_cells(
        ...
        "mo_solve_nonhydro_stencil_05", "z_exner_ic", iteration);
    serialize_dense_cells(0, (mesh.NumCells - 1), z_exner_ic_kSize,
                          (mesh.CellStride), z_exner_ic_dsl,
                          "mo_solve_nonhydro_stencil_05", "z_exner_ic_dsl",
                          iteration);
...
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_05", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_05(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_05 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_05(wgtfac_c, z_exner_ex_pr, z_exner_ic_before,
                                   verticalStart, verticalEnd, horizontalStart,
                                   horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_05 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_05...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_05(z_exner_ic_before, z_exner_ic,
...
void setup_mo_solve_nonhydro_stencil_05(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_05::setup(
      mesh, k_size, stream, z_exner_ic_k_size);
}

void free_mo_solve_nonhydro_stencil_05() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_05::free();
}
}
