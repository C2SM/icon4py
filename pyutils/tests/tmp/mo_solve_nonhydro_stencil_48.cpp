...
#include "mo_solve_nonhydro_stencil_48.hpp"
...
class mo_solve_nonhydro_stencil_48 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_48 has not been set up! make sure "
    ...
    generated::mo_solve_nonhydro_stencil_48(connectivities)(
...
extern "C" {
void run_mo_solve_nonhydro_stencil_48(
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_48 s;
...
bool verify_mo_solve_nonhydro_stencil_48(
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_48::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_48::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_48::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int z_rho_expl_kSize = dawn_generated::cuda_ico::
      mo_solve_nonhydro_stencil_48::get_z_rho_expl_KSize();
  ...
  MetricsSerialiser serialiser_z_rho_expl(
      ...
      "mo_solve_nonhydro_stencil_48", "z_rho_expl");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_cells(
        ...
        "mo_solve_nonhydro_stencil_48", "z_rho_expl", iteration);
    serialize_dense_cells(0, (mesh.NumCells - 1), z_rho_expl_kSize,
                          (mesh.CellStride), z_rho_expl_dsl,
                          "mo_solve_nonhydro_stencil_48", "z_rho_expl_dsl",
                          iteration);
  ...
  int z_exner_expl_kSize = dawn_generated::cuda_ico::
      mo_solve_nonhydro_stencil_48::get_z_exner_expl_KSize();
  stencilMetrics = ::dawn::verify_field(
  ...
  MetricsSerialiser serialiser_z_exner_expl(
      stencilMetrics, metricsNameFromEnvVar("SLURM_JOB_ID"),
      "mo_solve_nonhydro_stencil_48", "z_exner_expl");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_cells(0, (mesh.NumCells - 1), z_exner_expl_kSize,
                          ...
                          "mo_solve_nonhydro_stencil_48", "z_exner_expl_dsl",
                          iteration);
...
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_48", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_48(
    double *z_rho_expl, double *z_exner_expl, double *rho_nnow,
    double *inv_ddqz_z_full, double *z_flxdiv_mass, double *z_contr_w_fl_l,
    double *exner_pr, double *z_beta, double *z_flxdiv_theta,
    double *theta_v_ic, double *ddt_exner_phy, double dtime,
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_48 ("
            << iteration << ") ...\n"
            << std::flush;
  ...
  std::cout << "[DSL] mo_solve_nonhydro_stencil_48 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_48...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_48(
...
void setup_mo_solve_nonhydro_stencil_48(dawn::GlobalGpuTriMesh *mesh,
                                        int k_size, cudaStream_t stream,
                                        const int z_rho_expl_k_size,
                                        const int z_exner_expl_k_size) {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_48::setup(
      mesh, k_size, stream, z_rho_expl_k_size, z_exner_expl_k_size);
}

void free_mo_solve_nonhydro_stencil_48() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_48::free();
}
}
