...
#include "mo_velocity_advection_stencil_05.hpp"
...
class mo_velocity_advection_stencil_05 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_velocity_advection_stencil_05 has not been set up! make sure "
    ...
    generated::mo_velocity_advection_stencil_05(connectivities)(
        cuda_backend, vn_sid, vt_sid, vn_ie_sid, z_vt_ie_sid, z_kin_hor_e_sid,
  ...
  void copy_pointers(double *vn, double *vt, double *vn_ie, double *z_vt_ie,
                     double *z_kin_hor_e) {
    vn_ = vn;
    vt_ = vt;
    vn_ie_ = vn_ie;
...
extern "C" {
void run_mo_velocity_advection_stencil_05(double *vn, double *vt, double *vn_ie,
                                          double *z_vt_ie, double *z_kin_hor_e,
                                          const int verticalStart,
                                          const int verticalEnd,
                                          const int horizontalStart,
                                          const int horizontalEnd) {
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_05 s;
  s.copy_pointers(vn, vt, vn_ie, z_vt_ie, z_kin_hor_e);
  s.run(verticalStart, verticalEnd, horizontalStart, horizontalEnd);
  return;
}

bool verify_mo_velocity_advection_stencil_05(
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_05::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_05::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_05::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int vn_ie_kSize = dawn_generated::cuda_ico::mo_velocity_advection_stencil_05::
      get_vn_ie_KSize();
  ...
  MetricsSerialiser serialiser_vn_ie(
      ...
      "mo_velocity_advection_stencil_05", "vn_ie");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_edges(
        ...
        "mo_velocity_advection_stencil_05", "vn_ie", iteration);
    serialize_dense_edges(
        0, (mesh.NumEdges - 1), vn_ie_kSize, (mesh.EdgeStride), vn_ie_dsl,
        "mo_velocity_advection_stencil_05", "vn_ie_dsl", iteration);
    std::cout << "[DSL] serializing vn_ie as error is high.\n" << std::flush;
#endif
  }
  int z_vt_ie_kSize = dawn_generated::cuda_ico::
      mo_velocity_advection_stencil_05::get_z_vt_ie_KSize();
  stencilMetrics = ::dawn::verify_field(
  ...
  MetricsSerialiser serialiser_z_vt_ie(
      stencilMetrics, metricsNameFromEnvVar("SLURM_JOB_ID"),
      "mo_velocity_advection_stencil_05", "z_vt_ie");
  serialiser_z_vt_ie.writeJson(iteration);
#endif
  if (!stencilMetrics.isValid) {
#ifdef __SERIALIZE_ON_ERROR
    serialize_dense_edges(
        0, (mesh.NumEdges - 1), z_vt_ie_kSize, (mesh.EdgeStride), z_vt_ie,
        "mo_velocity_advection_stencil_05", "z_vt_ie", iteration);
    serialize_dense_edges(
        0, (mesh.NumEdges - 1), z_vt_ie_kSize, (mesh.EdgeStride), z_vt_ie_dsl,
        "mo_velocity_advection_stencil_05", "z_vt_ie_dsl", iteration);
    std::cout << "[DSL] serializing z_vt_ie as error is high.\n" << std::flush;
#endif
  }
  int z_kin_hor_e_kSize = dawn_generated::cuda_ico::
      mo_velocity_advection_stencil_05::get_z_kin_hor_e_KSize();
  stencilMetrics =
  ...
  MetricsSerialiser serialiser_z_kin_hor_e(
      stencilMetrics, metricsNameFromEnvVar("SLURM_JOB_ID"),
      "mo_velocity_advection_stencil_05", "z_kin_hor_e");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_edges(0, (mesh.NumEdges - 1), z_kin_hor_e_kSize,
                          ...
                          "mo_velocity_advection_stencil_05", "z_kin_hor_e_dsl",
                          iteration);
...
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_velocity_advection_stencil_05", iteration);
#endif
...
void run_and_verify_mo_velocity_advection_stencil_05(
  ...
  std::cout << "[DSL] Running stencil mo_velocity_advection_stencil_05 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_velocity_advection_stencil_05(
      vn, vt, vn_ie_before, z_vt_ie_before, z_kin_hor_e_before, verticalStart,
      verticalEnd, horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_velocity_advection_stencil_05 run time: " << time
            << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_velocity_advection_stencil_05...\n"
            << std::flush;
  verify_mo_velocity_advection_stencil_05(
...
void setup_mo_velocity_advection_stencil_05(dawn::GlobalGpuTriMesh *mesh,
                                            int k_size, cudaStream_t stream,
                                            const int vn_ie_k_size,
                                            const int z_vt_ie_k_size,
                                            const int z_kin_hor_e_k_size) {
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_05::setup(
      mesh, k_size, stream, vn_ie_k_size, z_vt_ie_k_size, z_kin_hor_e_k_size);
}

void free_mo_velocity_advection_stencil_05() {
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_05::free();
}
}
