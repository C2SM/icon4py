...
#include "mo_solve_nonhydro_stencil_45.hpp"
...
class mo_solve_nonhydro_stencil_45 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  static void setup(const dawn::GlobalGpuTriMesh *mesh, int kSize,
                    cudaStream_t stream, const int z_alpha_kSize) {
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_45 has not been set up! make sure "
    ...
    generated::mo_solve_nonhydro_stencil_45(connectivities)(
        cuda_backend, z_alpha_sid, horizontalStart, horizontalEnd,
  ...
  void copy_pointers(double *z_alpha) { z_alpha_ = z_alpha; }
};
} // namespace cuda_ico
} // namespace dawn_generated
...
void run_mo_solve_nonhydro_stencil_45(double *z_alpha, const int verticalStart,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_45 s;
  s.copy_pointers(z_alpha);
...
bool verify_mo_solve_nonhydro_stencil_45(const double *z_alpha_dsl,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_45::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_45::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_45::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int z_alpha_kSize = dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_45::
      get_z_alpha_KSize();
  ...
  MetricsSerialiser serialiser_z_alpha(
      ...
      "mo_solve_nonhydro_stencil_45", "z_alpha");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_cells(0, (mesh.NumCells - 1), z_alpha_kSize,
                          ...
                          "mo_solve_nonhydro_stencil_45", "z_alpha", iteration);
    serialize_dense_cells(
        0, (mesh.NumCells - 1), z_alpha_kSize, (mesh.CellStride), z_alpha_dsl,
        "mo_solve_nonhydro_stencil_45", "z_alpha_dsl", iteration);
    std::cout << "[DSL] serializing z_alpha as error is high.\n" << std::flush;
#endif
  };
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_45", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_45(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_45 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_45(z_alpha_before, verticalStart, verticalEnd,
                                   horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_45 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_45...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_45(z_alpha_before, z_alpha, z_alpha_rel_tol,
                                      z_alpha_abs_tol, iteration);
...
void setup_mo_solve_nonhydro_stencil_45(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_45::setup(
      mesh, k_size, stream, z_alpha_k_size);
}

void free_mo_solve_nonhydro_stencil_45() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_45::free();
}
}
