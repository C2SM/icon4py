...
#include "mo_velocity_advection_stencil_04.hpp"
...
class mo_velocity_advection_stencil_04 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_velocity_advection_stencil_04 has not been set up! make sure "
    ...
    generated::mo_velocity_advection_stencil_04(connectivities)(
...
void run_mo_velocity_advection_stencil_04(
    ...
    const int horizontalStart, const int horizontalEnd) {
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_04 s;
...
bool verify_mo_velocity_advection_stencil_04(
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_04::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_04::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_velocity_advection_stencil_04::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int z_w_concorr_me_kSize = dawn_generated::cuda_ico::
      mo_velocity_advection_stencil_04::get_z_w_concorr_me_KSize();
  ...
  MetricsSerialiser serialiser_z_w_concorr_me(
      ...
      "mo_velocity_advection_stencil_04", "z_w_concorr_me");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_edges(0, (mesh.NumEdges - 1), z_w_concorr_me_kSize,
                          ...
                          "mo_velocity_advection_stencil_04",
                          "z_w_concorr_me_dsl", iteration);
    std::cout << "[DSL] serializing z_w_concorr_me as error is high.\n"
              << std::flush;
#endif
  };
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_velocity_advection_stencil_04", iteration);
#endif
...
void run_and_verify_mo_velocity_advection_stencil_04(
  ...
  std::cout << "[DSL] Running stencil mo_velocity_advection_stencil_04 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_velocity_advection_stencil_04(
      vn, ddxn_z_full, ddxt_z_full, vt, z_w_concorr_me_before, verticalStart,
      verticalEnd, horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_velocity_advection_stencil_04 run time: " << time
            << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_velocity_advection_stencil_04...\n"
            << std::flush;
  verify_mo_velocity_advection_stencil_04(z_w_concorr_me_before, z_w_concorr_me,
...
void setup_mo_velocity_advection_stencil_04(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_04::setup(
      mesh, k_size, stream, z_w_concorr_me_k_size);
}

void free_mo_velocity_advection_stencil_04() {
  dawn_generated::cuda_ico::mo_velocity_advection_stencil_04::free();
}
}
