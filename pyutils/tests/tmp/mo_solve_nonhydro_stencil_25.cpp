...
#include "mo_solve_nonhydro_stencil_25.hpp"
...
class mo_solve_nonhydro_stencil_25 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
private:
  ...
  inline static int z_graddiv2_vn_kSize_;
...
public:
  ...
  static int get_z_graddiv2_vn_KSize() { return z_graddiv2_vn_kSize_; }

  static void free() {}

  static void setup(const dawn::GlobalGpuTriMesh *mesh, int kSize,
                    cudaStream_t stream, const int z_graddiv2_vn_kSize) {
    mesh_ = GpuTriMesh(mesh);
    kSize_ = kSize;
    is_setup_ = true;
    stream_ = stream;
    z_graddiv2_vn_kSize_ = z_graddiv2_vn_kSize;
  }
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_25 has not been set up! make sure "
    ...
    auto geofac_grdiv_sid_comp = sid::composite::keys<
        ...
        integral_constant<int, 4>>::make_values(geofac_grdiv_sid_0,
                                                ...
                                                geofac_grdiv_sid_3,
                                                geofac_grdiv_sid_4);
    generated::mo_solve_nonhydro_stencil_25(connectivities)(
        cuda_backend, geofac_grdiv_sid_comp, z_graddiv_vn_sid,
        z_graddiv2_vn_sid, horizontalStart, horizontalEnd, verticalStart,
        verticalEnd);
#ifndef NDEBUG
  ...
  void copy_pointers(double *geofac_grdiv, double *z_graddiv_vn,
                     double *z_graddiv2_vn) {
...
void run_mo_solve_nonhydro_stencil_25(
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_25 s;
  s.copy_pointers(geofac_grdiv, z_graddiv_vn, z_graddiv2_vn);
...
bool verify_mo_solve_nonhydro_stencil_25(const double *z_graddiv2_vn_dsl,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_25::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_25::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_25::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int z_graddiv2_vn_kSize = dawn_generated::cuda_ico::
      mo_solve_nonhydro_stencil_25::get_z_graddiv2_vn_KSize();
  stencilMetrics = ::dawn::verify_field(
      stream, (mesh.EdgeStride) * z_graddiv2_vn_kSize, z_graddiv2_vn_dsl,
      z_graddiv2_vn, "z_graddiv2_vn", z_graddiv2_vn_rel_tol,
      z_graddiv2_vn_abs_tol, iteration);
#ifdef __SERIALIZE_METRICS
  MetricsSerialiser serialiser_z_graddiv2_vn(
      stencilMetrics, metricsNameFromEnvVar("SLURM_JOB_ID"),
      "mo_solve_nonhydro_stencil_25", "z_graddiv2_vn");
  serialiser_z_graddiv2_vn.writeJson(iteration);
#endif
  if (!stencilMetrics.isValid) {
#ifdef __SERIALIZE_ON_ERROR
    serialize_dense_edges(0, (mesh.NumEdges - 1), z_graddiv2_vn_kSize,
                          (mesh.EdgeStride), z_graddiv2_vn,
                          "mo_solve_nonhydro_stencil_25", "z_graddiv2_vn",
                          iteration);
    serialize_dense_edges(0, (mesh.NumEdges - 1), z_graddiv2_vn_kSize,
                          (mesh.EdgeStride), z_graddiv2_vn_dsl,
                          "mo_solve_nonhydro_stencil_25", "z_graddiv2_vn_dsl",
                          iteration);
    std::cout << "[DSL] serializing z_graddiv2_vn as error is high.\n"
              << std::flush;
#endif
  };
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_25", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_25(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_25 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_25(geofac_grdiv, z_graddiv_vn,
                                   z_graddiv2_vn_before, verticalStart,
                                   verticalEnd, horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_25 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_25...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_25(z_graddiv2_vn_before, z_graddiv2_vn,
                                      z_graddiv2_vn_rel_tol,
                                      z_graddiv2_vn_abs_tol, iteration);
...
void setup_mo_solve_nonhydro_stencil_25(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_25::setup(
      mesh, k_size, stream, z_graddiv2_vn_k_size);
}

void free_mo_solve_nonhydro_stencil_25() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_25::free();
}
}
