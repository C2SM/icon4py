...
#include "mo_solve_nonhydro_stencil_53.hpp"
...
class mo_solve_nonhydro_stencil_53 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_53 has not been set up! make sure "
    ...
    generated::mo_solve_nonhydro_stencil_53(connectivities)(
        cuda_backend, z_q_sid, w_sid, horizontalStart, horizontalEnd,
  ...
  void copy_pointers(double *z_q, double *w) {
    z_q_ = z_q;
    w_ = w;
  }
};
...
void run_mo_solve_nonhydro_stencil_53(double *z_q, double *w,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_53 s;
  s.copy_pointers(z_q, w);
...
bool verify_mo_solve_nonhydro_stencil_53(const double *w_dsl, const double *w,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_53::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_53::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_53::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int w_kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_53::get_w_KSize();
  stencilMetrics =
...
void run_and_verify_mo_solve_nonhydro_stencil_53(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_53 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_53(z_q, w_before, verticalStart, verticalEnd,
                                   horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_53 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_53...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_53(w_before, w, w_rel_tol, w_abs_tol,
                                      iteration);
...
void setup_mo_solve_nonhydro_stencil_53(dawn::GlobalGpuTriMesh *mesh,
                                        ...
                                        const int w_k_size) {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_53::setup(
      mesh, k_size, stream, w_k_size);
}

void free_mo_solve_nonhydro_stencil_53() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_53::free();
}
}
