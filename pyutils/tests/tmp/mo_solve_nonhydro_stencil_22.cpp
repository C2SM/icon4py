...
#include "mo_solve_nonhydro_stencil_22.hpp"
...
class mo_solve_nonhydro_stencil_22 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_22 has not been set up! make sure "
    ...
    generated::mo_solve_nonhydro_stencil_22(connectivities)(
        cuda_backend, ipeidx_dsl_sid, pg_exdist_sid, z_hydro_corr_sid,
  ...
  void copy_pointers(int *ipeidx_dsl, double *pg_exdist, double *z_hydro_corr,
                     double *z_gradh_exner) {
    ipeidx_dsl_ = ipeidx_dsl;
    pg_exdist_ = pg_exdist;
    z_hydro_corr_ = z_hydro_corr;
...
void run_mo_solve_nonhydro_stencil_22(
    ...
    const int horizontalStart, const int horizontalEnd) {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_22 s;
  s.copy_pointers(ipeidx_dsl, pg_exdist, z_hydro_corr, z_gradh_exner);
...
bool verify_mo_solve_nonhydro_stencil_22(const double *z_gradh_exner_dsl,
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_22::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_22::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_22::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int z_gradh_exner_kSize = dawn_generated::cuda_ico::
      mo_solve_nonhydro_stencil_22::get_z_gradh_exner_KSize();
  ...
  MetricsSerialiser serialiser_z_gradh_exner(
      ...
      "mo_solve_nonhydro_stencil_22", "z_gradh_exner");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_edges(0, (mesh.NumEdges - 1), z_gradh_exner_kSize,
                          ...
                          "mo_solve_nonhydro_stencil_22", "z_gradh_exner_dsl",
                          iteration);
...
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_22", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_22(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_22 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_22(ipeidx_dsl, pg_exdist, z_hydro_corr,
                                   z_gradh_exner_before, verticalStart,
                                   verticalEnd, horizontalStart, horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_22 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_22...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_22(z_gradh_exner_before, z_gradh_exner,
...
void setup_mo_solve_nonhydro_stencil_22(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_22::setup(
      mesh, k_size, stream, z_gradh_exner_k_size);
}

void free_mo_solve_nonhydro_stencil_22() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_22::free();
}
}
