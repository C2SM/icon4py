...
#include "mo_solve_nonhydro_stencil_33.hpp"
...
class mo_solve_nonhydro_stencil_33 {
public:
  struct GpuTriMesh {
    int NumVertices;
    int NumEdges;
...
public:
  ...
  void run(const int verticalStart, const int verticalEnd,
    ...
    if (!is_setup_) {
      printf("mo_solve_nonhydro_stencil_33 has not been set up! make sure "
    ...
    generated::mo_solve_nonhydro_stencil_33(connectivities)(
        cuda_backend, vn_traj_sid, mass_flx_me_sid, horizontalStart,
  ...
  void copy_pointers(double *vn_traj, double *mass_flx_me) {
    vn_traj_ = vn_traj;
...
void run_mo_solve_nonhydro_stencil_33(double *vn_traj, double *mass_flx_me,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_33 s;
  s.copy_pointers(vn_traj, mass_flx_me);
...
bool verify_mo_solve_nonhydro_stencil_33(
  ...
  const auto &mesh =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_33::getMesh();
  cudaStream_t stream =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_33::getStream();
  int kSize =
      dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_33::getKSize();
  high_resolution_clock::time_point t_start = high_resolution_clock::now();
  struct VerificationMetrics stencilMetrics;

  int vn_traj_kSize = dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_33::
      get_vn_traj_KSize();
  ...
  MetricsSerialiser serialiser_vn_traj(
      ...
      "mo_solve_nonhydro_stencil_33", "vn_traj");
  ...
  MetricsSerialiser serialiser_mass_flx_me(
      stencilMetrics, metricsNameFromEnvVar("SLURM_JOB_ID"),
      "mo_solve_nonhydro_stencil_33", "mass_flx_me");
  ...
  if (!stencilMetrics.isValid) {
    ...
    serialize_dense_edges(
        ...
        mass_flx_me, "mo_solve_nonhydro_stencil_33", "mass_flx_me", iteration);
    serialize_dense_edges(0, (mesh.NumEdges - 1), mass_flx_me_kSize,
                          (mesh.EdgeStride), mass_flx_me_dsl,
                          "mo_solve_nonhydro_stencil_33", "mass_flx_me_dsl",
                          iteration);
...
#ifdef __SERIALIZE_ON_ERROR
  serialize_flush_iter("mo_solve_nonhydro_stencil_33", iteration);
#endif
...
void run_and_verify_mo_solve_nonhydro_stencil_33(
  ...
  std::cout << "[DSL] Running stencil mo_solve_nonhydro_stencil_33 ("
            << iteration << ") ...\n"
            << std::flush;
  run_mo_solve_nonhydro_stencil_33(vn_traj_before, mass_flx_me_before,
                                   verticalStart, verticalEnd, horizontalStart,
                                   horizontalEnd);

  std::cout << "[DSL] mo_solve_nonhydro_stencil_33 run time: " << time << "s\n"
            << std::flush;
  std::cout << "[DSL] Verifying stencil mo_solve_nonhydro_stencil_33...\n"
            << std::flush;
  verify_mo_solve_nonhydro_stencil_33(
...
void setup_mo_solve_nonhydro_stencil_33(dawn::GlobalGpuTriMesh *mesh,
  ...
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_33::setup(
      mesh, k_size, stream, vn_traj_k_size, mass_flx_me_k_size);
}

void free_mo_solve_nonhydro_stencil_33() {
  dawn_generated::cuda_ico::mo_solve_nonhydro_stencil_33::free();
}
}
